...
class NoSuchEntryPoint(Exception):
    ...
    def __str__(self):
        return "No {!r} entry point found in group {!r}".format(
...
class EntryPoint(object):
    def __init__(
        self, name, module_name, object_name, extras=None, distro=None
    ):
        self.name = name
...
def iter_files_distros(path=None, repeated_distro="first"):
    ...
    for folder in path:
            ...
            with zipfile.ZipFile(folder) as zf:
                for info in zf.infolist():
                    ...
                    with zf.open(info) as f:
                        fu = io.TextIOWrapper(f)
                        cp.read_file(
                            fu, source=osp.join(folder, info.filename)
                        )
                    yield cp, distro
        ...
        for path in itertools.chain(
            ...
            distro_name_version = osp.splitext(
