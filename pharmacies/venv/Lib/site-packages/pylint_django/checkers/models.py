...
def _is_unicode_or_str_in_python_2_compatibility(method):
    ...
    if (
        method.name == "__str__"
        and _has_python_2_unicode_compatible_decorator(method.parent)
    ):
        return True
    ...
    def visit_classdef(self, node):
        ...
        for child in node.get_children():
            ...
            if isinstance(child, FunctionDef) and child.name == "__unicode__":
                if PY3:
                    self.add_message(
                        "W%s02" % BASE_ID, args=node.name, node=node
                    )
                return
        ...
        for method in node.methods():
            if (
                method.parent != node
                and _is_unicode_or_str_in_python_2_compatibility(method)
            ):
