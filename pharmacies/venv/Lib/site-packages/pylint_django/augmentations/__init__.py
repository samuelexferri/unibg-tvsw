...
def ignore_import_warnings_for_related_fields(orig_method, self, node):
    ...
    iterat = (
        consumer.to_consume.items if PY3 else consumer.to_consume.iteritems
    )
    for name, stmts in iterat():
        if isinstance(stmts[0], ImportFrom):
            if any(
                [
                    n[0] in ("ForeignKey", "OneToOneField")
                    for n in stmts[0].names
                ]
            ):
                continue
        new_things[name] = stmts

    consumer._atomic = ScopeConsumer(
        new_things, consumer.consumed, consumer.scope_type
...
def is_model_field_display_method(node):
    ...
    if node.last_child():
        ...
        try:
            for cls in node.last_child().inferred():
                if node_is_subclass(
                    cls, "django.db.models.base.Model", ".Model"
                ):
                    return True
        except InferenceError:
            return False
...
def apply_wrapped_augmentations():
    ...
    current_is_name_ignored = (
        VariablesChecker._is_name_ignored
    )  # pylint: disable=protected-access
    if current_is_name_ignored.__name__ == "_is_name_ignored":
        # pylint: disable=protected-access
        VariablesChecker._is_name_ignored = wrap(
            current_is_name_ignored,
            ignore_unused_argument_warnings_for_request,
        )
...
def apply_augmentations(linter):
    ...
    suppress_message(
        linter, TypeChecker.visit_attribute, "no-member", is_style_attribute
    )
    suppress_message(
        linter,
    ...
    suppress_message(
        linter, TypeChecker.visit_attribute, "no-member", is_manager_attribute
    )
    suppress_message(
        linter, TypeChecker.visit_attribute, "no-member", is_admin_attribute
    )
    suppress_message(
        linter, TypeChecker.visit_attribute, "no-member", is_model_attribute
    )
    suppress_message(
        linter, TypeChecker.visit_attribute, "no-member", is_field_attribute
    )
    suppress_message(
        linter,
        TypeChecker.visit_attribute,
    ...
    suppress_message(
        linter, TypeChecker.visit_attribute, "no-member", is_ipfield_attribute
    )
    suppress_message(
        linter,
        TypeChecker.visit_attribute,
    ...
    suppress_message(
        linter, TypeChecker.visit_attribute, "no-member", is_form_attribute
    )
    ...
    pylint_newstyle_classdef_compat(
        linter, "old-style-class", is_model_meta_subclass
    )
    suppress_message(
        linter, ClassChecker.visit_classdef, "no-init", is_model_meta_subclass
    )
    suppress_message(
        linter,
    ...
    pylint_newstyle_classdef_compat(
        linter, "old-style-class", is_model_media_subclass
    )
    suppress_message(
        linter, ClassChecker.visit_classdef, "no-init", is_model_media_subclass
    )
    suppress_message(
        linter,
        MisdesignChecker.leave_classdef,
    ...
    pylint_newstyle_classdef_compat(
        linter, "old-style-class", is_model_mpttmeta_subclass
    )
    suppress_message(
        linter,
        ClassChecker.visit_classdef,
