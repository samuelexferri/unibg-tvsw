...
HIDE_WINDOWS_KNOWN_ERRORS = is_win and os.environ.get(
    "HIDE_WINDOWS_KNOWN_ERRORS", True
)
...
def py_where(program, path=None):
    ...
    for folder in path.split(os.pathsep):
        ...
        if folder:
            ...
            for f in [exe_path] + [
                "%s%s" % (exe_path, e) for e in winprog_exts
            ]:
                if is_exec(f):
                    progs.append(f)
    return progs
...
class RemoteProgress(object):
    ...
    re_op_relative = re.compile(
        r"(remote: )?([\w\s]+):\s+(\d+)% \((\d+)/(\d+)\)(.*)"
    )
            ...
            try:
                ...
                if config_reader is not None:
                    setattr(
                        actor,
                        attr,
                        config_reader.get_value("user", cvar, default),
                    )
...
class BlockingLockFile(LockFile):
    ...
    def __init__(
        self, file_path, check_interval_s=0.3, max_block_time_s=maxsize
    ):
