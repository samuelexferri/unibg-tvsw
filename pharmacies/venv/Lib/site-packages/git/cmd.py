...
def handle_process_output(
    ...
    def pump_stream(cmdline, name, stream, is_decode, handler):
        try:
            ...
            log.error(
                "Pumping %r of cmd(%s) failed due to: %r", name, cmdline, ex
            )
...
PROC_CREATIONFLAGS = (
    CREATE_NO_WINDOW | subprocess.CREATE_NEW_PROCESS_GROUP if is_win else 0
)
...
class Git(LazyMixin):
    ...
    def __getattr__(self, name):
        ...
        return lambda *args, **kwargs: self._call_process(
            name, *args, **kwargs
        )
    ...
    def transform_kwargs(self, split_single_char_options=True, **kwargs):
        ...
        for k, v in kwargs.items():
            if isinstance(v, (list, tuple)):
                for value in v:
                    args += self.transform_kwarg(
                        k, value, split_single_char_options
                    )
            else:
    ...
    def get_object_header(self, ref):
        ...
        cmd = self._get_persistent_cmd(
            "cat_file_header", "cat_file", batch_check=True
        )
    ...
    def stream_object_data(self, ref):
        ...
        return (
            ...
            typename,
            size,
            self.CatFileContentStream(size, cmd.stdout),
        )
