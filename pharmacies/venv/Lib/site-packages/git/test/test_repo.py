...
from git.test.lib import TestBase, with_rw_repo, fixture
...
class TestRepo(TestBase):
    ...
    def test_new_should_raise_on_invalid_repo_location(self):
        self.assertRaises(
            InvalidGitRepositoryError, Repo, tempfile.gettempdir()
        )
    ...
    def test_repo_creation_from_different_paths(self, rw_repo):
        ...
        self.assertEqual(
            r_from_gitdir.git.working_dir, rw_repo.git.working_dir
        )
    ...
    def test_commits(self):
        ...
        self.assertEqual(
            ["c76852d0bff115720af3f27acdb084c59361e5f6"],
            [p.hexsha for p in c.parents],
        )
        self.assertEqual(
            "ce41fc29549042f1aa09cc03174896cf23f112e3", c.tree.hexsha
        )
    ...
    def test_clone_from_pathlib(self, rw_dir):
        ...
        Repo.clone_from(
            original_repo.git_dir, pathlib.Path(rw_dir) / "clone_pathlib"
        )
    ...
    def test_clone_from_pathlib_withConfig(self, rw_dir):
        ...
        self.assertEqual(
            cloned.config_reader().get_value("core", "filemode"), False
        )
        self.assertEqual(
            cloned.config_reader().get_value('submodule "repo"', "update"),
            "checkout",
        )
    ...
    def test_clone_from_with_path_contains_unicode(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                Repo.clone_from(
                    url=self._small_repo_url(), to_path=path_with_unicode,
                )
    ...
    def test_max_chunk_size(self, repo):
        ...
        for chunk_size in [16, 128, 1024]:
            repo.git.status(
                output_stream=TestOutputStream(chunk_size),
        ...
        repo.git.log(
            n=100, output_stream=TestOutputStream(io.DEFAULT_BUFFER_SIZE)
        )
    ...
    def test_is_dirty_with_path(self, rwrepo):
        ...
        with open(
            osp.join(rwrepo.working_dir, "doc", "no-such-file.txt"), "wt"
        ) as f:
            f.write("junk")
        assert rwrepo.is_dirty(path="doc") is False
    ...
    def test_blame_complex_revision(self, git):
        ...
        self.assertEqual(
            len(res[0][1]), 83, "Unexpected amount of parsed blame lines"
        )
    ...
    def test_creation_deletion(self):
        ...
        with self.rorepo.config_writer():
            pass
        try:
            remote = self.rorepo.create_remote(
                "new_remote", "git@server:repo.git"
            )
        finally:
    ...
    def test_git_cmd(self):
        ...
        self.assertEqual(
            s._stream.tell(), len(d)
        )  # must have scrubbed to the end
    ...
    def test_rev_parse(self):
        ...
        self.assertEqual(
            rev_parse(rev + ":CHANGES"), tag.object.tree["CHANGES"]
        )
        ...
        for i, binsha in enumerate(self.rorepo.odb.sha_iter()):
            self.assertEqual(
                rev_parse(
                    bin_to_hex(binsha)[: 8 - (i % 2)].decode("ascii")
                ).binsha,
                binsha,
            )
            if i > max_items:
    ...
    def test_git_file(self, rwrepo):
        ...
        real_path_abs = osp.abspath(
            join_path_native(rwrepo.working_tree_dir, ".real")
        )
