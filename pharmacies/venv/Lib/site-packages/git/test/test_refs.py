...
class TestRefs(TestBase):
    def test_from_path(self):
        ...
        # invalid path
        self.assertRaises(
            ValueError, TagReference, self.rorepo, "refs/invalid/tag"
        )
    ...
    def test_tag_base(self):
        ...
        for tag in self.rorepo.tags:
            ...
            if tag.tag is not None:
                ...
                # have no dict
                self.assertRaises(
                    AttributeError, setattr, tagobj, "someattr", 1
                )
                ...
                self.assertRaises(
                    AttributeError, setattr, tag, "object", tagobj
                )
    ...
    def test_heads(self, rwrepo):
        for head in rwrepo.heads:
            ...
            assert (
                git_tracking_branch.tracking_branch().name
                == special_name_remote_ref.name
            )
        # END for each head
    ...
    def test_head_reset(self, rw_repo):
        ...
        cur_head.reset(
            new_head_commit, index=True, working_tree=True
        )  # index + wt
        assert cur_head.reference.commit == new_head_commit
        ...
        for count, new_name in enumerate(("my_new_head", "feature/feature1")):
            ...
            self.assertRaises(
                ...
                rw_repo,
                new_name,
                new_head.commit.parents[0],
            )

            # force it
            new_head = Head.create(
                rw_repo, new_name, actual_commit, force=True
            )
            ...
            assert (
                new_head.rename(old_name).name == old_name
                and new_head.path == old_path
            )
        ...
        self.assertRaises(
            TypeError, RemoteReference.create, rw_repo, "some_name"
        )
        ...
        self.assertRaises(
            GitCommandError, TagReference.create, rw_repo, tag_name
        )
        light_tag = TagReference.create(
            rw_repo, tag_name, "HEAD~1", force=True
        )
        ...
        self.assertRaises(
            OSError, Reference.create, rw_repo, full_ref, "HEAD~20"
        )
        ...
        self.assertRaises(
            ValueError,
            getattr,
            Reference(rw_repo, "refs/doesntexist"),
            "commit",
        )
        ...
        symref = SymbolicReference.create(
            rw_repo, symref_path, cur_head.reference
        )
        ...
        SymbolicReference.create(
            rw_repo, symref.path, symref.reference
        ).path == symref.path  # @NoEffect
