...
class Tutorials(TestBase):
    ...
    def test_init_repo_object(self, rw_dir):
        ...
        assert (
            Repo.init(os.path.join(rw_dir, "path/for/new/repo")).__class__
            is Repo
        )
        # ![5-test_init_repo_object]
        ...
        self.assertEqual(
            repo.refs["0.3.5"], repo.tags["0.3.5"]
        )  # ... and tags
        # ![8-test_init_repo_object]

        # create a new head/branch
        # [9-test_init_repo_object]
        new_branch = cloned_repo.create_head(
            "feature"
        )  # create a new branch ...
        assert (
        ...
        self.assertEqual(
            len(bare_repo.remotes), 0
        )  # this one was just initialized
        origin = bare_repo.create_remote(
            "origin", url=cloned_repo.working_tree_dir
        )
        ...
        )  # Now the past is checked out

        new_file_path = os.path.join(
            cloned_repo.working_tree_dir, "my-new-file"
        )
        open(new_file_path, "wb").close()  # create new file in working tree
        cloned_repo.index.add([new_file_path])  # add it to the index
        # Commit the changes to deviate masters history
        cloned_repo.index.commit(
            "Added a new file in the past - for later merege"
        )
        ...
        master.commit = (
            new_branch.commit
        )  # let master point to most recent commit
    ...
    def test_references_and_objects(self, rw_dir):
        ...
        assert (
            hct.trees[0].path != ""
        )  # the first contained item has one though
        self.assertEqual(
            hct.mode, 0o40000
        )  # trees have the mode of a linux directory
        self.assertEqual(
    ...
    def test_submodules(self):
        ...
        self.assertEqual(
            len(sm.hexsha), 40
        )  # Its sha defines the commit to checkout
