...
class TestObjDBPerformance(TestBigRepoR):
    ...
    def test_large_data_streaming(self, rwrepo):
        ...
        for randomize in range(2):
            ...
            st = time()
            size, stream = make_memory_file(
                self.large_data_size_bytes, randomize
            )
            elapsed = time() - st
            ...
            # read chunks
            st = time()
            _hexsha, _typename, size, stream = rwrepo.git.stream_object_data(
                gitsha
            )
            while True:
                data = stream.read(cs)
            ...
            print(
                ...
                % (
                    100.0 - (elapsed_readchunks / gelapsed_readchunks) * 100,
                    desc,
                ),
                file=sys.stderr,
            )
