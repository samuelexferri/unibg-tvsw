...
class TestCommit(TestCommitSerialization):
    ...
    def test_stats(self):
        ...
        self.assertEqual(
            commit.author_tz_offset, 14400, commit.author_tz_offset
        )
        self.assertEqual(
            commit.committer_tz_offset, 14400, commit.committer_tz_offset
        )
    ...
    def test_traversal(self):
        ...
        # depth
        self.assertEqual(
            len(list(start.traverse(ignore_self=False, depth=0))), 1
        )

        # prune
        self.assertEqual(
            next(start.traverse(branch_first=1, prune=lambda i, d: i == p0)),
            p1,
        )
        ...
        self.assertEqual(
            next(
                start.traverse(branch_first=1, predicate=lambda i, d: i == p1)
            ),
            p1,
        )
    ...
    def test_iteration(self):
        ...
        less_ltd_commits = list(
            Commit.iter_items(
                self.rorepo, "master", paths=("CHANGES", "AUTHORS")
            )
        )
    ...
    def test_ambiguous_arg_iteration(self, rw_dir):
        ...
        list(
            rw_repo.iter_commits(rw_repo.head.ref)
        )  # should fail unless bug is fixed

    def test_count(self):
        self.assertEqual(
            self.rorepo.tag("refs/tags/0.1.5").commit.count(), 143
        )
    ...
    def test_repr(self):
        commit = Commit(self.rorepo, Commit.NULL_BIN_SHA)
        self.assertEqual(
            '<git.Commit "%s">' % Commit.NULL_HEX_SHA, repr(commit)
        )
    ...
    def test_serialization_unicode_support(self):
        ...
        assert isinstance(
            cmt.message, str
        )  # it automatically decodes it as such
    ...
    def test_invalid_commit(self):
        ...
        self.assertEqual(
            cmt.author.name, "E.Azer Ko�o�o�oculu", cmt.author.name
        )
        self.assertEqual(
            cmt.author.email, "azer@kodfabrik.com", cmt.author.email
        )
            ...
            def odb(self):
                class ODBMock:
                    ...
                    def stream(self, *args):
                        stream = Mock(
                            spec_set=["read"], return_value=self.bytestr
                        )
