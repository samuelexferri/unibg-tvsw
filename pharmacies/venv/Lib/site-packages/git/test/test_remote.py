    ...
    def update(self, op_code, cur_count, max_count=None, message=""):
        ...
        if op_code & self.WRITING > 0:
            if op_code & self.BEGIN > 0:
                assert (
                    not message
                ), "should not have message when remote begins writing"
...
class TestRemote(TestBase):
    ...
    def _do_test_push_result(self, results, remote):
        ...
        for info in results:
            ...
            if info.flags & info.ERROR:
                ...
                self.assertIn(
                    type(info.remote_ref), (TagReference, RemoteReference)
                )
            # END error checking
        # END for each info
    ...
    def _do_test_fetch_info(self, repo):
        self.assertRaises(
            ValueError, FetchInfo._from_line, repo, "nonsense", ""
        )
        self.assertRaises(
            ValueError,
    ...
    def _do_test_fetch(self, remote, rw_repo, remote_repo):
        ...
        self.assertEqual(
            other_branch_info.ref.commit, new_branch_info.ref.commit
        )
        ...
        res = fetch_and_test(
            remote, refspec="master:refs/remotes/%s/master" % remote
        )
    ...
    def _assert_push_and_pull(self, remote, rw_repo, remote_repo):
        ...
        new_tag = TagReference.create(
            rw_repo, to_be_updated
        )  # @UnusedVariable
        other_tag = TagReference.create(
            rw_repo, "my_obj_tag.2.1aRV", message="my message"
        )
        ...
        new_tag = TagReference.create(
            rw_repo, to_be_updated, ref="HEAD~1", force=True
        )
        ...
        force_with_lease = (
            "%s:0000000000000000000000000000000000000000" % new_head.path
        )
        res = remote.push(
            ":%s" % new_head.path, force_with_lease=force_with_lease
        )
        self.assertTrue(res[0].flags & PushInfo.ERROR)
        self.assertTrue(res[0].flags & PushInfo.REJECTED)
    ...
    def test_creation_and_removal(self, bare_rw_repo):
        ...
        self.assertRaises(
            GitCommandError, Remote.create, bare_rw_repo, *arg_list
        )
    ...
    def test_uncommon_branch_names(self):
        stderr_lines = (
            fixture("uncommon_branch_prefix_stderr")
            .decode("ascii")
            .splitlines()
        )
        fetch_lines = (
            fixture("uncommon_branch_prefix_FETCH_HEAD")
            .decode("ascii")
            .splitlines()
        )
    ...
    def test_multiple_urls(self, rw_repo):
        ...
        self.assertRaises(
            GitCommandError, remote.set_url, test2, add=True, delete=True
        )
