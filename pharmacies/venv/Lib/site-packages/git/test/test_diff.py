...
class TestDiff(TestBase):
    ...
    def test_diff_of_modified_files_not_added_to_the_index(self):
        output = StringProcessAdapter(
            fixture("diff_abbrev-40_full-index_M_raw_no-color")
        )
        diffs = Diff._index_from_raw_format(self.rorepo, output)

        self.assertEqual(len(diffs), 1, "one modification")
        self.assertEqual(
            len(list(diffs.iter_change_type("M"))), 1, "one modification"
        )
    ...
    def test_diff_initial_commit(self):
        ...
        self.assertEqual(
            diff_index[0].b_path, "CHANGES", repr(diff_index[0].b_path)
        )
        self.assertIsNotNone(diff_index[0].new_file)
    ...
    def test_diff_unsafe_paths(self):
        ...
        self.assertEqual(
            res[10].b_rawpath, b"path/\x80-invalid-unicode-path.txt"
        )
        ...
        self.assertEqual(
            res[11].a_path, "a/with spaces"
        )  # NOTE: path a/ here legit!
        self.assertEqual(
    ...
    def test_diff_submodule(self):
        ...
        repo.index.add(["test"])
        Submodule.add(
            repo, "subtest", "sub", url="file://" + self.submodule_dir
        )
    ...
    def test_diff_interface(self):
        ...
        for i, commit in enumerate(
            self.rorepo.iter_commits("0.1.6", max_count=2)
        ):
            diff_item = commit
            if i % 2 == 0:
