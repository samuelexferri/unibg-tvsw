...
class TestSubmodule(TestBase):
    ...
    def _do_base_tests(self, rwrepo):
        ...
        self.assertRaises(
            ValueError, smold.set_parent_commit, self.k_subm_current
        )
        ...
        self.assertRaises(
            ValueError, smold.set_parent_commit, self.k_no_subm_tag
        )
        ...
        if rwrepo.bare:
            ...
            for repo in smods:
                assert (
                    repo.head.commit != repo.head.ref.tracking_branch().commit
                )
            ...
            self.assertRaises(
                ValueError, csm.remove, module=False, configuration=False
            )
            ...
            self.assertRaises(
                InvalidGitRepositoryError, sm.remove, dry_run=True
            )
            ...
            # rename a module
            nmp = (
                join_path_native("new", "module", "dir") + "/"
            )  # new module path
            pmp = nsm.path
    ...
    @skipIf(
        HIDE_WINDOWS_KNOWN_ERRORS and sys.version_info[:2] == (3, 5),
        """
    ...
    def test_root_module(self, rwrepo):
        ...
        )  # would not change head, and thus doens't fail
        ...
        for branch in ("some_virtual_branch", cur_branch.name):
            with nsm.config_writer() as writer:
                writer.set_value(
                    Submodule.k_head_option, git.Head.to_full_path(branch)
                )
            csmbranchchange = rwrepo.index.commit(
                "changed branch to %s" % branch
            )
        ...
        assert (
            nsm.exists() and nsm.module_exists() and len(nsm.children()) >= 1
        )
        ...
        rm.update(
            recursive=True, progress=prog, dry_run=True
    ...
    def test_git_submodules_and_add_sm_with_new_commit(self, rwdir):
        ...
        sm2 = parent.create_submodule(
            "nongit-file-submodule", clone.working_tree_dir
        )
    ...
    def test_remove_norefs(self, rwdir):
        ...
        sm = parent.create_submodule(
            sm_name, sm_name, url=self._small_repo_url()
        )
        assert sm.exists()
    ...
    def test_branch_renames(self, rw_dir):
        ...
        self.assertRaises(
            RepositoryDirtyError,
            parent_repo.submodule_update,
            to_latest_revision=True,
        )
        parent_repo.submodule_update(to_latest_revision=True, force_reset=True)
        assert (
            sm_mod.commit() == sm_pfb.commit
        ), "Now head should have been reset"
    ...
    def test_to_relative_path_with_super_at_root_drive(self):
        ...
        msg = (
            '_to_relative_path should be "submodule_path" but was "%s"'
            % relative_path
        )
