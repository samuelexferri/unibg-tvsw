...
from git import Blob, Tree, Commit, TagObject
from git.compat import is_win
from git.objects.util import get_object_type_by_name
from git.test.lib import TestBase, with_rw_repo, with_rw_and_rw_remote_repo
...
class TestBase(TestBase):
    ...
    def test_with_bare_rw_repo(self, bare_rw_repo):
        assert bare_rw_repo.config_reader("repository").getboolean(
            "core", "bare"
        )
    ...
    def test_with_rw_repo(self, rw_repo):
        assert not rw_repo.config_reader("repository").getboolean(
            "core", "bare"
        )
    ...
    def test_with_rw_remote_and_rw_repo(self, rw_repo, rw_remote_repo):
        ...
        assert rw_remote_repo.config_reader("repository").getboolean(
            "core", "bare"
        )
        assert osp.isdir(osp.join(rw_repo.working_tree_dir, "lib"))

    @skipIf(
        sys.version_info < (3,) and is_win,
