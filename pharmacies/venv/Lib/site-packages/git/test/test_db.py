...
class TestDB(TestBase):
    def test_base(self):
        gdb = GitCmdObjectDB(
            osp.join(self.rorepo.git_dir, "objects"), self.rorepo.git
        )
        ...
        assert (
            bin_to_hex(gdb.partial_to_complete_sha_hex(hexsha[:20])) == hexsha
        )

        # fails with BadObject
        for invalid_rev in ("0000", "bad/ref", "super bad"):
            self.assertRaises(
                BadObject, gdb.partial_to_complete_sha_hex, invalid_rev
            )
