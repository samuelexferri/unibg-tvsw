...
from git.test.lib import TestBase, fixture_path, fixture, with_rw_repo
...
@skipIf(
    HIDE_WINDOWS_KNOWN_ERRORS, "TODO: fix hooks execution on Windows: #703"
)
...
class TestIndex(TestBase):
    ...
    def test_index_file_from_tree(self, rw_repo):
        ...
        two_way_index = IndexFile.from_tree(
            rw_repo, common_ancestor_sha, cur_sha
        )
        ...
        assert len(
            [e for e in three_way_index.entries.values() if e.stage != 0]
        )
        ...
        three_way_index.resolve_blobs(
            l[0][1] for l in unmerged_blob_map.values()
        )
    ...
    def test_index_merge_tree(self, rw_repo):
        ...
        assert (
            manifest_entry.binsha != rw_repo.index.entries[manifest_key].binsha
        )
        ...
        assert (
            rw_repo.index.entries[manifest_key].binsha != Object.NULL_BIN_SHA
        )
        ...
        index.write()
        self.assertEqual(
            rw_repo.index.entries[manifest_key].hexsha, Diff.NULL_HEX_SHA
        )
        ...
        unmerged_tree = IndexFile.from_tree(
            rw_repo, parent_commit, tree, next_commit
        )
    ...
    def test_index_file_diffing(self, rw_repo):
        ...
        odiff = index.diff(
            cur_head_commit, R=False
        )  # now its not reversed anymore
        assert adiff != odiff
        ...
        self.assertRaises(
            CheckoutError, index.checkout, "doesnt_exist_ever.txt.that"
        )
        self.assertRaises(
            CheckoutError, index.checkout, paths=["doesnt/exist"]
        )
    ...
    def test_index_mutation(self, rw_repo):
        ...
        my_committer = Actor(
            "Committing Frèderic Çaufl€", "committer@example.com"
        )
        ...
        self.assertEqual(
            commit_actor.committer.name, "Committing Frèderic Çaufl€"
        )
        ...
        commit_no_parents = index.commit(
            commit_message, parent_commits=[], head=True
        )
        ...
        entries = index.reset(new_commit).add(
            [old_blob], fprogress=self._fprogress_add
        )
        self._assert_entries(entries)
        self._assert_fprogress(entries)
        self.assertEqual(
            index.entries[(old_blob.path, 0)].hexsha, old_blob.hexsha
        )
        ...
        if not is_win:
            for target in ("/etc/nonexisting", "/etc/passwd", "/etc"):
                ...
                self.assertTrue(
                    S_ISLNK(
                        index.entries[
                            index.entry_key("my_real_symlink", 0)
                        ].mode
                    )
                )
                ...
                self.assertEqual(
                    index.repo.odb.stream(entries[0].binsha)
                    .read()
                    .decode("ascii"),
                    target,
                )
        ...
        fake_symlink_path = self._make_file(
            fake_symlink_relapath, link_target, rw_repo
        )
    ...
    def test_commit_msg_hook_success(self, rw_repo):
        ...
        self.assertEqual(
            new_commit.message,
            "{} {}".format(commit_message, from_hook_message),
        )
