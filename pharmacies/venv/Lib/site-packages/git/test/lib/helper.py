...
GIT_REPO = os.environ.get(
    "GIT_PYTHON_TEST_GIT_REPO_BASE", ospd(ospd(ospd(ospd(__file__))))
)
    ...
    def argument_passer(func):
        ...
        def repo_creator(self):
            ...
            repo_dir = tempfile.mktemp(
                prefix="%sbare_%s" % (prefix, func.__name__)
            )
            rw_repo = self.rorepo.clone(
                repo_dir, shared=True, bare=bare, n=True
            )
    ...
    try:
        ...
        if gd:
            try:
                ...
                log.debug(
                    "Hidden error while Killing git-daemon: %s", ex, exc_info=1
                )
...
def with_rw_and_rw_remote_repo(working_tree_ref):
    ...
    def argument_passer(func):
        ...
        def remote_repo_creator(self):
            ...
            rw_repo = rw_daemon_repo.clone(
                rw_repo_dir, shared=True, bare=False, n=True
            )
            try:
                ...
                d_remote = Remote.create(
                    rw_repo, "daemon_origin", rw_daemon_repo_dir
                )
                d_remote.fetch()
...
class TestBase(TestCase):
    ...
    def _small_repo_url(self):
        ...
        return Git.polish_url(
            osp.join(
                self.rorepo.working_tree_dir, "git/ext/gitdb/gitdb/ext/smmap"
            )
        )
