...
class SymbolicReference(object):
                ...
                for line in fp:
                    ...
                    if line.startswith("#"):
                        ...
                        if (
                            ...
                            raise TypeError(
                                "PackingType of packed-Refs not understood: %r"
                                % line
                            )
    ...
    def _get_ref_info_helper(cls, repo, ref_path):
        ...
        try:
            with open(
                osp.join(repodir, ref_path), "rt", encoding="UTF-8"
            ) as fp:
        ...
        raise ValueError(
            "Failed to parse reference information from %r" % ref_path
        )
    ...
    def _get_commit(self):
        ...
        if obj.type != Commit.type:
            raise TypeError(
                "Symbolic Reference pointed to object %r, commit was required"
                % obj
            )
        # END handle type
        return obj
    ...
    commit = property(
        _get_commit, set_commit, doc="Query or set commits directly"
    )
    object = property(
        _get_object,
        set_object,
    ...
    def set_reference(self, ref, logmsg=None):
        ...
        if (
            obj is not None
            and self._points_to_commits_only
            and obj.type != Commit.type
        ):
            ...
            try:
                with open(pack_file_path, "rb") as reader:
                    ...
                    for line in reader:
                        ...
                        if (
                            line.startswith("#") or full_ref_path not in line
                        ) and (
    ...
    def rename(self, new_path, force=False):
        ...
        if osp.isfile(new_abs_path):
            if not force:
                ...
                if f1 != f2:
                    raise OSError(
                        "File at path %r already exists" % new_abs_path
                    )
    ...
    def _iter_items(cls, repo, common_path=None):
        ...
        for root, dirs, files in os.walk(
            ...
            for f in files:
                ...
                rela_paths.add(
                    abs_path.replace(
                        to_native_path_linux(repo.common_dir) + "/", ""
                    )
                )
    ...
    def from_path(cls, repo, path):
        ...
        for ref_type in (
            ...
            try:
                ...
                if (
                    instance.__class__ == SymbolicReference
                    and instance.is_detached
                ):
