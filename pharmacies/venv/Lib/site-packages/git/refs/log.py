...
class RefLogEntry(tuple):
    ...
    @classmethod
    def new(
        cls, oldhexsha, newhexsha, actor, time, tz_offset, message
    ):  # skipcq: PYL-W0621
        ...
        # END check types
        return RefLogEntry(
            (oldhexsha, newhexsha, actor, (time, tz_offset), message)
        )
...
class RefLog(list, Serializable):
    ...
    @classmethod
    def append_entry(
        cls, config_reader, filepath, oldbinsha, newbinsha, message
    ):
        ...
        committer = (
            isinstance(config_reader, Actor)
            and config_reader
            or Actor.committer(config_reader)
        )
        entry = RefLogEntry(
            (
