...
BlameEntry = namedtuple(
    "BlameEntry", ["commit", "linenos", "orig_path", "orig_linenos"]
)
...
class Repo(object):
    ...
    def __init__(
            ...
            if is_git_dir(curpath):
                ...
                if os.environ.get("GIT_COMMON_DIR") is None:
                    ...
                    if gitconf.has_option("core", "worktree"):
                        self._working_tree_dir = gitconf.get(
                            "core", "worktree"
                        )
        ...
        try:
            common_dir = (
                open(osp.join(self.git_dir, "commondir"), "rt")
                .readlines()[0]
                .strip()
            )
    ...
    def create_tag(
        self, path, ref="HEAD", message=None, force=False, **kwargs
    ):
    ...
    def _get_config_path(self, config_level):
        ...
        if config_level == "system":
            ...
            return osp.normpath(
                osp.join(self._common_dir or self.git_dir, "config")
            )
    ...
    def config_writer(self, config_level="repository"):
        ...
        return GitConfigParser(
            self._get_config_path(config_level), read_only=False
        )
    ...
    def merge_base(self, *rev, **kwargs):
        ...
        if len(rev) < 2:
            raise ValueError(
                "Please specify at least two revs, got only %i" % len(rev)
            )
        # end handle input
        ...
        if untracked_files:
            if len(
                self._get_untracked_files(
                    path, ignore_submodules=not submodules
                )
            ):
                return True
        # END untracked files
            ...
            if hexsha not in commits:
                ...
                c = Commit(
                    ...
                    committer=Actor(
                        ...
                        safe_decode(
                            props[b"committer-mail"].lstrip(b"<").rstrip(b">")
                        ),
                    ),
