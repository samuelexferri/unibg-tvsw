...
class PushInfo(object):
    ...
    def remote_ref(self):
        ...
        if self.remote_ref_string.startswith("refs/tags"):
            ...
            raise ValueError(
                "Could not handle remote ref: %r" % self.remote_ref_string
            )
        # END
    ...
    def _from_line(cls, remote, line):
        ...
        return PushInfo(
            flags, from_ref, to_ref_string, remote, old_commit, summary
        )
...
class FetchInfo(object):
    ...
    def __init__(
        self, ref, flags, note="", old_commit=None, remote_ref_path=None
    ):
        """
        Initialize a new instance
        """
    ...
    def _from_line(cls, repo, line, fetch_line):
        ...
        * means birth of new branch or tag
        ...
        try:
            ...
            raise ValueError(
                "Failed to parse FETCH_HEAD line: %r" % fetch_line
            )
        ...
        if ref_type is SymbolicReference:
            ...
            if remote_local_ref.startswith(
                Reference._common_path_default + "/"
            ):
                ...
                if (
                    ...
                    and not remote_local_ref.startswith(
                ...
                ref_path = join_path(
                    RemoteReference._common_path_default, remote_local_ref
                )
            else:
                ref_path = join_path(
                    ref_type._common_path_default, remote_local_ref
                )
            # END obtain refpath
...
class Remote(LazyMixin, Iterable):
    ...
    def iter_items(cls, repo):
        ...
        for section in repo.config_reader("repository").sections():
            ...
            if lbound == -1 or rbound == -1:
                raise ValueError(
                    "Remote-Section has invalid format: %r" % section
                )
    ...
    def urls(self):
        ...
        try:
            remote_details = self.repo.git.remote(
                "get-url", "--all", self.name
            )
    ...
    def refs(self):
        ...
        out_refs.extend(
            RemoteReference.list_items(self.repo, remote=self.name)
        )
        return out_refs
    ...
    def stale_refs(self):
        ...
        for line in self.repo.git.remote(
            "prune", "--dry-run", self
        ).splitlines()[2:]:
            # expecting
            ...
            if ref_name.startswith(Reference._common_path_default + "/"):
                out_refs.append(
                    SymbolicReference.from_path(self.repo, ref_name)
                )
            else:
                fqhn = "%s/%s" % (
                    RemoteReference._common_path_default,
    ...
    def _get_fetch_info_from_stderr(self, proc, progress):
        ...
        handle_process_output(
            proc, None, progress_handler, finalizer=None, decode_streams=False
        )

        stderr_text = (
            progress.error_lines and "\n".join(progress.error_lines) or ""
        )
    ...
    def _get_push_info(self, proc, progress):
        ...
        try:
            ...
            if not output:
                ...
                log.warning(
                    "Error lines received while fetching: %s", stderr_text
                )
