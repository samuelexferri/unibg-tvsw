...
from git.util import hex_to_bin, Actor, Iterable, Stats, finalize_process
...
class Commit(base.Object, Iterable, Diffable, Traversable, Serializable):
    ...
    def _set_cache_(self, attr):
        if attr in Commit.__slots__:
            ...
            _binsha, _typename, self.size, stream = self.repo.odb.stream(
                self.binsha
            )
    ...
    def stats(self):
        ...
        if not self.parents:
            text = self.repo.git.diff_tree(
                self.hexsha, "--", numstat=True, root=True
            )
            text2 = ""
    ...
    def _deserialize(self, stream):
        ...
        while True:
            ...
            self.parents.append(
                type(self)(
                    self.repo,
                    hex_to_bin(parent_line.split()[-1].decode("ascii")),
                )
            )
