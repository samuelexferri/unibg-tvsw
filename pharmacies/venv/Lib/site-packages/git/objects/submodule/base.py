...
class UpdateProgress(RemoteProgress):
    ...
    CLONE, FETCH, UPDWKTREE = [
        1 << x
        for x in range(
            RemoteProgress._num_op_codes, RemoteProgress._num_op_codes + 3
        )
    ]
...
class Submodule(IndexObject, Iterable, Traversable):
    ...
    def _sio_modules(cls, parent_commit):
        ...
        sio = BytesIO(
            parent_commit.tree[cls.k_modules_file].data_stream.read()
        )
    ...
    def _clone_repo(cls, repo, url, path, name, **kwargs):
        ...
        if cls._need_gitfile_submodules(repo.git):
            cls._write_git_file_and_module_config(
                module_checkout_path, module_abspath
            )
        # end
        return clone
    ...
    def _to_relative_path(cls, parent_repo, path):
        ...
        if osp.isabs(path):
            working_tree_linux = to_native_path_linux(
    ...
    def _write_git_file_and_module_config(
        cls, working_tree_dir, module_abspath
    ):
        ...
        with GitConfigParser(
            osp.join(module_abspath, "config"),
            read_only=False,
    ...
    def add(
        ...
        br = git.Head(
            repo, git.Head.to_full_path(str(branch) or cls.k_head_default)
        )
        ...
        try:
            ...
            try:
                ...
                for i, remote in enumerate(rmts):
                    ...
                    progress.update(
                        ...
                        len_rmts,
                        prefix
                        + "Fetching remote %s of submodule %r"
                        % (remote, self.name),
                    )
                    # ===============================
                    if not dry_run:
                ...
                if not dry_run:
                    mrepo = self._clone_repo(
                        self.repo, self.url, self.path, self.name, n=True
                    )
                # END handle dry-run
                progress.update(
                    END | CLONE,
                ...
                if not dry_run:
                    ...
                    try:
                        ...
                        local_branch.set_object(
                            Object(mrepo, self.NULL_BIN_SHA)
                        )
                        ...
                        log.warning(
                            "Failed to checkout tracking branch %s",
                            self.branch_path,
                        )
                    ...
                    with self.repo.config_writer() as writer:
                        writer.set_value(
                            sm_section(self.name), "url", self.url
                        )
                # END handle dry_run
            ...
            if mrepo is not None and to_latest_revision:
                ...
                if not is_detached:
                    ...
                    log.error(
                        "%s there was no local tracking branch", msg_base
                    )
            ...
            if mrepo is not None and mrepo.head.commit.binsha != binsha:
                ...
                if mrepo.head.commit.binsha != self.NULL_BIN_SHA:
                    ...
                    if (
                        len(base_commit) == 0
                        or base_commit[0].hexsha == hexsha
                    ):
                        if force:
                ...
                progress.update(
                    ...
                    1,
                    prefix
                    + "Done updating working tree for submodule %r"
                    % self.name,
                )
    ...
    def move(self, module_path, configuration=True, module=True):
        ...
        module_checkout_abspath = join_path_native(
            self.repo.working_tree_dir, module_checkout_path
        )
        if osp.isfile(module_checkout_abspath):
            raise ValueError(
                "Cannot move repository onto a file: %s"
        ...
        if module and osp.exists(cur_path):
            ...
            if osp.isfile(osp.join(module_checkout_abspath, ".git")):
                module_abspath = self._module_abspath(
                    self.repo, self.path, self.name
                )
    ...
    @unbare_repo
    def remove(
        self, module=True, force=False, configuration=True, dry_run=False
    ):
                ...
                for remote in mod.remotes:
                    ...
                    for rref in rrefs:
                        num_branches_with_new_commits += (
                            len(mod.git.cherry(rref)) != 0
                        )
                    # END for each remote ref
                    # not a single remote branch contained all our commits
                    if len(rrefs) and num_branches_with_new_commits == len(
                        rrefs
                    ):
                        raise InvalidGitRepositoryError(
    ...
    def set_parent_commit(self, commit, check=True):
        ...
        if check:
            parser = self._config_parser(
                self.repo, self._parent_commit, read_only=True
            )
    ...
    def rename(self, new_name):
        ...
        if mod.has_separate_working_tree():
            ...
            if destination_module_abspath.startswith(mod.git_dir):
                tmp_dir = self._module_abspath(
                    self.repo, self.path, str(uuid.uuid4())
                )
    ...
    def module(self):
        ...
        try:
            ...
            raise InvalidGitRepositoryError(
                "Repository at %r was not yet checked out"
                % module_checkout_abspath
            )
        # END handle exceptions
