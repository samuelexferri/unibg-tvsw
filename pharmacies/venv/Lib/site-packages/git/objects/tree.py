...
class Tree(IndexObject, diff.Diffable, util.Traversable, util.Serializable):
    ...
    def _get_intermediate_items(cls, index_object):
        if index_object.type == "tree":
            return tuple(
                index_object._iter_convert_to_object(index_object._cache)
            )
        return ()
    ...
    def _iter_convert_to_object(self, iterable):
        ...
        for binsha, mode, name in iterable:
            ...
            try:
                yield self._map_id_to_type[mode >> 12](
                    self.repo, binsha, mode, path
                )
            except KeyError:
                raise TypeError(
                    "Unknown mode %o found in tree data for path '%s'"
                    % (mode, path)
                )
        # END for each item
