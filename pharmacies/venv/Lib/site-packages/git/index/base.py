...
from git.exc import GitCommandError, CheckoutError, InvalidGitRepositoryError
...
class IndexFile(LazyMixin, diff.Diffable, Serializable):
    ...
    def new(cls, repo, *tree_sha):
        ...
        :return:
            ...
            to its 'write' method."""
        base_entries = aggressive_tree_merge(
            repo.odb, [to_bin_sha(str(t)) for t in tree_sha]
        )
    ...
    def _write_path_to_stdin(
        self, proc, filepath, item, fmakeexc, fprogress, read_from_stdout=True
    ):
    ...
    def resolve_blobs(self, iter_blobs):
        ...
        for blob in iter_blobs:
            ...
            if stage_null_key in self.entries:
                raise ValueError(
                    "Path %r already exists at stage 0" % blob.path
                )
    ...
    def write_tree(self):
        ...
        binsha, tree_items = write_tree_from_cache(
            entries, mdb, slice(0, len(entries))
        )
            ...
            for path in paths:
                if osp.isabs(path):
                    ...
                    gitrelative_path = path[
                        len(self.repo.working_tree_dir) + 1 :
                    ]
                else:
                    gitrelative_path = path
                    abspath = osp.join(
                        self.repo.working_tree_dir, gitrelative_path
                    )
    ...
    def add(
        ...
        if paths:
            entries_added.extend(
                self._entries_for_paths(
                    paths, path_rewriter, fprogress, entries
                )
            )
            ...
            if null_entries_indices:
                ...
                def handle_null_entries(self):
                    for ei in null_entries_indices:
                        ...
                        new_entry = self._store_path(
                            null_entry.path, fprogress
                        )
    ...
    def commit(
        ...
        if not skip_hooks:
            ...
            run_commit_hook(
                "commit-msg", self, self._commit_editmsg_filepath()
            )
    ...
    def _write_commit_editmsg(self, message):
        with open(
            self._commit_editmsg_filepath(), "wb"
        ) as commit_editmsg_file:
    ...
    def _read_commit_editmsg(self):
        with open(
            self._commit_editmsg_filepath(), "rb"
        ) as commit_editmsg_file:
    ...
    @default_index
    def checkout(
        self, paths=None, force=False, fprogress=lambda *args: None, **kwargs
    ):
        ...
        def handle_stderr(proc, iter_checked_out_files):
            ...
            for line in stderr.splitlines():
                if not line.startswith(
                    "git checkout-index: "
                ) and not line.startswith("git-checkout-index: "):
    ...
    def diff(
        ...
        **kwargs
        ...
        if other is not None:
            raise ValueError(
                "other must be None, Diffable.Index, a Tree or Commit, was %r"
                % other
            )

        # diff against working copy - can be handled by superclass natively
        return super(IndexFile, self).diff(
            other, paths, create_patch, **kwargs
        )
