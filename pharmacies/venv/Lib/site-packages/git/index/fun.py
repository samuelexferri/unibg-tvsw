...
def run_commit_hook(name, index, *args):
    ...
    try:
        ...
        handle_process_output(
            cmd, stdout.append, stderr.append, finalize_process
        )
        stdout = "".join(stdout)
        stderr = "".join(stderr)
        if cmd.returncode != 0:
...
def stat_mode_to_index_mode(mode):
    ...
    if S_ISDIR(mode) or S_IFMT(mode) == S_IFGITLINK:  # submodules
        return S_IFGITLINK
    return (
        S_IFREG | 0o644 | (mode & 0o111)
    )  # blobs with or without executable bit


def write_cache(
    entries, stream, extension_data=None, ShaStreamCls=IndexFileSHA1Writer
):
    ...
    for entry in entries:
        ...
        assert plen == len(path), (
            "Path %s too long to fit into index" % entry[3]
        )
        flags = plen | (
            entry[2] & CE_NAMEMASK_INV
