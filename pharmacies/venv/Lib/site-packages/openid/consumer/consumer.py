...
class GenericConsumer(object):
    ...
    def _verifyDiscoveryResultsOpenID1(self, resp_msg, endpoint):
        claimed_id = resp_msg.getArg(
            BARE_NS, self.openid1_return_to_identifier_name
        )
    ...
    def _discoverAndVerify(self, claimed_id, to_match_endpoints):
        ...
        return self._verifyDiscoveredServices(
            claimed_id, services, to_match_endpoints
        )

    def _verifyDiscoveredServices(
        self, claimed_id, services, to_match_endpoints
    ):
        ...
        for endpoint in services:
            ...
            logging.error(
                "Discovery verification failure for %s" % (claimed_id,)
            )
    ...
    def _extractSupportedAssociationType(
        self, server_error, endpoint, assoc_type
    ):
    ...
    def _getOpenID1SessionType(self, assoc_response):
        ...
        if session_type == "no-encryption":
            logging.warning(
                'OpenID server sent "no-encryption"' "for OpenID 1.X"
            )
...
class AuthRequest(object):
    ...
    def formMarkup(
        self, realm, return_to=None, immediate=False, form_tag_attrs=None
    ):
