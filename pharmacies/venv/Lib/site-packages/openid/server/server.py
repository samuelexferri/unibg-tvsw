...
class DiffieHellmanSHA1ServerSession(object):
    ...
    def answer(self, secret):
        mac_key = self.dh.xorSecret(
            self.consumer_pubkey, secret, self.hash_func
        )
        return {
...
class AssociateRequest(OpenIDRequest):
    ...
    def fromMessage(klass, message, op_endpoint=UNUSED):
        ...
        try:
            ...
            raise ProtocolError(
                message, "Unknown session type %r" % (session_type,)
            )
    ...
    def answer(self, assoc):
        ...
        response.fields.updateArgs(
            OPENID_NS, self.session.answer(assoc.secret)
        )

        if not (
            self.session.session_type == "no-encryption"
            and self.message.isOpenID1()
        ):
...
class CheckIDRequest(OpenIDRequest):
    ...
    def fromMessage(klass, message, op_endpoint):
        ...
        self.trust_root = (
            message.getArg(OPENID_NS, trust_root_param) or self.return_to
        )
...
class Signatory(object):
    ...
    def verify(self, assoc_handle, message):
        ...
        try:
            ...
            logging.exception(
                "Error in verifying %s with %s: %s" % (message, assoc, ex)
            )
            return False
        return valid
    ...
    def encode(self, response):
        ...
        if (not isinstance(response, Exception)) and response.needsSigning():
            if not self.signatory:
                raise ValueError(
                    "Must have a store to sign this request: %s" % (response,),
                    response,
                )
