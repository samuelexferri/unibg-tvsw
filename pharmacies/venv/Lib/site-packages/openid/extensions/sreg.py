...
def checkFieldName(field_name):
    ...
    if field_name not in data_fields:
        raise ValueError(
            "%r is not a defined simple registration field" % (field_name,)
        )
...
def supportsSReg(endpoint):
    ...
    @rtype: bool
    """
    return endpoint.usesExtension(ns_uri_1_1) or endpoint.usesExtension(
        ns_uri_1_0
    )
...
class SRegRequest(Extension):
    ...
    def __init__(
        self, required=None, optional=None, policy_url=None, sreg_ns_uri=ns_uri
    ):
