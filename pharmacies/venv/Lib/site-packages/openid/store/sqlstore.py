...
class SQLStore(OpenIDStore):
    ...
    def __init__(self, conn, associations_table=None, nonces_table=None):
        ...
        if not (
            hasattr(self.exceptions, "IntegrityError")
            and hasattr(self.exceptions, "OperationalError")
        ):
            raise RuntimeError(
                "Error using database connection module "
...
class PostgreSQLStore(SQLStore):
    ...
    def db_set_assoc(
        self, server_url, handle, secret, issued, lifetime, assoc_type
    ):
        """
        ...
        if len(rows):
            ...
            return self.db_update_assoc(
                secret, issued, lifetime, assoc_type, server_url, handle
            )
        else:
            # Insert a new record because this association wasn't
            # found.
            return self.db_new_assoc(
