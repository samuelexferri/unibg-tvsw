...
def validate_options(options):
    ...
    if kwcase not in [None, "upper", "lower", "capitalize"]:
        raise SQLParseError(
            "Invalid value for keyword_case: " "{0!r}".format(kwcase)
        )

    idcase = options.get("identifier_case")
    if idcase not in [None, "upper", "lower", "capitalize"]:
        raise SQLParseError(
            "Invalid value for identifier_case: " "{0!r}".format(idcase)
        )
    ...
    if strip_comments not in [True, False]:
        raise SQLParseError(
            "Invalid value for strip_comments: " "{0!r}".format(strip_comments)
        )
    ...
    if strip_ws not in [True, False]:
        raise SQLParseError(
            "Invalid value for strip_whitespace: " "{0!r}".format(strip_ws)
        )
    ...
    if indent_columns not in [True, False]:
        raise SQLParseError(
            "Invalid value for indent_columns: " "{0!r}".format(indent_columns)
        )
    elif indent_columns:
    ...
    if reindent not in [True, False]:
        raise SQLParseError(
            "Invalid value for reindent: " "{0!r}".format(reindent)
        )
    elif reindent:
    ...
    if reindent_aligned not in [True, False]:
        raise SQLParseError(
            "Invalid value for reindent_aligned: " "{0!r}".format(reindent)
        )
    ...
    if indent_tabs not in [True, False]:
        raise SQLParseError(
            "Invalid value for indent_tabs: " "{0!r}".format(indent_tabs)
        )
    elif indent_tabs:
        options["indent_char"] = "\t"
    else:
