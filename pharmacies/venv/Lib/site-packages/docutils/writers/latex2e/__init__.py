...
class Writer(writers.Writer):
    ...
    settings_spec = (
        ...
        (
            ...
            (
                ...
                {
                    "action": "store_false",
                    "dest": "section_prefix_for_enumerators",
                },
            ),
            (
    ...
    def translate(self):
        ...
        try:
            ...
            template_file = open(
                os.path.join(
                    self.default_template_path, self.document.settings.template
                ),
                "rb",
            )
...
class Babel(object):
    ...
    language_codes = dict(
        [(k.lower(), v) for (k, v) in language_codes.items()]
    )
...
class Table(object):
    ...
    def set_table_style(self, table_style, classes):
        ...
        self.colwidths_auto = (
            "colwidths-auto" in classes
            and "colwidths-given" not in table_style
        ) or (
            "colwidths-auto" in table_style
    ...
    def get_colspecs(self, node):
        ...
        for node in self._col_specs:
            ...
            latex_table_spec += "%sp{%.3f\\DUtablewidth}" % (
                bar,
                colwidth + 0.005,
            )
        return latex_table_spec + bar
        ...
        try:
            mc_width = sum(
                [
                    width
                    for width in (
                        [self._col_width[start + co] for co in range(len_)]
                    )
                ]
            )
    ...
    def depart_thead(self):
        ...
        if self._latex_type == "longtable":
            if 1 == self._translator.thead_depth():
                ...
                a.append("\\endhead\n")
                a.append(
                    r"\multicolumn{%d}{c}" % len(self._col_specs)
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    def __init__(self, document, babel_class=Babel):
        ...
        self.section_enumerator_separator = settings.section_enumerator_separator.replace(
            "_", r"\_"
        )
        # literal blocks:
        ...
        if settings.literal_block_env:
            (
                ...
                self.literal_block_options,
                none,
            ) = re.split(r"(\w+)(.*)", settings.literal_block_env)
        ...
        self.d_class = DocumentClass(
            settings.documentclass, settings.use_part_section
        )
        ...
        if settings.sectnum_xform:  # section numbering by Docutils
            ...
            if (
                hasattr(settings, "sectnum_start")
    ...
    def stylesheet_call(self, path):
        ...
        if self.settings.embed_stylesheet:
            ...
            try:
                content = io.FileInput(
                    source_path=path, encoding="utf-8"
                ).read()
                self.settings.record_dependencies.add(path)
            except IOError as err:
                msg = u"Cannot embed stylesheet '%s':\n  %s." % (
                    path,
                    SafeString(err.strerror),
                )
                self.document.reporter.error(msg)
                return "% " + msg.replace("\n", "\n% ")
            if is_package:
                content = "\n".join(
                    [r"\makeatletter", content, r"\makeatother"]
                )
    ...
    def visit_admonition(self, node):
        ...
        node["classes"] = [
            cls for cls in node["classes"] if cls != "admonition"
        ]
    ...
    def depart_document(self, node):
        ...
        # * language setup
        if self.babel.otherlanguages or self.babel.language not in (
            "",
            "english",
        ):
    ...
    def visit_enumerated_list(self, node):
            ...
            if (
                ...
                prefix = (
                    ".".join(
                        [
                            str(n)
                            for n in self._section_number[: self.section_level]
                        ]
                    )
        ...
        if enumeration_level <= 4:
            ...
            if (prefix, enumtype, suffix) != labels[enumeration_level - 1]:
                self.out.append(
    ...
    def visit_footnote_reference(self, node):
        ...
        if format == "brackets":
            ...
            self.fallbacks["footnotes"] = PreambleCmds.footnotes
            self.out.append(
                r"\DUfootnotemark{%s}{%s}{" % (node["ids"][0], href)
            )
            self.context.append("}")
    ...
    def to_latex_length(self, length_str, pxunit=None):
        ...
        if pxunit is not None:
            sys.stderr.write(
                "deprecation warning: LaTeXTranslator.to_latex_length()"
    ...
    def visit_literal_block(self, node):
        ...
        _use_env = _plaintext and not isinstance(
            node.parent, (nodes.footnote, nodes.admonition)
        )
        ...
        if (
            literal_env
            and _use_env
            and (not _autowidth_table or _use_listings)
        ):
            try:
    ...
    def visit_math(self, node, math_env="$"):
        ...
        if math_env == "$":
            ...
            wrapper = u"\n".join(
                ["%%", r"\begin{%s}" % math_env, "%s", r"\end{%s}" % math_env]
            )
    ...
    def visit_paragraph(self, node):
        ...
        if index == 0 and isinstance(
            node.parent, (nodes.list_item, nodes.description, nodes.compound)
        ):
            pass
        elif (
            index > 0
            ...
            if index == 1:  # second paragraph
                self.warn(
                    "LaTeX merges paragraphs in tables "
    ...
    def visit_reference(self, node):
        ...
        if self._reference_label:
            self.out.append(
                "\\%s{%s}}" % (self._reference_label, href.replace("#", ""))
            )
            raise nodes.SkipNode
    ...
    def visit_subtitle(self, node):
        ...
        # section subtitle: "starred" (no number, not in ToC)
            ...
            self.out.append(
                r"\%s*{" % self.d_class.section(self.section_level + 1)
            )
        else:
    ...
    def visit_table(self, node):
        ...
        if (
            ...
            and self.d_class.section(self.section_level).find("paragraph")
            != -1
        ):
            self.out.append("\\leavevmode")
        self.active_table.open()
        self.active_table.set_table_style(
            self.settings.table_style, node["classes"]
        )
        if "align" in node:
    ...
    def visit_title(self, node):
        ...
        if node.parent.tagname == "document":
            ...
            self.context.append("")
            self.pdfinfo.append(
                "  pdftitle={%s}," % self.encode(node.astext())
            )
        ...
        # Section title
            ...
            if "system-messages" in node.parent["classes"]:
                ...
                self.out.append(
                    r"\%s[%s]{\color{red}" % (section_name, section_title)
                )
            else:
    ...
    def depart_title(self, node):
        self.out.append(self.context.pop())
        if (
            isinstance(node.parent, nodes.table)
            or node.parent.tagname == "document"
        ):
            self.pop_output_collector()
    ...
    def minitoc(self, node, title, depth):
        ...
        self.requirements["minitoc-" + minitoc_name] = (
            r"\do%stoc" % minitoc_name
        )
    ...
    def visit_topic(self, node):
        ...
        if "contents" in node["classes"]:
            ...
            if self.use_latex_toc:
                ...
                if title != "Contents":
                    self.out.append(
                        "\n\\renewcommand{\\contentsname}{%s}" % title
                    )
                self.out.append("\n\\tableofcontents\n")
            ...
            "abstract" in node["classes"] and self.settings.use_latex_abstract
        ):
