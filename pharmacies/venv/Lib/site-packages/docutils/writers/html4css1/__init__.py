...
class HTMLTranslator(writers._html_base.HTMLTranslator):
    ...
    def depart_colspec(self, node):
        ...
        for node in self.colspecs:
            ...
            self.body.append(
                self.emptytag(node, "col", width="%i%%" % colwidth)
            )
    ...
    def visit_docinfo(self, node):
        ...
        self.body.append(
            self.starttag(
                node, "table", CLASS="docinfo", frame="void", rules="none"
            )
        )
        self.body.append(
    ...
    def visit_field_list(self, node):
        ...
        if "compact" in node["classes"]:
            ...
            self.settings.compact_field_lists and "open" not in node["classes"]
        ):
            self.compact_field_list = True
    ...
    def visit_field_name(self, node):
        ...
        if (
            self.settings.field_name_limit
            and len(node.astext()) > self.settings.field_name_limit
        ):
            atts["colspan"] = 2
            self.context.append(
                "</tr>\n"
                + self.starttag(node.parent, "tr", "", CLASS="field")
                + "<td>&nbsp;</td>"
            )
        else:
            self.context.append("")
    ...
    def visit_footnote_reference(self, node):
        ...
        self.body.append(
            self.starttag(
                node, "a", suffix, CLASS="footnote-reference", href=href
            )
        )
    ...
    def visit_label(self, node):
        self.body.append(
            self.starttag(
                node, "td", "%s[" % self.context.pop(), CLASS="label"
            )
        )

    def depart_label(self, node):
        self.body.append(
            "]%s</td><td>%s" % (self.context.pop(), self.context.pop())
        )
    ...
    def visit_subscript(self, node):
        if isinstance(node.parent, nodes.literal_block):
            self.body.append(
                self.starttag(node, "span", "", CLASS="subscript")
            )
        else:
    ...
    def visit_subtitle(self, node):
        if isinstance(node.parent, nodes.sidebar):
            self.body.append(
                self.starttag(node, "p", "", CLASS="sidebar-subtitle")
            )
            self.context.append("</p>\n")
    ...
    def visit_superscript(self, node):
        if isinstance(node.parent, nodes.literal_block):
            self.body.append(
                self.starttag(node, "span", "", CLASS="superscript")
            )
        else:
