...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def starttag(self, node, tagname, suffix="\n", empty=False, **attributes):
        ...
        for name, value in attlist:
            ...
            if isinstance(value, list):
                ...
                parts.append(
                    '%s="%s"' % (name.lower(), self.attval(" ".join(values)))
                )
            else:
                parts.append(
                    '%s="%s"' % (name.lower(), self.attval(unicode(value)))
                )
        if empty:
            infix = " /"
        else:
    ...
    def visit_address(self, node):
        ...
        self.body.append(
            self.starttag(node, "pre", suffix="", CLASS="address")
        )
    ...
    def is_compactable(self, node):
        ...
        if (
            isinstance(node, (nodes.field_list, nodes.definition_list))
            and not self.settings.compact_field_lists
        ):
            return False
        if (
            isinstance(node, (nodes.enumerated_list, nodes.bullet_list))
    ...
    def depart_citation(self, node):
        self.body.append("</dd>\n")
        if not isinstance(
            node.next_node(descend=False, siblings=True), nodes.citation
        ):
            self.body.append("</dl>\n")
            self.in_footnote_list = False
    ...
    def depart_colspec(self, node):
        ...
        if isinstance(
            node.next_node(descend=False, siblings=True), nodes.colspec
        ):
            return
    ...
    def visit_container(self, node):
        self.body.append(
            self.starttag(node, "div", CLASS="docutils container")
        )
    ...
    def visit_definition_list_item(self, node):
        ...
        node.children[0]["classes"] = node.get("classes", []) + node.children[
            0
        ].get("classes", [])
        node.children[0]["ids"] = node.get("ids", []) + node.children[0].get(
            "ids", []
        )
        node.children[0]["names"] = node.get("names", []) + node.children[
            0
        ].get("names", [])
    ...
    def visit_doctest_block(self, node):
        self.body.append(
            self.starttag(node, "pre", suffix="", CLASS="code python doctest")
        )
    ...
    def visit_field_name(self, node):
        self.body.append(
            self.starttag(
                node, "dt", "", CLASS="".join(node.parent["classes"])
            )
        )
    ...
    def visit_field_body(self, node):
        self.body.append(
            self.starttag(
                node, "dd", "", CLASS="".join(node.parent["classes"])
            )
        )
    ...
    def depart_footnote(self, node):
        self.body.append("</dd>\n")
        if not isinstance(
            node.next_node(descend=False, siblings=True), nodes.footnote
        ):
            self.body.append("</dl>\n")
            self.in_footnote_list = False
    ...
    def visit_footnote_reference(self, node):
        ...
        self.body.append(
            self.starttag(node, "a", "", CLASS=classes, href=href)  # suffix,
        )
    ...
    def visit_generated(self, node):
        if "sectnum" in node["classes"]:
            ...
            self.body.append(
                '<span class="sectnum">%s</span> ' % self.encode(sectnum)
            )
            # Content already processed:
            raise nodes.SkipNode
    ...
    def visit_image(self, node):
        ...
        if "scale" in node:
            if (
                PIL
                and not ("width" in node and "height" in node)
        ...
        if ext in self.object_image_types:
            ...
            self.body.append(
                self.starttag(node, "object", suffix, **atts)
                + node.get("alt", uri)
                + "</object>"
                + suffix
            )
    ...
    def visit_label(self, node):
        ...
        if self.settings.footnote_backlinks:
            ...
            if len(backrefs) == 1:
                self.body.append(
                    '<a class="fn-backref" href="#%s">' % backrefs[0]
                )
    ...
    def depart_label(self, node):
        ...
        if self.settings.footnote_backlinks and len(backrefs) > 1:
            ...
            self.body.append(
                '<span class="fn-backref">(%s)</span>' % ",".join(backlinks)
            )
        self.body.append("</dt>\n<dd>")
    ...
    def visit_math(self, node, math_env=""):
        ...
        if self.math_output == "mathjax" and not self.math_header:
                ...
                self.math_header = [
                    self.stylesheet_call(
                        utils.find_file_in_dirs(
                            s, self.settings.stylesheet_dirs
                        )
                    )
            ...
            try:
                ...
                self.body.append("</p>\n")
                self.body.append(
                    self.starttag(node, "pre", CLASS="literal-block")
                )
    ...
    def depart_option(self, node):
        self.body.append("</span>")
        if isinstance(
            node.next_node(descend=False, siblings=True), nodes.option
        ):
            self.body.append(", ")
    ...
    def depart_paragraph(self, node):
        self.body.append("</p>")
        if not (
            isinstance(node.parent, (nodes.list_item, nodes.entry))
    ...
    def visit_system_message(self, node):
        ...
        if len(node["backrefs"]):
            ...
            if len(backrefs) == 1:
                backref_text = (
                    '; <em><a href="#%s">backlink</a></em>' % backrefs[0]
                )
            else:
                i = 1
                backlinks = []
                for backref in backrefs:
                    ...
                    i += 1
                backref_text = "; <em>backlinks: %s</em>" % ", ".join(
                    backlinks
                )
    ...
    def visit_title(self, node):
        ...
        if isinstance(node.parent, nodes.topic):
            ...
            atts = {}
            if len(node.parent) >= 2 and isinstance(
                node.parent[1], nodes.subtitle
            ):
