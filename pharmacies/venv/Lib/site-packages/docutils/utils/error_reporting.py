    ...
    def __str__(self):
        try:
            ...
            if isinstance(self.data, Exception):
                args = [
                    str(SafeString(arg, self.encoding, self.encoding_errors))
    ...
    def __unicode__(self):
        ...
        * else decode with `self.encoding` and `self.decoding_errors`.
        ...
        try:
            ...
            if isinstance(self.data, EnvironmentError):
                return u"[Errno %s] %s: '%s'" % (
                    self.data.errno,
                    SafeString(
                        self.data.strerror, self.encoding, self.decoding_errors
                    ),
                    SafeString(
                        self.data.filename, self.encoding, self.decoding_errors
                    ),
                )
            if isinstance(self.data, Exception):
                args = [
    ...
    def write(self, data):
        ...
        try:
            ...
            if self.stream in (sys.stderr, sys.stdout):
                ...
                self.stream.write(
                    unicode(data, self.encoding, self.decoding_errors)
                )
