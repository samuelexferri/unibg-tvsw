...
def relative_path(source, target):
    ...
    """
    source_parts = os.path.abspath(source or type(target)("dummy_file")).split(
        os.sep
    )
    ...
    while (
        source_parts and target_parts and source_parts[-1] == target_parts[-1]
    ):
...
def get_trim_footnote_ref_space(settings):
    ...
    if settings.trim_footnote_reference_space is None:
        return (
            hasattr(settings, "footnote_references")
            and settings.footnote_references == "superscript"
        )
    else:
...
def unique_combinations(items, n):
    ...
    warnings.warn(
        "docutils.utils.unique_combinations is deprecated; "
    ...
    def set_output(self, output_file):
        ...
        if output_file:
            if output_file == "-":
                ...
                of = output_file
            self.file = docutils.io.FileOutput(
                destination_path=of, encoding="utf8", autoclose=False
            )
        else:
            self.file = None
