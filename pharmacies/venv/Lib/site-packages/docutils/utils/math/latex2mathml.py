...
sumintprod = "".join(
    [special[symbol] for symbol in ["sum", "int", "oint", "prod"]]
)
...
class mstyle(math):
    ...
    def xml_start(self):
        return (
            ["<mstyle "]
            + ['%s="%s"' % item for item in self.attrs.items()]
            + [">"]
        )
    ...
    while len(string) > 0:
        ...
        if c == " ":
            ...
            if isinstance(child, msub):
                ...
                isinstance(child, munder)
                and child.children[0].data in sumintprod
            ):
