...
class Node(object):
    ...
    def traverse(
        ...
        * the siblings of the parent (if ascend is true) and their
        ...
        return _traversal_list(
            self._traverse(condition, include_self, descend, siblings, ascend)
        )
    ...
    def _traverse(
        ...
        if descend and len(self.children):
            for child in self:
                for subnode in child._traverse(
                    condition=condition,
                    include_self=True,
                    descend=True,
                    siblings=False,
        ...
        if siblings or ascend:
            ...
            while node.parent:
                ...
                for sibling in node.parent[index + 1 :]:
                    for subnode in sibling._traverse(
                        condition=condition,
                        include_self=True,
                        descend=descend,
                        siblings=False,
    ...
    def next_node(
        ...
        """
        node_iterator = self._traverse(
            condition, include_self, descend, siblings, ascend
        )
        try:
...
class Element(Node):
    ...
    def replace_self(self, new):
        ...
        if isinstance(update, Element):
            ...
            for att in self.basic_attributes:
                assert not self[att], 'Losing "%s" attribute: %s' % (
                    att,
                    self[att],
                )
    ...
    def first_child_not_matching_class(
        self, childclass, start=0, end=sys.maxsize
    ):
        """
    ...
    def set_class(self, name):
        ...
        warnings.warn(
            "docutils.nodes.Element.set_class deprecated; "
...
class TextElement(Element):
    ...
    def __init__(self, rawsource="", text="", *children, **attributes):
        if text != "":
            textnode = Text(text)
            Element.__init__(
                self, rawsource, textnode, *children, **attributes
            )
        else:
            ...
            for name in node["names"]:
                ...
                if base_id and auto_id_prefix.endswith("%"):
                    ...
                    if prefix.endswith("%"):
                        prefix = "%s%s-" % (
                            prefix[:-1],
                            suggested_prefix or make_id(node.tagname),
                        )
                while True:
...
class pending(Special, Invisible, Element):
    ...
    def __init__(
        self, transform, details=None, rawsource="", *children, **attributes
    ):
        Element.__init__(self, rawsource, *children, **attributes)
    ...
    def pformat(self, indent="    ", level=0):
        internals = [
            ".. internal attributes:",
            "     .transform: %s.%s"
            % (self.transform.__module__, self.transform.__name__),
            "     .details:",
        ]
    ...
    def copy(self):
        obj = self.__class__(
            self.transform, self.details, self.rawsource, **self.attributes
        )
        obj.document = self.document
        obj.source = self.source
        obj.line = self.line
...
class NodeVisitor(object):
    ...
    def unknown_visit(self, node):
        ...
        """
        if (
            self.document.settings.strict_visitor
