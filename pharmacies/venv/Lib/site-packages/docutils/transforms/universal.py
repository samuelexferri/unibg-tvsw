    ...
    def generate_footer(self):
        ...
        if (
            ...
            if (
                ...
                and settings._source
                or settings.source_url
            ):
                if settings.source_url:
                    source = settings.source_url
                else:
                    source = utils.relative_path(
                        settings._destination, settings._source
                    )
                text.extend(
                    [
                        nodes.reference(
...
class SmartQuotes(Transform):
    ...
    def get_tokens(self, txtnodes):
        ...
        for node in txtnodes:
            if isinstance(node.parent, self.literal_nodes) or isinstance(
                node.parent.parent, self.literal_nodes
            ):
