    ...
    def apply(self):
        for target in self.document.traverse(nodes.target):
            ...
            if isinstance(target.parent, nodes.TextElement) or (
                target.hasattr("refid")
                or target.hasattr("refuri")
            ...
            if next_node is not None and (
                (
                    not isinstance(next_node, nodes.Invisible)
                    and not isinstance(next_node, nodes.Targetable)
                )
                or isinstance(next_node, nodes.target)
            ):
    ...
    def resolve_indirect_target(self, target):
        ...
        if refname is None:
            ...
            if not reftarget_id:
                ...
                for (
                    resolver_function
                ) in self.document.transformer.unknown_reference_resolvers:
        ...
        if (
            isinstance(reftarget, nodes.target)
...
class Footnotes(Transform):
    ...
    def symbolize_footnotes(self):
        ...
        for footnote in self.document.symbol_footnotes:
            reps, index = divmod(
                self.document.symbol_footnote_start, len(self.symbols)
            )
        ...
        for ref in subreflist:
            ...
            if key is None:
                ...
                if "rtrim" in subdef.attributes or "trim" in subdef.attributes:
                    if len(parent) > index + 1 and isinstance(
                        parent[index + 1], nodes.Text
                    ):
                ...
                try:
                    ...
                    if isinstance(parent, nodes.substitution_definition):
                        msg = self.document.reporter.error(
                            ...
                            nodes.literal_block(
                                parent.rawsource, parent.rawsource
                            ),
                            line=parent.line,
                            base_node=parent,
                        )
    ...
    def apply(self):
        ...
        for ref in self.document.traverse(nodes.reference):
            ...
            if ref.hasattr("refuri"):
                footnote = self.make_target_footnote(
                    ref["refuri"], [ref], notes
                )
        ...
        for target in self.document.traverse(nodes.target):
            if not target.referenced:
                ...
                self.document.reporter.info(
                    'Hyperlink target "%s" is not referenced.' % naming,
                    base_node=target,
                )
