...
class RSTStateMachine(StateMachineWS):
    ...
    def run(
        ...
        self.node = document
        results = StateMachineWS.run(
            self, input_lines, input_offset, input_source=document["source"]
        )
...
class NestedStateMachine(StateMachineWS):
    ...
    def run(self, input_lines, input_offset, memo, node, match_titles=True):
        ...
        assert results == [], (
            "NestedStateMachine.run() results should be " "empty!"
        )
        return results
...
class RSTState(StateWS):
    ...
    def runtime_init(self):
        ...
        if not hasattr(self.reporter, "get_source_and_line"):
            self.reporter.get_source_and_line = (
    ...
    def nested_parse(
        ...
        if not state_machine:
            state_machine = state_machine_class(
                debug=self.debug, **state_machine_kwargs
            )
        state_machine.run(
            block,
            input_offset,
    ...
    def inline_text(self, text, lineno):
        ...
        """
        nodes, messages = self.inliner.parse(
            text, lineno, self.memo, self.parent
        )
...
class Inliner(object):
    ...
    def init_customizations(self, settings):
        ...
        self.patterns = Struct(
            ...
            literal=re.compile(
                self.non_whitespace_before + "(``)" + end_string_suffix,
                re.UNICODE,
            ),
            target=re.compile(
        ...
        if settings.pep_references:
            self.implicit_dispatch.append(
                (self.patterns.pep, self.pep_reference)
            )
        if settings.rfc_references:
            self.implicit_dispatch.append(
                (self.patterns.rfc, self.rfc_reference)
            )
    ...
    def inline_obj(
        self, match, lineno, end_pattern, nodeclass, restore_backslashes=False
    ):
        string = match.string
        ...
        if match:  # embedded <URI> or <alias_>
            ...
            if not aliastext:
                raise ApplicationError(
                    "problem with embedded link: %r" % aliastext
                )
            if not text:
                text = alias
                unescaped = unescape(text)
    ...
    def interpreted(self, rawsource, text, role, lineno):
        ...
        if role_fn:
            ...
            return (
                [self.problematic(rawsource, rawsource, msg)],
                messages + [msg],
            )
    ...
    def substitution_reference(self, match, lineno):
        before, inlines, remaining, sysmessages, endstring = self.inline_obj(
            match,
            lineno,
            self.patterns.substitution_ref,
    ...
    def standalone_uri(self, match, lineno):
        if (
            ...
            reference = nodes.reference(
                unescape(text, True), text, refuri=refuri
            )
            return [reference]
...
class Body(RSTState):
    ...
    def list_item(self, indent):
        ...
        if indented:
            self.nested_parse(
                indented, input_offset=line_offset, node=listitem
            )
    ...
    def parse_enumerator(self, match, expected_sequence=None):
        ...
        text = groupdict[format][
            self.enum.formatinfo[format]
            .start : self.enum.formatinfo[format]
            .end
        ]
        if text == "#":
            sequence = "#"
    ...
    def make_enumerator(self, ordinal, sequence, format):
        ...
        if sequence == "#":
            ...
            if sequence.endswith("alpha"):
                ...
                raise ParserError(
                    'unknown enumerator sequence: "%s"' % sequence
                )
        ...
        next_enumerator = (
            formatinfo.prefix + enumerator + formatinfo.suffix + " "
        )
    ...
    def option_marker(self, match, context, next_state):
        ...
        try:
            ...
            msg = self.reporter.error(
                u"Invalid option list marker: %s" % error
            )
            self.parent += msg
            (
                indented,
    ...
    def option_list_item(self, match):
        ...
        if indented:
            self.nested_parse(
                indented, input_offset=line_offset, node=description
            )
    ...
    def parse_option_marker(self, match):
        ...
        for optionstring in optionstrings:
            ...
            if 0 < len(tokens) <= 2:
                ...
                if len(tokens) > 1:
                    option += nodes.option_argument(
                        tokens[1], tokens[1], delimiter=delimiter
                    )
    ...
    def table_top(
        self, match, context, next_state, isolate_function, parser_class
    ):
    ...
    def table(self, isolate_function, parser_class):
        ...
        if block:
            try:
                ...
                nodelist = (
                    self.malformed_table(
                        block, " ".join(err.args), offset=err.offset
                    )
                    + messages
                )
        else:
    ...
    def malformed_table(self, block, detail="", offset=0):
        ...
        error = self.reporter.error(
            message, nodes.literal_block(data, data), line=startline + offset
        )
        return [error]
    ...
    def build_table_row(self, rowdata, tableline):
        ...
        for cell in rowdata:
            ...
            if "".join(cellblock):
                self.nested_parse(
                    cellblock, input_offset=tableline + offset, node=entry
                )
        return row
    ...
    def hyperlink_target(self, match):
        ...
        target = self.make_target(
            block, blocktext, lineno, targetmatch.group("name")
        )
    ...
    def parse_target(self, block, block_text, lineno):
        ...
        reference = " ".join(
            "".join(unescape(part).split()) for part in ref_parts
        )
    ...
    def substitution_def(self, match):
        ...
        for node in substitution_node[:]:
            if not (
                isinstance(node, nodes.Inline) or isinstance(node, nodes.Text)
            ):
    ...
    def parse_directive_block(
        self, indented, line_offset, directive, option_presets
    ):
    ...
    def parse_directive_options(self, option_presets, option_spec, arg_block):
        ...
        for i, line in enumerate(arg_block):
            ...
            opt_block = []
        if opt_block:
            success, data = self.parse_extension_options(
                option_spec, opt_block
            )
    ...
    def parse_directive_arguments(self, directive, arg_block):
        ...
        if len(arguments) < required:
            raise MarkupError(
                "%s argument(s) required, %s supplied"
...
class RFC2822Body(Body):
    ...
    def rfc2822_field(self, match):
        ...
        if indented:
            self.nested_parse(
                indented, input_offset=line_offset, node=fieldbody
            )
...
class SubstitutionDef(Body):
    ...
    def embedded_directive(self, match, context, next_state):
        nodelist, blank_finish = self.directive(
            match, alt=self.parent["names"][0]
        )
        self.parent += nodelist
...
class QuotedLiteralBlock(RSTState):
    ...
    patterns = {
        "initial_quoted": r"(%(nonalphanum7bit)s)" % Body.pats,
        "text": r"",
    }
    ...
    def indent(self, match, context, next_state):
        assert context, (
            "QuotedLiteralBlock.indent: context should not " "be empty!"
        )
    ...
    def initial_quoted(self, match, context, next_state):
        ...
        self.add_transition(
            "quoted", (pattern, self.quoted, self.__class__.__name__)
        )
