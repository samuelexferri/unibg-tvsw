...
class TableParser(object):
    ...
    def find_head_body_sep(self):
        ...
        if self.head_body_sep == 0 or self.head_body_sep == (
            len(self.block) - 1
        ):
            raise TableMarkupError(
...
class GridTableParser(TableParser):
    ...
    def parse_table(self):
        ...
        while corners:
            ...
            cellblock = self.block.get_2D_block(
                top + 1, left + 1, bottom, right
            )
    ...
    def structure_from_cells(self):
        ...
        colspecs = [
            (colseps[i] - colseps[i - 1] - 1) for i in range(1, len(colseps))
    ...
    def parse_table(self):
        ...
        while offset < len(self.block):
            ...
            if self.span_pat.match(line):
                ...
                self.parse_row(
                    self.block[start:offset], start, (line.rstrip(), offset)
                )
                start = offset + 1
                text_found = None
    ...
    def parse_columns(self, line, offset):
        ...
        if self.columns:
            if cols[-1][1] != self.border_end:
                raise TableMarkupError(
                    "Column span incomplete in table "
                    "line %s." % (offset + 1),
                    offset=offset,
                )
