...
class CSVTable(Table):
    ...
    def run(self):
        try:
            ...
            self.extend_short_rows_with_empty_cells(
                max_cols, (table_head, table_body)
            )
        ...
        table_node = self.state.build_table(
            table, self.content_offset, stub_columns, widths=self.widths
        )
        table_node["classes"] += self.options.get("class", [])
        if "align" in self.options:
            table_node["align"] = self.options.get("align")
...
class ListTable(Table):
    ...
    def build_table_from_list(
        self, table_data, col_widths, header_rows, stub_columns
    ):
        table = nodes.table()
        if self.widths == "auto":
