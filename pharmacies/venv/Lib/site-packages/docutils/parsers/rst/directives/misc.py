...
class Role(Directive):
    ...
    argument_pattern = re.compile(
        r"(%s)\s*(\(\s*(%s)\s*\)\s*)?$" % ((states.Inliner.simplename,) * 2)
    )
    ...
    def run(self):
        ...
        if self.content_offset > self.lineno or not self.content:
            raise self.error(
                '"%s" directive requires arguments on the first '
                "line." % self.name
            )
        args = self.content[0]
        ...
        if not match:
            raise self.error(
                '"%s" directive arguments not valid role names: '
                '"%s".' % (self.name, args)
            )
    ...
    def run(self):
        ...
        if sys.version_info < (3, 0):
            try:
                ...
                raise self.warning(
                    u"Cannot encode date format string "
