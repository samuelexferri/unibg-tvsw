...
class BasePseudoSection(Directive):
    ...
    def run(self):
        if not (
            self.state_machine.match_titles
            or isinstance(self.state_machine.node, nodes.sidebar)
        ):
            raise self.error(
                'The "%s" directive may not be used within '
        ...
        if "subtitle" in self.options:
            ...
            titles.append(
                nodes.subtitle(self.options["subtitle"], "", *textnodes)
            )
...
class Sidebar(BasePseudoSection):
    ...
    def run(self):
        if isinstance(self.state_machine.node, nodes.sidebar):
            raise self.error(
                'The "%s" directive may not be used within a '
