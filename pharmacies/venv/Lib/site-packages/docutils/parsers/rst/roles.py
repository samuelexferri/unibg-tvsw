...
def role(role_name, language_module, lineno, reporter):
    ...
    if not canonicalname:
        try:
            ...
            msg_text.append(
                'Using English fallback for role "%s".' % role_name
            )
        except KeyError:
...
class CustomRole(object):
    ...
    def __call__(
        ...
        return self.base_role(
            role, rawtext, text, lineno, inliner, options=opts, content=cont
        )
...
def pep_reference_role(
    ...
    ref = (
        inliner.document.settings.pep_base_url
...
def rfc_reference_role(
    ...
    node = nodes.reference(
        rawtext, "RFC " + str(rfcnum), refuri=ref, **options
    )
    return [node], []
...
register_canonical_role(
    "restructuredtext-unimplemented-role", unimplemented_role
)
