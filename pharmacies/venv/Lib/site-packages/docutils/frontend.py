...
from docutils.utils.error_reporting import (
    locale_encoding,
    SafeString,
    ErrorOutput,
    ErrorString,
...
def validate_encoding(
    ...
    try:
        ...
        raise LookupError(
            'setting "%s": unknown encoding: "%s"' % (setting, value)
        )
    return value
...
def validate_encoding_and_error_handler(
    ...
    if ":" in value:
        ...
        if config_parser:
            config_parser.set(
                config_section, setting + "_error_handler", handler
            )
        else:
        ...
        encoding = value
    validate_encoding(
        setting, encoding, option_parser, config_parser, config_section
    )
    return encoding
...
def validate_strip_class(
    ...
    for cls in value:
        ...
        if cls != normalized:
            raise ValueError(
                "Invalid class value %r (perhaps %r?)" % (cls, normalized)
            )
    return value
    ...
    for key in keys:
        if key in pathdict:
            ...
            if isinstance(value, list):
                value = [
                    make_one_path_absolute(base_path, path) for path in value
                ]
            elif value:
...
class Values(optparse.Values):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            not hasattr(self, "record_dependencies")
...
class OptionParser(optparse.OptionParser, docutils.SettingsSpec):
    ...
    default_error_encoding = (
        getattr(sys.stderr, "encoding", None) or locale_encoding or "ascii"
    )
    ...
    settings_spec = (
        ...
        (
            ...
            (
                ...
                ["--no-generator"],
                {"action": "store_false", "dest": "generator"},
            ),
            (
        ...
        for component in components:
            ...
            for i in range(0, len(settings_spec), 3):
                ...
                for (help_text, option_strings, kwargs) in option_spec:
                    option = group.add_option(
                        help=help_text, *option_strings, **kwargs
                    )
    ...
    def get_config_file_settings(self, config_file):
        ...
        for component in self.components:
            ...
            for section in tuple(
                component.config_section_dependencies or ()
...
"""
    ...
    def handle_old_config(self, filename):
        warnings.warn_explicit(
            self.old_warning, ConfigDeprecationWarning, filename, 0
        )
