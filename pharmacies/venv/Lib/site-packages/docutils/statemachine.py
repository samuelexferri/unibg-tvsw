...
class StateMachine(object):
    ...
    def insert_input(self, input_lines, source):
        ...
        self.input_lines.insert(
            self.line_offset + 2, StringList(input_lines, source)
        )
    ...
    def get_text_block(self, flush_left=False):
        ...
        """
        try:
            block = self.input_lines.get_text_block(
                self.line_offset, flush_left
            )
...
class StateWS(State):
    ...
    def known_indent(self, match, context, next_state):
        ...
        sm = self.known_indent_sm(
            debug=self.debug, **self.known_indent_sm_kwargs
        )
        results = sm.run(indented, input_offset=line_offset)
        return context, next_state, results
    ...
    def insert(self, i, item, source=None, offset=0):
        if source is None:
            ...
            if self.parent:
                index = (len(self.data) + i) % len(self.data)
                self.parent.insert(
                    index + self.parent_offset, item, source, offset
                )
    ...
    def get_text_block(self, start, flush_left=False):
        ...
        while end < last:
            ...
            if flush_left and (line[0] == " "):
                ...
                raise UnexpectedIndentationError(
