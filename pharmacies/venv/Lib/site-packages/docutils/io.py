...
class Input(TransformSpec):
    ...
    def __repr__(self):
        return "%s: source=%r, source_path=%r" % (
            self.__class__,
            self.source,
            self.source_path,
        )
...
class FileOutput(Output):
    ...
    def __init__(
        ...
        """
        Output.__init__(
            self, destination, destination_path, encoding, error_handler
        )
        self.opened = True
        self.autoclose = autoclose
        if mode is not None:
        ...
        if destination is None:
            ...
            mode
            and hasattr(self.destination, "mode")
            and mode != self.destination.mode
        ):
            print(
                'Warning: Destination mode "%s" differs from specified '
    ...
    def open(self):
        ...
        try:
            ...
            raise OutputError(
                error.errno, error.strerror, self.destination_path
            )
        self.opened = True
        ...
        try:
            ...
            if sys.version_info >= (3, 0) and isinstance(data, bytes):
                try:
                    ...
                    if (
                        check_encoding(self.destination, self.encoding)
                        is False
                    ):
                        raise ValueError(
