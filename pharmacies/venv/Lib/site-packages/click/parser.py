...
def _error_opt_args(nargs, opt):
    ...
    raise BadOptionUsage(
        opt, "{} option requires {} arguments".format(opt, nargs)
    )
...
class Option(object):
    def __init__(self, opts, dest, action=None, nargs=1, const=None, obj=None):
        ...
        for opt in opts:
            ...
            if not prefix:
                raise ValueError(
                    "Invalid start character for option ({})".format(opt)
                )
...
class OptionParser(object):
    ...
    def add_option(
        ...
        option = Option(
            opts, dest, action=action, nargs=nargs, const=const, obj=obj
        )
    ...
    def _match_long_opt(self, opt, explicit_value, state):
        if opt not in self._long_opt:
            possibilities = [
                word for word in self._long_opt if word.startswith(opt)
            ]
        ...
        if option.takes_value:
            ...
            raise BadOptionUsage(
                opt, "{} option does not take a value".format(opt)
            )
