...
def _length_hint(obj):
    ...
    try:
        ...
        try:
            ...
            return None
        if (
            hint is NotImplemented
            or not isinstance(hint, int_types)
            or hint < 0
        ):
            return None
        return hint
...
def pager(generator, color=None):
    ...
    try:
        if (
            hasattr(os, "system")
            and os.system('more "{}"'.format(filename)) == 0
        ):
...
class Editor(object):
    def __init__(
        self, editor=None, env=None, require_save=True, extension=".txt"
    ):
        self.editor = editor
        self.env = env
...
def open_url(url, wait=False, locate=False):
    ...
    if sys.platform == "darwin":
        ...
        if locate:
            ...
            args = 'explorer /select,"{}"'.format(
                _unquote_file(url.replace('"', ""))
            )
        else:
            args = 'start {} "" "{}"'.format(
        ...
        if locate:
            url = _unquote_file(url)
            args = 'cygstart "{}"'.format(
                os.path.dirname(url).replace('"', "")
            )
        else:
            args = 'cygstart {} "{}"'.format(
                "-w" if wait else "", url.replace('"', "")
            )
        return os.system(args)
