...
class _NonClosingTextIOWrapper(io.TextIOWrapper):
    def __init__(
        ...
        **extra
        ...
        self._stream = stream = _FixupStream(
            stream, force_readable, force_writable
        )
...
if PY2:
    ...
    def _force_correct_text_reader(
        text_reader, encoding, errors, force_readable=False
    ):
    ...
    def _force_correct_text_writer(
        text_writer, encoding, errors, force_writable=False
    ):
        return _force_correct_text_stream(
            text_writer,
            encoding,
    ...
    def filename_to_ui(value):
        if isinstance(value, bytes):
            ...
            value = value.encode("utf-8", "surrogateescape").decode(
                "utf-8", "replace"
            )
        return value
...
def open_stream(
    filename, mode="r", encoding=None, errors="strict", atomic=False
):
    binary = "b" in mode
...
if WIN:
    ...
    def _get_argv_encoding():
        return (
            getattr(sys.stdin, "encoding", None) or get_filesystem_encoding()
        )
