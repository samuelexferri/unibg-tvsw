...
def as_bootstrap_inline(form):
    ...
    for field in form.fields:
        name = form.fields[field].widget.__class__.__name__.lower()
        if not name.startswith("radio") and not name.startswith("checkbox"):
            form.fields[field].widget.attrs["placeholder"] = form.fields[
                field
            ].label
    ...
    c = context_class({"form": form, "css_classes": css_classes,})
    return template.render(c)
    ...
    for label_class in label_classes.split(" "):
        ...
        if column_count < 12:
            offset_class = "{split_class}-offset-{column_count}".format(
                split_class=split_class, column_count=column_count,
            )
            wrap_class = "{split_class}-{column_count}".format(
                split_class=split_class, column_count=12 - column_count,
            )
            css_classes["single_container"] += (
                offset_class + " " + wrap_class + " "
            )
