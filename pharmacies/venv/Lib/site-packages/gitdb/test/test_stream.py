...
class TestStream(TestBase):
    ...
    def _assert_stream_reader(
        self, stream, cdata, rewind_stream=lambda s: None
    ):
            ...
            for with_size in range(2):
                for ds in self.data_sizes:
                    ...
                    if with_size:
                        # need object data
                        zdata = zlib.compress(
                            make_object(str_blob_type, cdata)
                        )
                    ...
                    self._assert_stream_reader(
                        reader, cdata, lambda r: r.seek(0)
                    )
    ...
    def test_decompress_reader_special_case(self):
        ...
        for sha in (
            ...
            dump = mdb.store(
                IStream(ostream.type, ostream.size, BytesIO(data))
            )
