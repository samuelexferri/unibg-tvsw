...
class tzfile(_tzinfo):
    ...
    **Examples:**
    ...
    def _read_tzfile(self, fileobj):
        ...
        if timecnt:
            out.trans_idx = struct.unpack(
                ">%dB" % timecnt, fileobj.read(timecnt)
            )
        else:
            out.trans_idx = []
        ...
        if ttisstdcnt:
            isstd = struct.unpack(
                ">%db" % ttisstdcnt, fileobj.read(ttisstdcnt)
            )
        ...
        if ttisgmtcnt:
            isgmt = struct.unpack(
                ">%db" % ttisgmtcnt, fileobj.read(ttisgmtcnt)
            )
    ...
    def _find_comp(self, dt):
        ...
        try:
            with self._cache_lock:
                return self._cachecomp[
                    self._cachedate.index((dt, self._fold(dt)))
                ]
        except ValueError:
            pass
