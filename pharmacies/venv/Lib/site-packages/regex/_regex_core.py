...
class StringSet(RegexBase):
    _opcode = {
        (NOCASE, False): OP.STRING_SET,
        (IGNORECASE, False): OP.STRING_SET_IGN,
        (FULLCASE, False): OP.STRING_SET,
        (FULLIGNORECASE, False): OP.STRING_SET_FLD,
        (NOCASE, True): OP.STRING_SET_REV,
        (IGNORECASE, True): OP.STRING_SET_IGN_REV,
        (FULLCASE, True): OP.STRING_SET_REV,
        (FULLIGNORECASE, True): OP.STRING_SET_FLD_REV,
    }
    ...
    def _compile(self, reverse, fuzzy):
        ...
        if fuzzy:
            ...
            if len(branches) > 1:
                ...
                branch = branches[0]
            branch = branch.optimise(self.info, reverse).pack_characters(
                self.info
            )
            ...
            return [
                (self._opcode[case_flags, reverse], index, min_len, max_len)
            ]
    ...
    def _flatten(self, s):
        ...
        if isinstance(s, Branch):
            ...
            if n > 1:
                seq[:n] = [
                    String(
                        [c.value for c in seq[:n]], case_flags=self.case_flags
                    )
                ]
    ...
    def max_width(self):
        ...
        if self.case_flags & IGNORECASE:
            ...
            return max(
                len(_regex.fold_case(fold_flags, i))
                for i in self.info.kwargs[self.name]
            )
        else:
