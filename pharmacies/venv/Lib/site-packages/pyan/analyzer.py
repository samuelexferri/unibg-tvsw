...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def __init__(self, filenames, logger=None):
        ...
        self.module_to_filename = (
            {}
        )  # inverse mapping for recording which file each AST node came from
        ...
        self.class_base_nodes = (
            {}
        )  # pass 2: class Node: list of Node objects (local bases, no recursion)
        self.mro = (
            {}
        )  # pass 2: class Node: list of Node objects in Python's MRO order
    ...
    def process_one(self, filename):
        ...
        self.analyze_scopes(
            content, filename
        )  # add to the currently known scopes
    ...
    def resolve_base_classes(self):
        ...
        for node in self.class_base_ast_nodes:  # Node: list of AST nodes
            ...
            for ast_node in self.class_base_ast_nodes[node]:
                ...
                if isinstance(ast_node, ast.Name):
                    ...
                    _, baseclass_node = self.get_attribute(
                        ast_node
                    )  # don't care about obj, just grab attr
                else:  # give up
                    baseclass_node = None
                ...
                if (
                    isinstance(baseclass_node, Node)
                    and baseclass_node.namespace is not None
                ):
                    self.class_base_nodes[node].append(baseclass_node)

        self.logger.debug(
            "All base classes (non-recursive, local level only): %s"
            % self.class_base_nodes
        )
        ...
        self.mro = resolve_method_resolution_order(
            self.class_base_nodes, self.logger
        )
        self.logger.debug(
            "Method resolution order (MRO) for all analyzed classes: %s"
            % self.mro
        )
    ...
    def visit_Module(self, node):
        ...
        module_node = self.get_node(
            "", self.module_name, node, flavor=Flavor.MODULE
        )
    ...
    def visit_ClassDef(self, node):
        self.logger.debug(
            "ClassDef %s, %s:%s" % (node.name, self.filename, node.lineno)
        )
    ...
    def visit_FunctionDef(self, node):
        self.logger.debug(
            "FunctionDef %s, %s:%s" % (node.name, self.filename, node.lineno)
        )
        ...
        if self_name is not None:
            ...
            self.logger.info(
                'Method def: setting self name "%s" to %s'
                % (self_name, class_node)
            )
    ...
    def visit_AsyncFunctionDef(self, node):
        self.visit_FunctionDef(
            node
        )  # TODO: alias for now; tag async functions in output in a future version?
    ...
    def visit_Import(self, node):
        self.logger.debug(
            "Import %s, %s:%s"
            % (
                [format_alias(x) for x in node.names],
                self.filename,
                node.lineno,
            )
        )
    ...
    def visit_ImportFrom(self, node):
        ...
        if node.module:  # import some names from a module
            ...
            self.logger.debug(
                "Use from %s to ImportFrom %s" % (from_node, to_node)
            )
            ...
            for (
                import_item
            ) in node.names:  # the names are items inside the module
                ...
                self.logger.info(
                    "From setting name %s to %s" % (new_name, tgt_id)
                )
    ...
    def analyze_module_import(self, import_item, ast_node):
        ...
        tgt_name = (
            import_item.asname if import_item.asname is not None else src_name
        )  # under which name
        ...
        to_node = self.get_node(
            "", tgt_name, ast_node, flavor=Flavor.IMPORTEDITEM
        )
        ...
        if src_name in self.module_names:
            ...
            mod_name = src_name
        tgt_module = self.get_node(
            "", mod_name, ast_node, flavor=Flavor.MODULE
        )
        ...
        if is_new_edge:
            self.logger.info(
                "New edge added for Use from %s to Import %s"
                % (from_node, to_node)
            )
    ...
    def visit_Constant(self, node):
        self.logger.debug(
            "Constant %s, %s:%s" % (node.value, self.filename, node.lineno)
        )
        t = type(node.value)
        tn = t.__name__
    ...
    def visit_Attribute(self, node):
        ...
        self.logger.debug(
            "Attribute %s of %s in context %s, %s:%s"
            % (node.attr, objname, type(node.ctx), self.filename, node.lineno)
        )
        ...
        if isinstance(node.ctx, ast.Store):
            ...
            if isinstance(attr_node, Node):
                self.logger.info(
                    "getattr %s on %s returns %s"
                    % (node.attr, objname, attr_node)
                )
                ...
                if self.add_uses_edge(from_node, attr_node):
                    self.logger.info(
                        "New edge added for Use from %s to %s"
                        % (from_node, attr_node)
                    )
            ...
            #
                ...
                ns = (
                    obj_node.get_name()
                )  # fully qualified namespace **of attr**
                to_node = self.get_node(
                    ns, tgt_name, node, flavor=Flavor.ATTRIBUTE
                )
                self.logger.debug(
                    "Use from %s to %s (target obj %s known but target attr %s not resolved; maybe fwd ref or unanalyzed import)"
                    % (from_node, to_node, obj_node, node.attr)
                )
                if self.add_uses_edge(from_node, to_node):
                    self.logger.info(
                        "New edge added for Use from %s to %s (target obj %s known but target attr %s not resolved; maybe fwd ref or unanalyzed import)"
                        % (from_node, to_node, obj_node, node.attr)
                    )
            ...
            # Object unknown, add uses edge to a wildcard by attr name.
                ...
                from_node = self.get_node_of_current_namespace()
                to_node = self.get_node(
                    None, tgt_name, node, flavor=Flavor.UNKNOWN
                )
                self.logger.debug(
                    "Use from %s to %s (target obj %s not resolved; maybe fwd ref, function argument, or unanalyzed import)"
                    % (from_node, to_node, objname)
                )
                if self.add_uses_edge(from_node, to_node):
                    self.logger.info(
                        "New edge added for Use from %s to %s (target obj %s not resolved; maybe fwd ref, function argument, or unanalyzed import)"
                        % (from_node, to_node, objname)
                    )
    ...
    def visit_Name(self, node):
        self.logger.debug(
            "Name %s in context %s, %s:%s"
            % (node.id, type(node.ctx), self.filename, node.lineno)
        )
        ...
        # A name in a load context is a use of the object the name points to.
            ...
            if (
                current_class is None or to_node is not current_class
            ):  # add uses edge only if not pointing to "self"
                ...
                from_node = self.get_node_of_current_namespace()
                self.logger.debug(
                    "Use from %s to Name %s" % (from_node, to_node)
                )
                if self.add_uses_edge(from_node, to_node):
                    self.logger.info(
                        "New edge added for Use from %s to Name %s"
                        % (from_node, to_node)
                    )
    ...
    def visit_Assign(self, node):
        ...
        if len(node.targets) > 1:
            self.logger.debug(
                "Assign (chained with %d outputs)" % (len(node.targets))
            )
        ...
        values = sanitize_exprs(
            node.value
        )  # values is the same for each set of targets
    ...
    def visit_AnnAssign(self, node):  # PEP 526, Python 3.6+
        ...
        if node.value is not None:
            ...
            self.logger.debug(
                "AnnAssign %s <no value>, %s:%s"
                % (get_ast_node_name(target[0]), self.filename, node.lineno)
            )
            self.last_value = None
            self.visit(target[0])
    ...
    def visit_AsyncFor(self, node):
        self.visit_For(
            node
        )  # TODO: alias for now; tag async for in output in a future version?
    ...
    def visit_DictComp(self, node):
        ...
        self.analyze_comprehension(
            node, "dictcomp", field1="key", field2="value"
        )
    ...
    def analyze_comprehension(self, node, label, field1="elt", field2=None):
        ...
        for expr in outermost_iters:
            self.visit(
                expr
            )  # set self.last_value (to something and hope for the best)
    ...
    def visit_Call(self, node):
        self.logger.debug(
            "Call %s, %s:%s"
            % (get_ast_node_name(node.func), self.filename, node.lineno)
        )
        ...
        if isinstance(result_node, Node):  # resolved result
            ...
            to_node = result_node
            self.logger.debug(
                "Use from %s to %s (via resolved call to built-ins)"
                % (from_node, to_node)
            )
            if self.add_uses_edge(from_node, to_node):
                self.logger.info(
                    "New edge added for Use from %s to %s (via resolved call to built-ins)"
                    % (from_node, to_node)
                )
            ...
            if self.last_value in self.class_base_ast_nodes:
                ...
                self.logger.debug(
                    "Use from %s to %s (call creates an instance)"
                    % (from_node, to_node)
                )
                if self.add_uses_edge(from_node, to_node):
                    self.logger.info(
                        "New edge added for Use from %s to %s (call creates an instance)"
                        % (from_node, to_node)
                    )

    def visit_With(self, node):
        self.logger.debug(
            "With (context manager), %s:%s" % (self.filename, node.lineno)
        )
        ...
        def add_uses_enter_exit_of(graph_node):
            ...
            if isinstance(graph_node, Node):
                ...
                self.logger.debug(
                    "Use from %s to With %s" % (from_node, withed_obj_node)
                )
                for methodname in ("__enter__", "__exit__"):
                    to_node = self.get_node(
                        withed_obj_node.get_name(),
                        methodname,
                        None,
                        flavor=Flavor.METHOD,
                    )
            ...
            if vars is not None:
                ...
                if isinstance(vars, ast.Name):
                    self.analyze_binding(
                        sanitize_exprs(vars), sanitize_exprs(expr)
                    )
                else:
                    self.visit(
                        vars
                    )  # just capture any uses on the With line itself
    ...
    def analyze_functiondef(self, ast_node):
        ...
        if not isinstance(ast_node, ast.FunctionDef):
            raise TypeError(
                "Expected ast.FunctionDef; got %s" % (type(ast_node))
            )
        ...
        for deco in ast_node.decorator_list:
            self.visit(
                deco
            )  # capture function name of decorator (self.last_value hack)
    ...
    def analyze_binding(self, targets, values):
        ...
        if len(targets) == len(
            ...
            for value in values:
                self.visit(
                    value
                )  # set self.last_value to **something** on the RHS and hope for the best
            ...
            if funcname == "super":
                ...
                if class_node in self.mro:
                    ...
                    if len(self.mro[class_node]) > 1:
                        ...
                        self.logger.debug(
                            "super of %s is %s" % (class_node, result)
                        )
                        return result
                    else:
                        msg = "super called for %s, but no known bases" % (
                            class_node
                        )
                    ...
                    msg = (
                        "super called for %s, but MRO not determined for it (maybe still in pass 1?)"
                        % (class_node)
                    )
                    self.logger.info(msg)
                    raise UnresolvedSuperCallError(msg)
    ...
    def resolve_attribute(self, ast_node):
        ...
        if not isinstance(ast_node, ast.Attribute):
            raise TypeError(
                "Expected ast.Attribute; got %s" % (type(ast_node))
            )

        self.logger.debug(
            "Resolve %s.%s in context %s"
            % (
                get_ast_node_name(ast_node.value),
                ast_node.attr,
                type(ast_node.ctx),
            )
        )
        ...
        if isinstance(ast_node.value, ast.Attribute):
            ...
            if isinstance(obj_node, Node) and obj_node.namespace is not None:
                ...
                if (
                    ns in self.scopes
                ):  # imported modules not in the set of analyzed files are not seen by Pyan
                    sc = self.scopes[ns]
                    if attr_name in sc.defs:
                        self.logger.debug(
                            "Resolved to attr %s of %s"
                            % (ast_node.attr, sc.defs[attr_name])
                        )
            ...
            # attribute of a function call. Detect cases like super().dostuff()
                ...
                if not isinstance(obj_node, Node):
                    self.logger.debug(
                        "Unresolved function call as obj, returning attr %s of unknown"
                        % (ast_node.attr)
                    )
                    return None, ast_node.attr
            else:
                ...
                obj_node = self.get_value(
                    get_ast_node_name(ast_node.value)
                )  # resolves "self" if needed

        self.logger.debug(
            "Resolved to attr %s of %s" % (ast_node.attr, obj_node)
        )
    ...
    def analyze_scopes(self, code, filename):
        ...
        process(
            self.module_name,
            symtable.symtable(code, filename, compile_type="exec"),
        )
    ...
    def get_value(self, name):
        ...
        if sc is not None:
            ...
            if isinstance(value, Node):
                self.logger.info(
                    "Get %s in %s, found in %s, value %s"
                    % (name, self.scope_stack[-1], sc, value)
                )
                return value
            else:
                # TODO: should always be a Node or None
                self.logger.debug(
                    "Get %s in %s, found in %s: value %s is not a Node"
                    % (name, self.scope_stack[-1], sc, value)
                )
        else:
            self.logger.debug(
                "Get %s in %s: no Node value (or name not in scope)"
    ...
    def set_value(self, name, value):
        ...
        if sc is not None:
            if isinstance(value, Node):
                ...
                # TODO: should always be a Node or None
                self.logger.debug(
                    "Set %s in %s: value %s is not a Node" % (name, sc, value)
                )
        else:
    ...
    def get_attribute(self, ast_node):
        ...
        if not isinstance(ast_node.ctx, ast.Load):
            raise ValueError(
                "Expected a load context, got %s" % (type(ast_node.ctx))
            )
        ...
        if isinstance(obj_node, Node) and obj_node.namespace is not None:
            ...
            if ns in ("Num", "Str"):  # TODO: other types?
                return (
                    obj_node,
                    self.get_node(
                        ns, attr_name, None, flavor=Flavor.ATTRIBUTE
                    ),
                )
            ...
            if obj_node in self.mro:
                for base_node in tail(
                    self.mro[obj_node]
                ):  # the first element is always obj itself
                ...
                return (
                    base_node,
                    value_node,
                )  # as obj, return the base class in which attr was found

        return (
            obj_node,
            None,
        )  # here obj_node is either None or unknown (namespace None)
    ...
    def set_attribute(self, ast_node, new_value):
        ...
        if not isinstance(ast_node.ctx, ast.Store):
            raise ValueError(
                "Expected a store context, got %s" % (type(ast_node.ctx))
            )
    ...
    def get_node(
        self, namespace, name, ast_node=None, flavor=Flavor.UNSPECIFIED
    ):
        ...
        if name in self.nodes:
            for n in self.nodes[name]:
                if n.namespace == namespace:
                    if Flavor.specificity(flavor) > Flavor.specificity(
                        n.flavor
                    ):
                        n.flavor = flavor
                    return n
    ...
    def remove_wild(self, from_node, to_node, name):
        ...
        matching_wilds = [
            n
            for n in self.uses_edges[from_node]
            if n.namespace is None and n.name == name
        ]
        assert (
            len(matching_wilds) < 2
        )  # the set can have only one wild of matching name
        if len(matching_wilds):
            wild_node = matching_wilds[0]
            self.logger.info(
                "Use from %s to %s resolves %s; removing wildcard"
                % (from_node, to_node, wild_node)
            )
        ...
        for n in self.uses_edges:
            for n2 in self.uses_edges[n]:
                if n2.namespace is not None and not n2.defined:
                    ...
                    self.logger.info(
                        "Contracting non-existent from %s to %s as %s"
                        % (n, n2, n3)
                    )
    ...
    def expand_unknowns(self):
        ...
        for from_node, to_node in new_defines_edges:
            ...
            self.logger.info(
                "Expanding unknowns: new defines edge from %s to %s"
                % (from_node, to_node)
            )
        ...
        for from_node, to_node in new_uses_edges:
            self.add_uses_edge(from_node, to_node)
            self.logger.info(
                "Expanding unknowns: new uses edge from %s to %s"
                % (from_node, to_node)
            )
            ...
            for n2 in self.uses_edges[n]:
                ...
                for n3 in self.uses_edges[n]:
                    if (
                        ...
                        if (
                            pn2 in self.uses_edges
                            and pn3 in self.uses_edges[pn2]
                        ):  # remove the first edge W to X.name
                ...
                if inherited and n in self.uses_edges:
                    removed_uses_edges.append((n, n2))
                    self.logger.info(
                        "Removing inherited edge from %s to %s" % (n, n2)
                    )
