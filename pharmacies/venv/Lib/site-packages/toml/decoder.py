    ...
    if _ispath(f):
        ...
        for l in f:
            if op.exists(l):
                ...
                warn(
                    "Non-existent filename in list with at least one valid "
                    "filename"
                )
        return d
    else:
        try:
            ...
            raise TypeError(
                "You can only load a file descriptor, filename or " "list"
            )
...
def loads(s, _dict=dict, decoder=None):
    ...
    for i, item in enumerate(sl):
        ...
        if item == '"' and openstrchar != "'":
            ...
            if openstring:
                ...
                openstrchar = ""
        if item == "#" and (
            not openstring and not keygroup and not arrayoftables
        ):
            j = i
            try:
                while sl[j] != "\n":
    ...
    for idx, line in enumerate(s):
        ...
        if line[0] == "[":
            ...
            if len(line) == 1:
                raise TomlDecodeError(
                    "Opening key group bracket on line by " "itself.",
                    original,
                    pos,
                )
            if line[1] == "[":
            ...
            while i < len(groups):
                ...
                if len(groups[i]) > 0 and (
                    groups[i][0] == '"' or groups[i][0] == "'"
                ):
                    groupstr = groups[i]
                    j = i + 1
            ...
            for i in _range(len(groups)):
                ...
                if group == "":
                    raise TomlDecodeError(
                        "Can't have a keygroup with an empty " "name",
                        original,
                        pos,
                    )
                try:
            ...
            if line[-1] != "}":
                raise TomlDecodeError(
                    "Line breaks are not allowed in inline" "objects",
                    original,
                    pos,
                )
            try:
                decoder.load_inline_object(
                    line, currentlevel, multikey, multibackslash
                )
            except ValueError as err:
                raise TomlDecodeError(str(err), original, pos)
        elif "=" in line:
            try:
                ret = decoder.load_line(
                    line, currentlevel, multikey, multibackslash
                )
            except ValueError as err:
                raise TomlDecodeError(str(err), original, pos)
...
class TomlDecoder(object):
    ...
    def load_inline_object(
        ...
        for group in groups:
            status = self.load_line(
                group, currentlevel, multikey, multibackslash
            )
        ...
        if len(doublequotesplits) > 1 and "'" in doublequotesplits[0]:
            ...
            while len(singlequotesplits) % 2 == 0 and len(doublequotesplits):
                ...
                if "'" in singlequotesplits[-1]:
                    singlequotesplits = singlequotesplits[
                        :-1
            ...
            if '"' in pair[0] or "'" in pair[0]:
                ...
                for quotesplit in quotesplits:
                    if quoted:
                        ...
                        levels += [
                            level.strip() for level in quotesplit.split(".")
                        ]
                    quoted = not quoted
            else:
                levels = pair[0].split(".")
    ...
    def load_value(self, v, strictly_valid=True):
        ...
        if v == "true":
            ...
            if quotechar == '"':
                ...
                for prefix in ["\\u", "\\U"]:
                    if prefix in v:
                        ...
                        v = _load_unicode_escapes(
                            hexbytes[0], hexbytes[1:], prefix
                        )
                v = _unescape(v)
            if (
                len(v) > 1
                and v[1] == quotechar
                and (len(v) < 3 or v[1] == v[2])
            ):
                v = v[2:-2]
            return (v[1:-1], "str")
        elif v[0] == "[":
            ...
            if not strictly_valid:
                raise ValueError(
                    "Weirdness with leading zeroes or "
            ...
            if "." in v or ("x" not in v and ("e" in v or "E" in v)):
                if "." in v and v.split(".", 1)[1] == "":
                    raise ValueError(
                        "This float is missing digits after " "the point"
                    )
                if v[0] not in "0123456789":
                    raise ValueError(
                        "This float doesn't have a leading " "digit"
                    )
                v = float(v)
                itype = "float"
        ...
        if "[" not in a[1:-1] or "" != a[1:-1].split("[")[0].strip():
            ...
            if not a[1:-1].strip().startswith("{"):
                ...
                while end_group_index < len(a[1:]):
                    ...
                    while (
                        start_group_index < len(a[1:])
                        and a[start_group_index] != "{"
                    ):
                        start_group_index += 1
