...
class TomlTz(datetime.tzinfo):
    ...
    def utcoffset(self, dt):
        return self._sign * datetime.timedelta(
            hours=self._hours, minutes=self._minutes
        )
...
def loads(s, _dict=dict):
    ...
    for line in s:
        ...
        if line[0] == "[":
            ...
            try:
                _load_inline_object(
                    line, currentlevel, _dict, multikey, multibackslash
                )
            except ValueError as err:
                raise TomlDecodeError(str(err))
        elif "=" in line:
...
def _load_inline_object(
    ...
    for group in groups:
        status = _load_line(
            group, currentlevel, _dict, multikey, multibackslash
        )
            ...
            while end_group_index < len(a[1:]):
                if a[end_group_index] == '"' or a[end_group_index] == "'":
                    if in_str:
                        ...
                        while (
                            backslash_index > -1 and a[backslash_index] == "\\"
                        ):
                            in_str = not in_str
                            backslash_index -= 1
                    in_str = not in_str
