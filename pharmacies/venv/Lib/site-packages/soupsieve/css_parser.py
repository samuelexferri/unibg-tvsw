...
# `nth` content
NTH = r"(?:[-+])?(?:[0-9]+n?|n)(?:(?<=n){ws}*(?:[-+]){ws}*(?:[0-9]+))?".format(
    ws=WSC
)
...
PAT_ATTR = r"\[{ws}*(?P<ns_attr>(?:(?:{ident}|\*)?\|)?{ident}){attr}".format(
...
PAT_COMBINE = r"{wsc}*?(?P<relation>[,+>~]|{ws}(?![,+>~])){wsc}*".format(
    ws=WS, wsc=WSC
)
...
RE_VALUES = re.compile(
    r"(?:(?P<value>{value})|(?P<split>{ws}*,{ws}*))".format(
        ws=WSC, value=VALUE
    ),
    re.X,
)
        ...
        for key, value in custom.items():
            ...
            if RE_CUSTOM.match(name) is None:
                raise SelectorSyntaxError(
                    "The name '{}' is not a valid custom pseudo-class name".format(
                        name
                    )
                )
            if name in custom_selectors:
                raise KeyError(
                    "The custom selector '{}' has already been registered".format(
                        name
                    )
                )
...
class CSSParser(object):
    ...
    def parse_pseudo_class(self, sel, m, has_selector, iselector, is_html):
        ...
        if complex_pseudo and pseudo in PSEUDO_COMPLEX:
            ...
            raise NotImplementedError(
                "'{}' pseudo-class is not implemented at this time".format(
                    pseudo
                )
            )
    ...
    def parse_has_combinator(
        self, sel, m, has_selector, selectors, rel_type, index
    ):
        ...
        try:
            while True:
                ...
                if key == "at_rule":
                    ...
                    raise NotImplementedError(
                        "Psuedo-element found at position {}".format(
                            m.start(0)
                        )
                    )
                    ...
                    if not has_selector:
                        raise SelectorSyntaxError(
                            "Expected a selector at postion {}".format(
                                m.start(0)
                            ),
                            self.pattern,
                            m.start(0),
    ...
    def selector_iter(self, pattern):
        ...
        while index <= end:
            ...
            if m is None:
                ...
                if c == "[":
                    msg = "Malformed attribute selector at position {}".format(
                        index
                    )
                elif c == ".":
                    msg = "Malformed class selector at position {}".format(
                        index
                    )
                elif c == "#":
                    msg = "Malformed id selector at position {}".format(index)
                elif c == ":":
                    msg = "Malformed pseudo-class selector at position {}".format(
                        index
                    )
                else:
