...
class ImmutableDict(util.Mapping):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            is_dict
            and not all([isinstance(v, util.Hashable) for v in arg.values()])
            or not is_dict
            and not all(
                [
                    isinstance(k, util.Hashable)
                    and isinstance(v, util.Hashable)
                    for k, v in arg
                ]
            )
        ):
...
class Namespaces(ImmutableDict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if is_dict and not all(
            [
                isinstance(k, util.string) and isinstance(v, util.string)
                for k, v in arg.items()
            ]
        ):
            raise TypeError(
                "Namespace keys and values must be Unicode strings"
            )
...
class CustomSelectors(ImmutableDict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if is_dict and not all(
            ...
            raise TypeError(
                "CustomSelectors keys and values must be Unicode strings"
            )
        elif not is_dict and not all(
            [
                isinstance(k, util.string) and isinstance(v, util.string)
