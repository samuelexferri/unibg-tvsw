...
def to_mutable(instance):
    if isinstance(instance, Dict):
        return {key: to_mutable(value) for key, value in instance.items()}
    elif isinstance(instance, List):
        return [to_mutable(value) for value in instance]
    return instance
...
def _to_hashable(instance):
    if isinstance(instance, Dict):
        ...
        return ((key, _to_hashable(value)) for key, value in items)
    elif isinstance(instance, List):
        return [_to_hashable(value) for value in instance]
    return instance
...
class Dict(Mapping):
    ...
    def __repr__(self):
        return "%s(%s)" % (self.__class__.__name__, to_mutable(self))
...
class List(Sequence):
    def __init__(self, *args):
        self._data = [to_immutable(value) for value in list(*args)]
