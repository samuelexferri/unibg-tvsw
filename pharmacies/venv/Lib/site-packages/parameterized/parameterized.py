...
class parameterized(object):
    ...
    def param_as_nose_tuple(self, test_self, func, num, p):
        ...
        if test_self is not None:
            ...
            func_self = (
                None if PY2 and detect_runner() == "nose" else test_self
            )
...
def parameterized_class(
    ...
    if classname_func:
        ...
        class_name_func = lambda cls, idx, input: classname_func(
            cls, idx, input_dicts
        )
    ...
    def decorator(base_class):
        ...
        for idx, input_dict in enumerate(input_dicts):
            ...
            test_class_module[name] = type(
                name, (base_class,), test_class_dict
            )
...
def get_class_name_suffix(params_dict):
    ...
    params_vals = (
        params_dict.values()
        if PY3
        else (v for (_, v) in sorted(params_dict.items()))
    )
    return parameterized.to_safe_name(
        next((v for v in params_vals if isinstance(v, string_types)), "")
    )


def default_class_name_func(cls, num, params_dict):
    suffix = get_class_name_suffix(params_dict)
    return "%s_%s%s" % (cls.__name__, num, suffix and "_" + suffix,)
