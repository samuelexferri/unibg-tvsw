...
class TestParameterizedExpandWithMockPatchForClass(TestCase):
    ...
    def test_one_function_patch_decorator(self, foo, mock_umask, mock_getpid):
        missing_tests.remove(
            "test_one_function_patch_decorator(%r, %r, %r)"
            % (foo, mock_umask._mock_name, mock_getpid._mock_name)
        )
...
class TestParameterizedExpandWithNoExpand(object):
    expect(
        "generator", ["test_patch_class_no_expand(42, 51, 'umask', 'getpid')",]
    )
    ...
    def test_patch_class_no_expand(self, foo, bar, mock_umask, mock_getpid):
        missing_tests.remove(
            "test_patch_class_no_expand"
            "(%r, %r, %r, %r)"
            % (foo, bar, mock_umask._mock_name, mock_getpid._mock_name)
        )
...
class TestParameterizedExpandWithNoMockPatchForClass(TestCase):
    ...
    def test_one_function_patch_decorator(self, foo, mock_umask):
        missing_tests.remove(
            "test_one_function_patch_decorator(%r, %r)"
    ...
    def test_multiple_function_patch_decorator(
        ...
        missing_tests.remove(
            "test_multiple_function_patch_decorator"
            "(%r, %r, %r, %r)"
            % (foo, bar, mock_umask._mock_name, mock_fdopen._mock_name)
        )


class TestParameterizedExpandWithNoMockPatchForClassNoExpand(object):
    expect("generator", ["test_patch_no_expand(42, 51, 'umask')",])

    @parameterized([(42, 51)])
    @mock.patch("os.umask")
    def test_patch_no_expand(self, foo, bar, mock_umask):
        missing_tests.remove(
            "test_patch_no_expand(%r, %r, %r)"
            % (foo, bar, mock_umask._mock_name)
        )


expect("standalone", ["test_mock_patch_standalone_function(42, 'umask')",])
...
class TestParameterizedExpandDocstring(TestCase):
    def _assert_docstring(self, expected_docstring, rstrip=False):
        ...
        test_method = (
            f_locals.get("testMethod")
            or f_locals.get("function")  # Py27
            or f_locals.get("method")  # Py33
            or f_locals.get("testfunction")  # Py38
            or None  # Py382
        )
        if test_method is None:
            raise AssertionError(
                "uh oh, unittest changed a local variable name"
            )
    ...
    def test_default_values_get_correct_value(self, foo, bar=12):
        """Documentation"""
        self._assert_docstring(
            "Documentation [with foo=%r, bar=%r]" % (foo, bar)
        )
    ...
    def test_with_leading_newline(self, foo, bar=12):
        ...
        """
        self._assert_docstring(
            "Documentation [with foo=%r, bar=%r]" % (foo, bar), rstrip=True
        )
...
expect("stadalone generator", ["test_wrapped_iterable_input('foo')",])
...
def test_old_style_classes():
    ...
    try:
        ...
        raise AssertionError(
            "expected TypeError not raised by old-style class"
        )
...
class TestOldStyleClass:
    expect(
        "py2nose2 generator",
        ["test_on_old_style_class('foo')", "test_on_old_style_class('bar')",],
    )
...
class TestParameterizedClass(TestCase):
    ...
    def _assertions(self, test_name):
        ...
        missing_tests.remove(
            "%s:%s(%r, %r, %r)"
            % (self.__class__.__name__, test_name, self.a, self.b, self.c,)
        )
...
class TestNamedParameterizedClass(TestCase):
    ...
    def test_method(self):
        missing_tests.remove(
            "%s:test_method(%r)" % (self.__class__.__name__, self.a,)
        )
...
class TestParameterizedClassDict(TestCase):
    ...
    def setUp(self):
        ...
        missing_tests.remove(
            "%s:setUp(%r, %r)" % (self.__class__.__name__, self.foo, self.bar,)
        )
    ...
    def tearDown(self):
        ...
        missing_tests.remove(
            "%s:tearDown(%r, %r)"
            % (self.__class__.__name__, self.foo, self.bar,)
        )
