...
def make_swagger_name(attribute_name):
    ...
    if attribute_name.startswith("x_"):
        return "x-" + camelize(
            attribute_name[2:], uppercase_first_letter=False
        )
...
def _bare_SwaggerDict(cls):
    ...
    OrderedDict.__init__(
        result
    )  # no __init__ called for SwaggerDict subclasses!
    return result
...
class SwaggerDict(OrderedDict):
    ...
    def __reduce__(self):
        ...
        attrs = {
            k: v for k, v in vars(self).items() if not k.startswith("_NP_")
        }
        return (
            _bare_SwaggerDict,
            (type(self),),
...
class PathItem(SwaggerDict):
    ...
    def operations(self):
        ...
        return [
            ...
            if k in PathItem.OPERATION_NAMES and v
        ]
...
def _check_type(type, format, enum, pattern, items, _obj_type):
    ...
    if (format or enum or pattern) and type in (TYPE_OBJECT, TYPE_ARRAY, None):
        raise AssertionError(
            "[format, enum, pattern] can only be applied to primitive "
            + _obj_type
        )
    ...
    def __init__(
        ...
        **extra
        ...
        if default and not type:
            raise AssertionError(
                "default can only be applied to a non-body Parameter"
            )
        _check_type(type, format, enum, pattern, items, self.__class__)
...
class Schema(SwaggerDict):
    ...
    def __init__(
        ...
        **extra
        ...
        self._insert_extras__()
        if (
            properties or (additional_properties is not None)
        ) and type != TYPE_OBJECT:
...
class _Ref(SwaggerDict):
    ...
    def __init__(
        self, resolver, name, scope, expected_type, ignore_unresolved=False
    ):
...
class ReferenceResolver(object):
    ...
    def _check_scope(self, scope):
        ...
        assert real_scope and real_scope in self._objects, (
            "invalid scope %s" % scope
        )
        return real_scope
    ...
    def get(self, name, scope=None):
        ...
        """
        scope = self._check_scope(scope)
        assert name in self._objects[scope], "#/%s/%s is not defined" % (
            scope,
            name,
        )
