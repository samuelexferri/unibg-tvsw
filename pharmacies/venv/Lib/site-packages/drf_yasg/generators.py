...
class EndpointEnumerator(_EndpointEnumerator):
    ...
    def should_include_endpoint(
        ...
        if not super(EndpointEnumerator, self).should_include_endpoint(
            path, callback
        ):
            return False
...
class OpenAPISchemaGenerator(object):
    ...
    def __init__(
        self, info, version="", url=None, patterns=None, urlconf=None
    ):
        """
        ...
        if overrides is not None:
            ...
            for method, _ in overrides.items():
                ...
                if view_method is not None:  # pragma: no cover
                    setattr(
                        view_method.__func__, "_swagger_auto_schema", overrides
                    )
    ...
    def get_paths(self, endpoints, components, request, public):
        ...
        assert (
            "{" not in prefix
        ), "base path cannot be templated in swagger 2.0"

        paths = OrderedDict()
        for path, (view_cls, methods) in sorted(endpoints.items()):
            operations = {}
            for method, view in methods:
                if not self.should_include_endpoint(
    ...
    def get_operation(self, view, path, prefix, method, components, request):
        ...
        view_inspector_cls = getattr(
            view, "swagger_schema", view_inspector_cls
        )
