...
class SwaggerAutoSchema(ViewInspector):
    ...
    def get_request_body_parameters(self, consumes):
        ...
        if any(is_form_media_type(encoding) for encoding in consumes):
            ...
            return (
                [self.make_body_parameter(schema)]
                if schema is not None
                else []
            )
    ...
    def get_default_responses(self):
        ...
        if default_schema:
            ...
            if self.should_page():
                default_schema = (
                    self.get_paginated_response(default_schema)
                    or default_schema
                )
    ...
    def get_response_serializers(self):
        ...
        if not any(
            is_success(int(sc)) for sc in manual_responses if sc != "default"
        ):
    ...
    def get_summary_and_description(self):
        ...
        if description is None:
            description = (
                self._sch.get_description(self.path, self.method) or ""
            )
