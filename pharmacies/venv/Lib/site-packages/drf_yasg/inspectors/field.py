...
class InlineSerializerInspector(SerializerInspector):
    ...
    def field_to_swagger_object(
        ...
        if isinstance(
            field, (serializers.ListSerializer, serializers.ListField)
        ):
            ...
            return SwaggerType(
                type=openapi.TYPE_ARRAY, items=child_schema, **limits
            )
            ...
            def make_schema_definition(serializer=field):
                ...
                for property_name, child in serializer.fields.items():
                    ...
                    if child.required and not getattr(
                        child_schema, "read_only", False
                    ):
                ...
                setattr(
                    result, "_NP_serializer", get_serializer_class(serializer)
                )
                return result
            ...
            actual_schema = definitions.setdefault(
                ref_name, make_schema_definition
            )
...
class RelatedFieldInspector(FieldInspector):
    ...
    def field_to_swagger_object(
        ...
        if isinstance(
            field,
            (serializers.PrimaryKeyRelatedField, serializers.SlugRelatedField),
        ):
            ...
            if field_queryset is not None:
                ...
                model, model_field = get_queryset_field(
                    field_queryset, target_field
                )
            else:
                ...
                if not this_model:
                    view_queryset = get_queryset_from_view(
...
def find_regex(regex_field):
    ...
    try:
        ...
        logger.warning(
            "failed to compile regex validator of " + str(regex_field),
            exc_info=True,
        )
        return None
...
def find_limits(field):
    ...
    if isinstance(field, serializers.DecimalField) and not decimal_as_float(
        field
    ):
        return limits
...
def decimal_field_type(field):
    return (
        openapi.TYPE_NUMBER if decimal_as_float(field) else openapi.TYPE_STRING
    )
...
serializer_field_to_basic_type = [
    ...
    (
        ...
        (
            openapi.TYPE_STRING,
            lambda field: ip_format.get(field.protocol, None),
        ),
    ),
...
if sys.version_info < (3, 0):
    ...
    hinting_type_info.append(
        (unicode, (openapi.TYPE_STRING, None))
    )  # noqa: F821
...
class SerializerMethodFieldInspector(FieldInspector):
    ...
    def field_to_swagger_object(
        ...
        if serializer:
            ...
            if not inspect.isclass(hint_class) and hasattr(
                hint_class, "__args__"
            ):
        ...
        if isinstance(field, serializers.FileField):
            ...
            if swagger_object_type == openapi.Schema:
                ...
                if getattr(
                    field,
                    "use_url",
                    rest_framework_settings.UPLOADED_FILES_USE_URL,
                ):
    ...
    def camelize_schema(self, schema):
        ...
        if getattr(schema, "properties", {}):
            ...
            if getattr(schema, "required", []):
                schema.required = [
                    self.camelize_string(p) for p in schema.required
                ]
        ...
        def field_to_swagger_object(
            ...
            if (
                ...
                ref = openapi.SchemaRef(
                    definitions, ref_name, ignore_unresolved=True
                )
                if isinstance(field.proxied, serializers.ListSerializer):
