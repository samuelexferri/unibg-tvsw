...
def swagger_auto_schema(
    ...
    **extra_overrides
    ...
    def decorator(view_method):
        ...
        mapping_methods = [
            mth
            for mth, name in mapping.items()
            if name == view_method.__name__
        ]
        ...
        if methods or method:
            ...
            assert bool(methods) != bool(
                method
            ), "specify either method or methods"
...
def is_list_view(path, method, view):
    ...
    if isinstance(
        view, (RetrieveModelMixin, UpdateModelMixin, DestroyModelMixin)
    ):
        return False
...
def param_list_to_odict(parameters):
    ...
    """
    result = OrderedDict(
        ((param.name, param.in_), param) for param in parameters
    )
...
def force_serializer_instance(serializer):
    ...
    assert isinstance(serializer, serializers.BaseSerializer), (
        "Serializer class or instance required, not %s"
...
def get_produces(renderer_classes):
    ...
    media_types = [
        ...
        for encoding in media_types
        if not any(
            excluded in encoding
            for excluded in swagger_settings.EXCLUDED_MEDIA_TYPES
        )
    ]
    return media_types
...
def decimal_as_float(field):
    ...
    if isinstance(field, serializers.DecimalField) or isinstance(
        ...
        return not getattr(
            field,
            "coerce_to_string",
            rest_framework_settings.COERCE_DECIMAL_TO_STRING,
        )
    return False
