...
class ApiField(object):
    ...
    def __init__(
        ...
        self._attrs = (
            ...
            and isinstance(attribute, six.string_types)
            else []
        )
...
class RelatedField(ApiField):
    ...
    def __init__(
        ...
        self.full = full
        self.full_list = (
            full_list if callable(full_list) else lambda bundle: full_list
        )
        self.full_detail = (
            full_detail
            if callable(full_detail)
            else lambda bundle: full_detail
        )
    ...
    def dehydrate_related(self, bundle, related_resource, for_list=True):
        ...
        """
        should_dehydrate_full_resource = self.should_full_dehydrate(
    ...
    def resource_from_uri(
        ...
        """
        err_msg = (
            "Could not find the provided %s object via resource URI '%s'."
        ...
        try:
            ...
            bundle = fk_resource.build_bundle(
                obj=obj, request=request, via_uri=True
            )
    ...
    def resource_from_data(
        ...
        obj = None
        if (
            getattr(fk_resource._meta, "include_resource_uri", True)
            and "resource_uri" in data
        ):
        ...
        fk_bundle = fk_resource.build_bundle(
            data=data, obj=obj, request=request
        )
        ...
        unique_keys = {
            ...
            if k == "pk"
            or (hasattr(fk_resource, k) and getattr(fk_resource, k).unique)
        }
...
class ToOneField(RelatedField):
    ...
    def contribute_to_class(self, cls, name):
        ...
        if not self.related_name and isinstance(
            self.attribute, six.string_types
        ):
            related_field = getattr(
                self._resource._meta.object_class, self.attribute, None
            )
