        ...
        if getattr(new_class._meta, "include_resource_uri", True):
            if "resource_uri" not in new_class.base_fields:
                new_class.base_fields["resource_uri"] = fields.CharField(
                    readonly=True, verbose_name="resource uri"
                )
        elif (
            "resource_uri" in new_class.base_fields
            and "resource_uri" not in attrs
        ):
...
class Resource(six.with_metaclass(DeclarativeMetaclass)):
    ...
    def _handle_500(self, request, exception):
        ...
        if six.PY2:
            the_trace = [six.text_type(line, "utf-8") for line in the_trace]
        the_trace = "\n".join(the_trace)

        response_class = self.get_response_class_for_exception(
            request, exception
        )
        ...
        if response_class.status_code >= 500:
            ...
            log.error(
                ...
                exc_info=True,
                extra={
                    "status_code": response_class.status_code,
                    "request": request,
                },
            )
    ...
    def deserialize(self, request, data, format="application/json"):
        ...
        deserialized = self._meta.serializer.deserialize(
            data, format=request.META.get("CONTENT_TYPE", format)
        )
        return deserialized
    ...
    def authorized_read_list(self, object_list, bundle):
        ...
        """
        try:
            auth_result = self._meta.authorization.read_list(
                object_list, bundle
            )
    ...
    def authorized_read_detail(self, object_list, bundle):
        ...
        """
        try:
            auth_result = self._meta.authorization.read_detail(
                object_list, bundle
            )
            if auth_result is not True:
                raise Unauthorized()
    ...
    def authorized_create_list(self, object_list, bundle):
        ...
        """
        try:
            auth_result = self._meta.authorization.create_list(
                object_list, bundle
            )
        except Unauthorized as e:
            self.unauthorized_result(e)
    ...
    def authorized_create_detail(self, object_list, bundle):
        ...
        """
        try:
            auth_result = self._meta.authorization.create_detail(
                object_list, bundle
            )
            if auth_result is not True:
                raise Unauthorized()
    ...
    def authorized_update_list(self, object_list, bundle):
        ...
        """
        try:
            auth_result = self._meta.authorization.update_list(
                object_list, bundle
            )
        except Unauthorized as e:
            self.unauthorized_result(e)
    ...
    def authorized_delete_list(self, object_list, bundle):
        ...
        """
        try:
            auth_result = self._meta.authorization.delete_list(
                object_list, bundle
            )
        except Unauthorized as e:
            self.unauthorized_result(e)
    ...
    def authorized_delete_detail(self, object_list, bundle):
        ...
        """
        try:
            auth_result = self._meta.authorization.delete_detail(
                object_list, bundle
            )
            if not auth_result:
                raise Unauthorized()
    ...
    def get_via_uri(self, uri, request=None):
        ...
        if end_of_resource_name == -1:
            raise NotFound(
                "An incorrect URL was provided '%s' for the '%s' resource."
        ...
        found_at = chomped_uri.rfind(
            self._meta.resource_name, 0, end_of_resource_name
        )
    ...
    def full_dehydrate(self, bundle, for_list=False):
        ...
        for field_name, field_object in self.fields.items():
            ...
            if callable(field_use_in):
                ...
                if field_use_in not in [
                    "all",
                    "list" if for_list else "detail",
                ]:
                    continue
                    ...
                    if not field_object.is_related:
                        ...
                        if value is not None:
                            ...
                            try:
                                ...
                                bundle.related_objects_to_save[
                                    field_object.attribute
                                ] = value.obj
                        # not required, not sent
                        elif (
                            field_object.blank
                            and field_name not in bundle.data
                        ):
                            continue
    ...
    def put_list(self, request, **kwargs):
        ...
        if self._meta.collection_name not in deserialized:
            raise BadRequest(
                "Invalid data sent: missing '%s'" % self._meta.collection_name
            )
        ...
        if not self._meta.always_return_data:
            ...
            to_be_serialized = {
                self._meta.collection_name: [
                    self.full_dehydrate(b, for_list=True) for b in bundles_seen
                ]
            }
            to_be_serialized = self.alter_list_data_to_serialize(
                request, to_be_serialized
        ...
        for data in deserialized[collection_name]:
            ...
            if "resource_uri" in data:
                ...
                try:
                    ...
                    bundle = self.alter_detail_data_to_serialize(
                        request, bundle
                    )
    ...
    def update_in_place(self, request, original_bundle, new_data):
        ...
        kwargs = {
            self._meta.detail_uri_name: self.get_bundle_detail_data(
                original_bundle
            ),
            "request": request,
        }
    ...
    def get_multiple(self, request, **kwargs):
        ...
        if queryset is not None:
            ...
            for obj in queryset:
                objects_dict[
                    str(getattr(obj, self._meta.detail_uri_name))
                ] = obj
            ...
            for identifier in obj_identifiers:
                if identifier in objects_dict:
                    bundle = self.build_bundle(
                        obj=objects_dict[identifier], request=request
                    )
                    bundle = self.full_dehydrate(bundle, for_list=True)
...
class ModelDeclarativeMetaclass(DeclarativeMetaclass):
    def __new__(cls, name, bases, attrs):
        ...
        if (
            meta
            and not hasattr(meta, "queryset")
            and not hasattr(meta, "object_class")
        ):
        ...
        if include_fields is None:
            if meta and meta.object_class:
                include_fields = [
                    f.name for f in meta.object_class._meta.fields
                ]
            else:
                include_fields = []
        ...
        for field_name in field_names:
            ...
            if field_name in new_class.declared_fields:
                continue
            if (
                specified_fields is not None
                and field_name not in include_fields
            ):
        ...
        if getattr(new_class._meta, "include_absolute_url", True):
            ...
            "absolute_url" in new_class.base_fields
            and "absolute_url" not in attrs
        ):
...
class BaseModelResource(Resource):
    ...
    def build_filters(self, filters=None, ignore_bad_filters=False):
        ...
        for filter_expr, value in filters.items():
            ...
            try:
                ...
                django_field = self._meta.object_class._meta.get_field(
                    django_field_name
                )
                ...
                raise InvalidFilterError(
                    "The '%s' field is not a valid field name" % field_name
                )
    ...
    def obj_get(self, bundle, **kwargs):
        ...
        applicable_filters = self.build_filters(
            filters=kwargs, ignore_bad_filters=True
        )
    ...
    def lookup_kwargs_with_identifiers(self, bundle, kwargs):
        ...
        if self._meta.detail_uri_name in kwargs:
            lookup_kwargs[self._meta.detail_uri_name] = kwargs.pop(
                self._meta.detail_uri_name
            )
        ...
        for identifier in kwargs:
            ...
            if (
                field_object.readonly
                or field_object.is_related
    ...
    def obj_update(self, bundle, skip_errors=False, **kwargs):
        ...
        if bundle_detail_data is None or (
            arg_detail_data is not None
            and str(bundle_detail_data) != str(arg_detail_data)
        ):
            try:
    ...
    def obj_delete_list(self, bundle, **kwargs):
        ...
        deletable_objects = self.authorized_delete_list(
            objects_to_delete, bundle
        )
    ...
    def obj_delete_list_for_update(self, bundle, **kwargs):
        ...
        """
        objects_to_delete = self.obj_get_list(bundle=bundle, **kwargs)
        deletable_objects = self.authorized_update_list(
            objects_to_delete, bundle
        )
...
class ModelResource(
    six.with_metaclass(ModelDeclarativeMetaclass, BaseModelResource)
):
    pass
