...
class Authentication(object):
    ...
    def get_authorization_data(self, request):
        ...
        try:
            ...
            raise ValueError(
                "Authorization header must have a space separating auth_type and data."
            )
...
class SessionAuthentication(Authentication):
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        if not constant_time_compare(
            unsalt_token(request_csrf_token), unsalt_token(csrf_token)
        ):
            return False
...
class DigestAuthentication(Authentication):
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        if not python_digest.validate_nonce(
            digest_response.nonce, settings.SECRET_KEY
        ):
            return self._unauthorized()
...
class OAuthAuthentication(Authentication):
    ...
    def validate_token(self, request, consumer, token):
        (
            oauth_server,
            oauth_request,
        ) = oauth_provider.utils.initialize_server_request(request)
