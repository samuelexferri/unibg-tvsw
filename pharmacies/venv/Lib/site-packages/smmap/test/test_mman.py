...
class TestMMan(TestBase):
    def test_cursor(self):
        with FileCreator(self.k_window_test_size, "cursor_test") as fc:
            ...
            assert (
                ci.size() == 0
            )  # this is cached, so we can query it in invalid state
    ...
    def test_memman_operation(self):
        ...
        with FileCreator(
            self.k_window_test_size, "manager_operation_test"
        ) as fc:
            with open(fc.path, "rb") as fp:
                data = fp.read()
            fd = os.open(fc.path, os.O_RDONLY)
            try:
                ...
                for mtype, args in (
                    ...
                    for item in (fc.path, fd):
                        ...
                        assert (
                            rr.ofs_begin() == 0
                        )  # it was aligned and expanded
                        if man.window_size():
                            # but isn't larger than the max window (aligned)
                            assert rr.size() == align_to_mmap(
                                man.window_size(), True
                            )
                        else:
                        ...
                        assert (
                            c.buffer()[:]
                            == data[
                                base_offset : base_offset + (size or c.size())
                            ]
                        )
                        ...
                        if man.window_size():
                            ...
                            assert (
                                c.region() is not rr
                            )  # old region is still available, but has not curser ref anymore
                            assert (
                                rr.client_count() == 1
                        ...
                        while num_random_accesses:
                            ...
                            assert (
                                c.buffer()[:]
                                == data[base_offset : base_offset + csize]
                            )
                            memory_read += csize
                        ...
                        elapsed = max(
                            time() - st, 0.001
                        )  # prevent zero divison errors on windows
                        mb = float(1000 * 1000)
                        print(
