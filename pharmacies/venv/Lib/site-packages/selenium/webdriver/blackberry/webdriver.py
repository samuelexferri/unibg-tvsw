    ...
    def __init__(
        ...
        if bb_tools_dir is not None:
            if os.path.isdir(bb_tools_dir):
                ...
                raise WebDriverException(
                    "Invalid blackberry tools location, must be a directory: {}".format(
                        bb_tools_dir
                    )
                )
        else:
        ...
        try:
            ...
            if returncode == 0:
                ...
                WebDriverWait(None, LOAD_TIMEOUT).until(
                    lambda x: subprocess.check_output(is_running_args).find(
                        "result::true"
                    ),
