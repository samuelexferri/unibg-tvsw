...
class RemoteConnection(object):
    ...
    def get_remote_connection_headers(cls, parsed_url, keep_alive=False):
        ...
        headers = {
            ...
            "User-Agent": "selenium/{} (python {})".format(
                __version__, system
            ),
        }
        ...
        if parsed_url.username:
            base64string = base64.b64encode(
                "{0.username}:{0.password}".format(parsed_url).encode()
            )
            headers.update(
    ...
    def __init__(self, remote_server_addr, keep_alive=False, resolve_ip=True):
        ...
        if parsed_url.hostname and resolve_ip:
            ...
            elif port and not common_utils.is_connectable(
                port, parsed_url.hostname
            ):
                ip = None
                LOGGER.info(
                    "Could not connect to port {} on host "
                    "{}".format(port, parsed_url.hostname)
                )
            else:
                ip = common_utils.find_connectable_ip(
                    parsed_url.hostname, port=port
                )
            if ip:
                netloc = ip
                if parsed_url.port:
                    netloc = common_utils.join_host_port(
                        netloc, parsed_url.port
                    )
                if parsed_url.username:
                ...
                LOGGER.info(
                    "Could not get IP address for host: %s"
                    % parsed_url.hostname
                )
        ...
        self._commands = {
            ...
            Command.SET_ALERT_CREDENTIALS: (
                "POST",
                "/session/$sessionId/alert/credentials",
            ),
            ...
            Command.GET_APP_CACHE: (
                "GET",
                "/session/$sessionId/application_cache",
            ),
