...
class FirefoxProfile(object):
    ...
    def set_proxy(self, proxy):
        ...
        if proxy.proxy_type is ProxyType.MANUAL:
            ...
            self.set_preference(
                "network.proxy.autoconfig_url", proxy.proxy_autoconfig_url
            )
    ...
    def _set_manual_proxy_preference(self, key, setting):
        ...
        if len(host_details) > 1:
            self.set_preference(
                "network.proxy.%s_port" % key, int(host_details[1])
            )
            ...
            for name in compressed_file.namelist():
                if name.endswith("/"):
                    ...
                    if not os.path.isdir(
                        os.path.dirname(os.path.join(tmpdir, name))
                    ):
                        os.makedirs(
                            os.path.dirname(os.path.join(tmpdir, name))
                        )
