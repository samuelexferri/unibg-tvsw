...
class AppsOrderCR(LegacyCR):
    ...
    def resolve_collisions(self, namespace):
        assert (
            self.APP_PRIORITIES is not None
    ...
    def _sort_models_depending_on_priorities(
        self, models
...
class AppNameCR(PathBasedCR):
    ...
    def transform_import(self, module_path):
        assert (
            self.MODIFICATION_STRING is not None
...
class CollisionResolvingRunner:
    ...
    @classmethod
    def _get_dictionary_of_names(
        cls, models_to_import
    ):  # type: (Dict[str, List[str]]) -> (Dict[str, str])
        from django.conf import settings
        ...
        result = collision_resolver_class().resolve_collisions(
            models_to_import
        )
    ...
    def _assert_is_collision_resolver_result_correct(cls, result):
        ...
        for key, value in result.items():
            ...
            assert isinstance(value, str), (
                "value in collision resolver result should be str not %s"
                % value
            )
    ...
    def _assert_is_collision_resolver_class_correct(
        ...
        assert (
            len(getargspec(collision_resolver_class.resolve_collisions).args)
            == 2
