...
try:
    ...
    raise ImportError(
        "Using an encrypted field requires the Keyczar module. "
...
class BaseEncryptedField(models.Field):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not getattr(settings, "ENCRYPTED_FIELD_KEYS_DIR", None):
            raise ImproperlyConfigured(
                "You must set the settings.ENCRYPTED_FIELD_KEYS_DIR "
    ...
    def calculate_crypt_max_length(self, unencrypted_length):
        ...
        return len(self.prefix) + len(
            self.crypt.Encrypt("x" * unencrypted_length)
        )
    ...
    def get_crypt_class(self):
        ...
        """
        crypt_type = getattr(
            settings, "ENCRYPTED_FIELD_MODE", "DECRYPT_AND_ENCRYPT"
        )
    ...
    def get_db_prep_value(self, value, connection, prepared=False):
        if value and not value.startswith(self.prefix):
            ...
            if max_length and len(value) > max_length:
                warnings.warn(
                    "Truncating field %s from %d to %d bytes"
                    % (self.name, len(value), max_length),
                    EncryptionWarning,
                )
