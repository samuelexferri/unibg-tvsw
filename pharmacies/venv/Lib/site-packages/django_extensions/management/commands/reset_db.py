...
from django_extensions.settings import (
    SQLITE_ENGINES,
    POSTGRESQL_ENGINES,
    MYSQL_ENGINES,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        database_name = (
            options["dbname"] or dbinfo.get("NAME") or database_name
        )
        if database_name == "":
            raise CommandError(
...
Type 'yes' to continue, or 'no' to cancel: """
        ...
        if engine in SQLITE_ENGINES:
            ...
            utf8_support = (
                "" if options["no_utf8_support"] else "CHARACTER SET utf8"
            )
            ...
            if settings.DEFAULT_TABLESPACE:
                create_query += (
                    " TABLESPACE = %s;" % settings.DEFAULT_TABLESPACE
                )
            else:
                create_query += ";"
