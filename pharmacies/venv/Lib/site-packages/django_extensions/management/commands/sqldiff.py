...
class SQLDiff(object):
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if reverse_type == "GeometryField":
            ...
            reverse_type = (
                "django.contrib.gis.db.models.fields.%s" % reverse_type
            )
    ...
    def find_unique_missing_in_db(
        ...
        for field in all_local_fields(meta):
            ...
            if field.unique and meta.managed:
                ...
                if not db_field_unique and table_constraints:
                    db_field_unique = any(
                        constraint["unique"]
                        for contraint_name, constraint in six.iteritems(
                            table_constraints
                        )
        ...
        db_unique_columns = normalize_together(
            [
                v["columns"]
                for v in six.itervalues(table_constraints)
    ...
    def find_unique_missing_in_model(
        ...
        fields = dict(
            [(field.column, field) for field in all_local_fields(meta)]
        )
        unique_together = self.expand_together(meta.unique_together, meta)
        ...
        for constraint_name, constraint in six.iteritems(table_constraints):
            ...
            self.add_difference(
                "unique-missing-in-model", table_name, constraint_name
            )
        ...
        for field_name, field in six.iteritems(fieldmap):
            if field_name not in db_fields:
                ...
                if self.options["include_defaults"] and field.has_default():
                    field_output.append(
                        "DEFAULT %s"
                        % field.get_prep_value(field.get_default())
                    )
                if not field.null:
    ...
    def find_field_parameter_differ(
        ...
        for field in all_local_fields(meta):
            ...
            if not self.strip_parameters(model_type) == self.strip_parameters(
                db_type
            ):
                continue
...
class MySQLDiff(SQLDiff):
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        db_type = super(MySQLDiff, self).get_field_db_type(
            description, field, table_name
        )
        if not db_type:
            return
        if field:
            ...
            if (
                db_type == "integer"
                and description[1] == FIELD_TYPE.TINY
                and description[2] == 1
            ):
                db_type = "bool"
            if (
...
class SqliteSQLDiff(SQLDiff):
    ...
    def find_unique_missing_in_db(
        ...
        super(SqliteSQLDiff, self).find_unique_missing_in_db(
            meta,
            table_indexes,
            table_constraints,
            table_name,
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        db_type = super(SqliteSQLDiff, self).get_field_db_type(
            description, field, table_name
        )
        if not db_type:
            return None
...
class PostgresqlSQLDiff(SQLDiff):
    ...
    def get_data_type_arrayfield(self, base_field):
        return {
            ...
            "kwargs": {"base_field": self.get_field_class(base_field)(),},
        }

    def get_data_types_reverse_override(self):
        return {
            1042: "CharField",
            1000: lambda: self.get_data_type_arrayfield(
                base_field="BooleanField"
            ),
            1001: lambda: self.get_data_type_arrayfield(
                base_field="BinaryField"
            ),
            1002: lambda: self.get_data_type_arrayfield(
                base_field="CharField"
            ),
            1005: lambda: self.get_data_type_arrayfield(
                base_field="IntegerField"
            ),
            1006: lambda: self.get_data_type_arrayfield(
                base_field="IntegerField"
            ),
            1007: lambda: self.get_data_type_arrayfield(
                base_field="IntegerField"
            ),
            1009: lambda: self.get_data_type_arrayfield(
                base_field="CharField"
            ),
            1014: lambda: self.get_data_type_arrayfield(
                base_field="CharField"
            ),
            1015: lambda: self.get_data_type_arrayfield(
                base_field="CharField"
            ),
            1016: lambda: self.get_data_type_arrayfield(
                base_field="BigIntegerField"
            ),
            1017: lambda: self.get_data_type_arrayfield(
                base_field="FloatField"
            ),
            1021: lambda: self.get_data_type_arrayfield(
                base_field="FloatField"
            ),
            1022: lambda: self.get_data_type_arrayfield(
                base_field="FloatField"
            ),
            1115: lambda: self.get_data_type_arrayfield(
                base_field="DateTimeField"
            ),
            1185: lambda: self.get_data_type_arrayfield(
                base_field="DateTimeField"
            ),
            1231: lambda: self.get_data_type_arrayfield(
                base_field="DecimalField"
            ),
            # {'name': 'django.contrib.postgres.fields.ArrayField', 'kwargs': {'base_field': 'IntegerField'}},
            1186: lambda: self.get_data_type_arrayfield(
                base_field="DurationField"
            ),
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        db_type = super(PostgresqlSQLDiff, self).get_field_db_type(
            description, field, table_name
        )
        if not db_type:
            return
...
Edit your settings file and change DATABASE_ENGINE to something like 'postgresql' or 'mysql'."""
        ...
        if migrate_for_tests:
            ...
            call_command(
                "migrate", *app_labels, no_input=True, run_syncdb=True
            )
