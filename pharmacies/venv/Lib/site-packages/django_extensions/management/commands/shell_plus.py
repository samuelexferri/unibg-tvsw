    ...
    def get_notebook(self, options):
        ...
        def install_kernel_spec(app, display_name, ipython_arguments):
            ...
            try_spec_names = getattr(
                ...
                ["python3" if PY3 else "python2", "python",],
            )
            ...
            manage_py_dir, manage_py = os.path.split(
                os.path.realpath(sys.argv[0])
            )

            if (
                manage_py == "manage.py"
                and os.path.isdir(manage_py_dir)
                and manage_py_dir != os.getcwd()
            ):
                ...
                ks.env["PYTHONPATH"] = os.pathsep.join(
                    filter(None, pythonpath)
                )
        ...
        with monkey_patch_cursordebugwrapper(
            ...
            if use_kernel:
                ...
                for shell_name, func in shells:
                    if verbosity > 2:
                        print(
                            self.style.NOTICE("Trying shell: %s" % shell_name)
                        )
                    shell = func(options)
                    if callable(shell):
                        if verbosity > 1:
            ...
            if not callable(shell):
                ...
                print(
                    self.style.ERROR(
                        "Could not load %s interactive Python environment."
                        % shell_name
                    )
                )
                return
