...
SEARCH_FIELD_NAMES = getattr(settings, "SEARCH_FIELD_NAMES", ("name", "slug",))

DATE_HIERARCHY_NAMES = getattr(
    settings,
    "DATE_HIERARCHY_NAMES",
    ("joined_at", "updated_at", "created_at",),
)

PREPOPULATED_FIELD_NAMES = getattr(
    settings, "PREPOPULATED_FIELD_NAMES", ("slug=name",)
)
    ...
    def _unicode_generator(self):
        ...
        for admin_model in self:
            yield PRINT_ADMIN_CLASS % dict(
                name=admin_model.name, class_=admin_model,
            )
...
class AdminModel(UnicodeMixin):
    ...
    def _yield_string(self, key, value, converter=repr):
        return PRINT_ADMIN_PROPERTY % dict(key=key, value=converter(value),)
