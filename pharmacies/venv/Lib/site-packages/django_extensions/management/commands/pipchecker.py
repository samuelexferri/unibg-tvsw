...
class Command(BaseCommand):
        ...
        for name, req in list(self.reqs.items()):
            if req["url"]:
                ...
                if not available_version:
                    ...
                    self.options["show_newer"]
                    and dist_version > available_version
                ):
                    msg = self.style.INFO(
                ...
                pkg_info = name
            self.stdout.write(
                "{pkg_info:40} {msg}".format(pkg_info=pkg_info, msg=msg)
            )
            del self.reqs[name]
    ...
    def check_github(self):
        ...
        for name, req in list(self.reqs.items()):
            ...
            if (
                "message" in test_auth
                and test_auth["message"] == "Bad credentials"
            ):
                self.stdout.write(
                    self.style.ERROR(
