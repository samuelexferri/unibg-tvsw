...
class Command(BaseCommand):
    ...
    def show_dsn(self, router, options):
        ...
        if engine == "mysql":
            ...
            dsn.append(
                self.style.ERROR("Unknown database, can" "t generate DSN")
            )

        if not quiet:
            sys.stdout.write(
                self.style.SQL_TABLE(
                    "DSN for router '%s' with engine '%s':\n"
                    % (router, engine)
                )
            )
    ...
    def _postgresql(
        self, dbhost, dbport, dbname, dbuser, dbpass, dsn_style=None
    ):  # noqa
        ...
        if dsn_style == "all" or dsn_style == "uri":
            ...
            dsn.append(
                dsnstr.format(
                    host="{host}:{port}".format(host=dbhost, port=dbport)
                    if dbport
                    else dbhost,  # noqa
                    name=dbname,
                    user=dbuser,
        ...
        if dsn_style == "all" or dsn_style == "pgpass":
            dsn.append(
                ":".join(
                    map(
                        str,
                        filter(None, [dbhost, dbport, dbname, dbuser, dbpass]),
                    )
                )
            )
