...
class BadCustomDirectoryException(Exception):
    def __init__(self, value):
        self.message = (
            value
            + " If --dir-policy is custom than you must set correct directory in "
...
class Command(EmailNotificationCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options["noscripts"]:
            subdirs.append(
                getattr(settings, "RUNSCRIPT_SCRIPT_DIR", "scripts")
            )
        ...
        def my_import(parent_package, module_name):
            ...
            try:
                ...
                try:
                    ...
                    module_file = (
                        os.path.join(
                            settings.BASE_DIR, *full_module_path.split(".")
                        )
                        + ".py"
                    )
        ...
        for script in scripts:
            ...
            if not modules:
                if verbosity > 0 and not silent:
                    print(
                        ERROR(
                            "No (valid) module for script '%s' found" % script
                        )
                    )
                    if verbosity < 2:
                        print(
