...
class Command(BaseCommand):
    help = (
        """DEBUG only: give all users a new email based on their account data ("%s" by default). Possible parameters are: username, first_name, last_name"""
        % (DEFAULT_FAKE_EMAIL,)
    )
    ...
    def handle(self, *args, **options):
        ...
        if exclude_groups:
            ...
            if groups:
                ...
                raise CommandError(
                    "No groups matches filter: %s" % exclude_groups
                )
        if include_groups:
            ...
            if groups:
                ...
                raise CommandError(
                    "No groups matches filter: %s" % include_groups
                )
        if exclude_regexp:
