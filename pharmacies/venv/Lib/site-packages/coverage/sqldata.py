...
class CoverageData(SimpleReprMixin):
    ...
    * **file tracer names**: the module names of the file tracer plugins that
    ...
    def __nonzero__(self):
        if get_thread_id() not in self._dbs and not os.path.exists(
            self._filename
        ):
            return False
        try:
    ...
    def dumps(self):
        ...
        if self._debug.should("dataio"):
            self._debug.write(
                "Dumping data from data file {!r}".format(self._filename)
            )
        with self._connect() as con:
    ...
    def loads(self, data):
        ...
        if self._debug.should("dataio"):
            self._debug.write(
                "Loading data into data file {!r}".format(self._filename)
            )
        if data[:1] != b"z":
            raise CoverageException(
    ...
    def add_file_tracers(self, file_tracers):
        ...
        if self._debug.should("dataop"):
            self._debug.write(
                "Adding file tracers: %d files" % (len(file_tracers),)
            )
        if not file_tracers:
            return
        self._start_using()
    ...
    def update(self, other_data, aliases=None):
        ...
        with self._connect() as conn:
            ...
            # dictionary.
            this_tracers = {
                path: "" for path, in conn.execute("select path from file")
            }
            this_tracers.update(
                {
    ...
    def measured_contexts(self):
        ...
        with self._connect() as con:
            contexts = set(
                row[0]
                for row in con.execute("select distinct(context) from context")
            )
        return contexts
