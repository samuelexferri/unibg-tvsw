...
class Coverage(object):
    ...
    def _post_init(self):
        ...
        if self.config._crash and self.config._crash in short_stack(limit=4):
            raise Exception(
                "Crashing because called by {}".format(self.config._crash)
            )
    ...
    def xml_report(
        ...
        with override_config(
            ...
            return render_report(
                self.config.xml_output, XmlReporter(self), morfs
            )
    ...
    def json_report(
        ...
        with override_config(
            ...
            return render_report(
                self.config.json_output, JsonReporter(self), morfs
            )
    ...
    def sys_info(self):
        ...
        info = [
            ...
            (
                "tracer",
                self._collector.tracer_name() if self._collector else "-none-",
            ),
            ...
            (
                ...
                plugin_info(self._plugins.context_switchers),
            ),
...
if int(os.environ.get("COVERAGE_DEBUG_CALLS", 0)):  # pragma: debugging
    ...
    Coverage = decorate_methods(
        show_calls(show_args=True), butnot=["get_data"]
    )(Coverage)
