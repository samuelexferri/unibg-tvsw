...
def data_filename(fname, pkgdir=""):
    ...
    raise CoverageException(
        "Couldn't find static file %r from %r, tried: %r"
        % (fname, os.getcwd(), tried)
    )
...
class HtmlDataGeneration(object):
    ...
    def data_for_file(self, fr, analysis):
        ...
        if self.config.show_contexts:
            contexts_by_lineno = analysis.data.contexts_by_lineno(
                analysis.filename
            )
        ...
        for lineno, tokens in enumerate(fr.source_token_lines(), start=1):
            ...
            if category and self.config.show_contexts:
                contexts = sorted(
                    c or self.EMPTY for c in contexts_by_lineno[lineno]
                )
...
class HtmlReporter(object):
    ...
    def __init__(self, cov):
        ...
        self.source_tmpl = Templite(
            self.pyfile_html_source, self.template_globals
        )
    ...
    def index_file(self):
        ...
        html = index_tmpl.render(
            {"files": self.file_summaries, "totals": self.totals,}
        )
