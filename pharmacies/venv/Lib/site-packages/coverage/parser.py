...
class PythonParser(object):
    ...
    def missing_arc_description(self, start, end, executed_arcs=None):
        ...
        for fragment_pair in fragment_pairs:
            ...
            if smsg is not None:
                msg += ", because {smsg}".format(
                    smsg=smsg.format(lineno=actual_start)
                )
...
new_contract(
    "ArcStarts", lambda seq: all(isinstance(x, ArcStart) for x in seq)
)
...
class AstArcAnalyzer(object):
    ...
    def analyze(self):
        ...
        for node in ast.walk(self.root_node):
            ...
            code_object_handler = getattr(
                self, "_code_object__" + node_name, None
            )
    ...
    def find_non_missing_node(self, node):
        ...
        missing_fn = getattr(
            self, "_missing__" + node.__class__.__name__, None
        )
        if missing_fn:
        ...
        for block in self.nearest_blocks():
            if isinstance(block, TryBlock):
                if block.handler_start is not None:
                    for xit in exits:
                        self.add_arc(
                            xit.lineno, block.handler_start, xit.cause
                        )
                    break
    ...
    def _handle__Try(self, node):
        ...
        if node.handlers:
            ...
            for handler_node in node.handlers:
                ...
                from_cause = (
                    "the exception caught by line {lineno} didn't happen"
                )
        ...
        if node.finalbody:
            ...
            final_exits = self.add_body_arcs(
            ...
            if try_block.raise_from:
                self.process_raise_exits(
                    self._combine_finally_starts(
                        try_block.raise_from, final_exits
                    )
                )
    ...
    def _handle__TryFinally(self, node):
        ...
        first = node.body[0]
        if (
            first.__class__.__name__ == "TryExcept"
    ...
    def _code_object__Module(self, node):
        ...
        if node.body:
            ...
            for xit in exits:
                self.add_arc(
                    xit.lineno, -start, xit.cause, "didn't exit the module"
                )
        else:
            # Empty module.
    ...
    def _make_oneline_code_method(noun):  # pylint: disable=no-self-argument
        ...
        def _code_object__oneline_callable(self, node):
            ...
            self.add_arc(
                ...
                start,
                None,
                "didn't run the {} on line {}".format(noun, start),
            )
            self.add_arc(
                start,
                -start,
    ...
    _code_object__GeneratorExp = _make_oneline_code_method(
        "generator expression"
    )
    _code_object__DictComp = _make_oneline_code_method(
        "dictionary comprehension"
    )
    _code_object__SetComp = _make_oneline_code_method("set comprehension")
    if env.PY3:
        _code_object__ListComp = _make_oneline_code_method(
            "list comprehension"
        )
            ...
            if 0:
                print(
                    "{}# mro: {}".format(
                        indent,
                        ", ".join(
                            c.__name__ for c in node.__class__.__mro__[1:]
                        ),
                    )
                )
            next_indent = indent + "    "
