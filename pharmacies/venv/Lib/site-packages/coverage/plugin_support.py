...
class Plugins(object):
    ...
    def _add_plugin(self, plugin, specialized):
        ...
        if self.debug and self.debug.should("plugin"):
            self.debug.write(
                "Loaded plugin %r: %r" % (self.current_module, plugin)
            )
            labelled = LabelledDebug(
                "plugin %r" % (self.current_module,), self.debug
            )
...
class DebugPluginWrapper(CoveragePlugin):
    ...
    def find_executable_files(self, src_dir):
        ...
        self.debug.write(
            "find_executable_files(%r) --> %r" % (src_dir, executable_files)
        )
...
class DebugFileTracerWrapper(FileTracer):
    ...
    def line_number_range(self, frame):
        ...
        self.debug.write(
            "line_number_range(%s) --> %r" % (self._show_frame(frame), pair)
        )
        return pair
