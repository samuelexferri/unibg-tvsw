...
class PyTracer(object):
    ...
    def __repr__(self):
        return "<PyTracer at {}: {} lines in {} files>".format(
            id(self), sum(len(v) for v in self.data.values()), len(self.data),
        )
    ...
    def _trace(self, frame, event, arg_unused):
        ...
        if (
            self.stopped and sys.gettrace() == self._trace
    ...
    def stop(self):
        ...
        if (
            self.threading
            and self.thread.ident != self.threading.currentThread().ident
        ):
        ...
        if self.warn:
            ...
            dont_warn = (
                env.PYPY
                and env.PYPYVERSION >= (5, 4)
                and self.in_atexit
                and tf is None
            )
            if (
                not dont_warn
            ) and tf != self._trace:  # pylint: disable=comparison-with-callable
                self.warn(
