...
def fail_for_missing_callable(string, name):
    ...
    if isinstance(name, Undefined):
        try:
            ...
            msg = "%s (%s; did you forget to quote the callable name?)" % (
                msg,
                e,
            )
...
class Environment(object):
        ...
        try:
            for name in self.list_templates(extensions, filter_func):
                ...
                if py_compile:
                    ...
                    write_file(
                        filename + "c", py_header + marshal.dumps(c), "wb"
                    )
                    log_function(
                        'Byte-compiled "%s" as %s' % (name, filename + "c")
                    )
                else:
    ...
    def list_templates(self, extensions=None, filter_func=None):
        ...
        if extensions is not None:
            ...
            filter_func = (
                lambda x: "." in x and x.rsplit(".", 1)[1] in extensions
            )
        if filter_func is not None:
...
class Template(object):
    ...
    def debug_info(self):
        ...
        return [
            tuple(imap(int, x.split("="))) for x in self._debug_info.split("&")
        ]
