...
def has_safe_repr(value):
    ...
    if (
        type(value)
        in (bool, int, float, complex, range_type, Markup) + string_types
    ):
        return True
...
class CodeGenerator(NodeVisitor):
    ...
    def enter_frame(self, frame):
        ...
        for target, (action, param) in iteritems(frame.symbols.loads):
            if action == VAR_LOAD_PARAMETER:
                ...
                self.writeline(
                    "%s = %s(%r)" % (target, self.get_resolve_func(), param)
                )
    ...
    def pop_assign_tracking(self, frame):
        ...
        if public_names:
            if len(public_names) == 1:
                self.writeline(
                    "context.exported_vars.add(%r)" % public_names[0]
                )
            else:
                self.writeline(
    ...
    def visit_Template(self, node, frame=None):
        ...
        for import_ in node.find_all(nodes.ImportedName):
            if import_.importname not in self.import_aliases:
                ...
                if "." in imp:
                    ...
                    self.writeline(
                        "from %s import %s as %s" % (module, obj, alias)
                    )
                else:
        ...
        self.writeline(
            "%s(context, missing=missing%s):" % (self.func("root"), envenv),
            extra=1,
        )
        self.indent()
        self.write_commons()
        ...
        for name, block in iteritems(self.blocks):
            ...
            if "super" in undeclared:
                ...
                self.writeline(
                    "%s = context.super(%r, " "block_%s)" % (ref, name, name)
                )
        ...
        self.writeline(
            "blocks = {%s}"
            % ", ".join("%r: block_%s" % (x, x) for x in self.blocks),
            extra=1,
        )
    ...
    def visit_Block(self, node, frame):
        ...
        if (
            ...
            self.writeline(
                "yield from context.blocks[%r][0](%s)" % (node.name, context),
                node,
            )
        else:
    ...
    def visit_Extends(self, node, frame):
        ...
        if self.extends_so_far > 0:
            ...
            if not self.has_known_extends:
                ...
                self.indent()
            self.writeline(
                "raise TemplateRuntimeError(%r)" % "extended multiple times"
            )
    ...
    def visit_FromImport(self, node, frame):
        ...
        if var_names:
            if len(var_names) == 1:
                ...
                self.writeline(
                    ...
                    % ", ".join(
                        "%r: %s" % (name, frame.symbols.ref(name))
        ...
        if discarded_names:
            if len(discarded_names) == 1:
                self.writeline(
                    "context.exported_vars.discard(%r)" % discarded_names[0]
                )
            else:
                self.writeline(
    ...
    def visit_For(self, node, frame):
        ...
        if node.test:
            ...
            self.writeline(
                "%s(fiter):" % self.func(loop_filter_func), node.test
            )
            self.indent()
            self.enter_frame(test_frame)
            self.writeline(
                self.environment.is_async and "async for " or "for "
            )
            self.visit(node.target, loop_frame)
            self.write(" in ")
            self.write(
                self.environment.is_async and "auto_aiter(fiter)" or "fiter"
            )
            self.write(":")
            self.indent()
        ...
        if node.recursive:
            self.writeline(
                "%s(reciter, loop_render_func, depth=0):" % self.func("loop"),
                node,
            )
            self.indent()
        ...
        self.writeline(
            self.environment.is_async and "async for " or "for ", node
        )
        self.visit(node.target, loop_frame)
        if extended_loop:
            if self.environment.is_async:
    ...
    def visit_Output(self, node, frame):
        ...
        if self.environment.finalize:
            ...
            if getattr(func, "contextfunction", False) or getattr(
                func, "evalcontextfunction", False
            ):
        ...
        if len(body) < 3 or frame.buffer is not None:
            ...
            for item in body:
                if isinstance(item, list):
                    ...
                    if frame.eval_ctx.volatile:
                        self.write(
                            "(escape if context.eval_ctx.autoescape"
                            " else to_string)("
                        )
                    elif frame.eval_ctx.autoescape:
                        self.write("escape(")
                    ...
                    if self.environment.finalize is not None:
                        ...
                        if getattr(
                            self.environment.finalize, "contextfunction", False
                        ):
                            self.write("context, ")
                        close += 1
        ...
        # otherwise we create a format string as this is faster in that case
            ...
            for argument in arguments:
                ...
                if frame.eval_ctx.volatile:
                    self.write(
                        "(escape if context.eval_ctx.autoescape else"
                        " to_string)("
                    )
                    close += 1
                elif frame.eval_ctx.autoescape:
                ...
                if self.environment.finalize is not None:
                    ...
                    if getattr(
                        ...
                        self.environment.finalize, "evalcontextfunction", False
                    ):
                        self.write("context.eval_ctx, ")
                    elif getattr(
                        self.environment.finalize, "environmentfunction", False
                    ):
    ...
    def visit_ExtensionAttribute(self, node, frame):
        self.write(
            "environment.extensions[%r].%s" % (node.identifier, node.name)
        )
