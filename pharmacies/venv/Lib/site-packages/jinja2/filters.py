...
def make_attrgetter(environment, attribute, postprocess=None):
    ...
    if attribute is None:
        ...
        attribute = [
            int(x) if x.isdigit() else x for x in attribute.split(".")
        ]
    else:
        attribute = [attribute]
...
def _min_or_max(environment, value, func, case_sensitive, attribute):
    ...
    key_func = make_attrgetter(
        environment, attribute, ignore_case if not case_sensitive else None
    )
...
def do_truncate(env, s, length=255, killwords=False, end="...", leeway=None):
    ...
    assert length >= len(end), "expected length >= %s, got %s" % (
        len(end),
        length,
    )
