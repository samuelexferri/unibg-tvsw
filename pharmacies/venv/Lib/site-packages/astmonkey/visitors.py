...
class GraphNodeVisitor(ast.NodeVisitor):
    ...
    def _dot_node_label(self, node):
        ...
        return "ast.{0}({1})".format(
            node.__class__.__name__, ", ".join(fields_labels)
        )
    ...
    def _dot_node_kwargs(self, node):
        return {"shape": "box", "fontname": "Curier"}

    def _dot_edge(self, node):
        return pydot.Edge(
            str(node.parent),
            str(node),
            label=self._dot_edge_label(node),
...
class BaseSourceGeneratorNodeVisitor(ast.NodeVisitor):
    ...
    def correct_line_number(
        self, node, within_statement=True, use_line_continuation=True
    ):
        ...
        with self.indent(indent):
            self.add_missing_lines(
                node, within_statement, use_line_continuation
            )
    ...
    def signature_kwonlyargs(self, node, write_comma):
        if (
            not self._is_node_args_valid(node, "kwonlyargs")
            or len(node.kwonlyargs) == 0
        ):
            return
    ...
    def visit_BinOp(self, node):
        with self.inside(
            "(", ")", cond=isinstance(node.parent, (ast.BinOp, ast.Attribute))
        ):
    ...
    def visit_Compare(self, node):
        with self.inside(
            "(", ")", cond=(isinstance(node.parent, ast.Compare))
        ):
            self.visit(node.left)
    ...
    def visit_UnaryOp(self, node):
        with self.inside(
            "(", ")", cond=isinstance(node.parent, (ast.BinOp, ast.UnaryOp))
        ):
            ...
            with self.inside(
                ...
                ")",
                cond=(
                    not isinstance(node.operand, (ast.Name, ast.Num))
    ...
    def slice_step(self, node):
        if node.step is not None:
            self.write(":")
            if not (
                isinstance(node.step, ast.Name) and node.step.id == "None"
            ):
