...
def get_by_python_version(classes, python_version=sys.version_info):
    ...
    for cls in classes:
        if cls.__python_version__ <= python_version:
            if (
                not result
                or cls.__python_version__ > result.__python_version__
            ):
                result = cls
    if not result:
        raise NotImplementedError(
            "astmonkey does not support Python %s." % sys.version
        )
    return result
...
def check_version(from_inclusive=None, to_exclusive=None):
    if (not from_inclusive or sys.version_info >= from_inclusive) and (
        not to_exclusive or sys.version_info < to_exclusive
    ):
        return True
    return False
