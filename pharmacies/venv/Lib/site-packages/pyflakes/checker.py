...
def is_typing_overload(value, scope):
    def is_typing_overload_decorator(node):
        return (
            ...
            and isinstance(scope[node.id], ImportationFrom)
            and scope[node.id].fullName == "typing.overload"
        ) or (
    ...
    return (
        isinstance(value.source, ast.FunctionDef)
        and len(value.source.decorator_list) == 1
        and is_typing_overload_decorator(value.source.decorator_list[0])
    )
...
class Checker(object):
    ...
    BOOLOP = (
        ...
        CALL
    ) = REPR = ATTRIBUTE = SUBSCRIPT = STARRED = NAMECONSTANT = handleChildren
