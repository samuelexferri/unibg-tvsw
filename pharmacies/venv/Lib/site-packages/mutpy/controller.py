...
class MutationController(views.ViewNotifier):
    ...
    def run(self):
        self.notify_initialize(
            self.target_loader.names, self.test_loader.names
        )
        try:
            timer = utils.Timer()
    ...
    def mutate_module(self, target_module, to_mutate, total_duration):
        ...
        for mutations, mutant_ast in self.mutant_generator.mutate(
            ...
            if (
                ...
                and self.mutation_number != mutation_number
            ):
                self.score.inc_incompetent()
                continue
            self.notify_mutation(
                mutation_number, mutations, target_module, mutant_ast
            )
            mutant_module = self.create_mutant_module(
                target_module, mutant_ast
            )
            if mutant_module:
        ...
        for mutation_to_apply in mutations_to_apply:
            for available_mutation in available_mutations[:]:
                if (
                    ...
                    in available_mutation.node.children
                    or available_mutation.node
                    in mutation_to_apply.node.children
                    or (
                        not allow_same_operators
                        and mutation_to_apply.operator
                        == available_mutation.operator
                    )
                ):
        ...
        while mutations:
            ...
            while len(mutations_to_apply) < self.order and available_mutations:
                ...
                self.remove_bad_mutations(
                    mutations_to_apply, available_mutations
                )
...
class FirstOrderMutator:
    ...
    def mutate(
        self, target_ast, to_mutate=None, coverage_injector=None, module=None
    ):
...
class HighOrderMutator(FirstOrderMutator):
    ...
    def generate_all_mutations(
        self, coverage_injector, module, target_ast, to_mutate
    ):
        mutations = []
        for op in utils.sort_operators(self.operators):
