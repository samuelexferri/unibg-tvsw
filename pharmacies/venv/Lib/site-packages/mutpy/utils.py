...
class ModulesLoader:
    ...
    def load_module(self, name):
        (
            ...
            remainder_path,
            last_exception,
        ) = self._split_by_module_and_remainder(name)
    ...
    def _is_c_extension(module):
        if isinstance(
            getattr(module, "__loader__", None), ExtensionFileLoader
        ):
            return True
...
class ModuleInjector:
    ...
    def try_incject_class_or_function(
        self, imported_as, class_or_function, target
    ):
        if (
            self.safe_getattr(class_or_function, "__name__")
            in self.source.__dict__
        ):
    ...
    def try_inject_other(self, imported_as, target):
        if imported_as in self.source.__dict__ and not self.is_restricted(
            imported_as
        ):
...
def get_by_python_version(classes, python_version=sys.version_info):
    candidates = [
        cls for cls in classes if cls.__python_version__ <= python_version
    ]
    if not candidates:
    ...
    return max(
        ...
        key=lambda cls: cls.__python_version__,
    )
