...
class ViewNotifier:
    ...
    def __getattr__(self, name):
        if name.startswith(ViewNotifier.PREFIX):
            ...
            return lambda *args, **kwargs: self.notify_all_views(
                notify, *args, **kwargs
            )
        else:
...
class QuietTextView:
    ...
    def end(self, score, duration):
        self.level_print(
            "Mutation score {}: {}".format(
                self.time_format(duration),
                self.decorate(
                    "{:.1f}%".format(score.count()), "blue", attrs=["bold"]
                ),
            )
        )
...
class TextView(QuietTextView):
    ...
    def end(self, score, duration):
        ...
        if score.all_mutants:
            self.level_print(
                "killed: {} ({:.1f}%)".format(
                    score.killed_mutants,
                    100 * score.killed_mutants / score.all_mutants,
                ),
                2,
            )
            self.level_print(
                "survived: {} ({:.1f}%)".format(
                    score.survived_mutants,
                    100 * score.survived_mutants / score.all_mutants,
                ),
                2,
            )
            self.level_print(
                "incompetent: {} ({:.1f}%)".format(
                    score.incompetent_mutants,
                    100 * score.incompetent_mutants / score.all_mutants,
                ),
                2,
            )
            self.level_print(
                "timeout: {} ({:.1f}%)".format(
                    score.timeout_mutants,
                    100 * score.timeout_mutants / score.all_mutants,
                ),
                2,
            )
            if score.all_nodes:
                self.level_print(
                    "Coverage: {} of {} AST nodes ({:.1f}%)".format(
                        score.covered_nodes,
                        score.all_nodes,
    ...
    def passed(self, tests, number_of_tests):
        ...
        for test, target, time in tests:
            ...
            self.level_print(
                "{} {}".format(test_name, self.time_format(time)), 2
            )
    ...
    def original_tests_fail(self, result):
        ...
        for fail in result.failed:
            self.level_print(
                "fail in {} - {}".format(fail.name, fail.short_message), 2
            )
    ...
    def mutation(self, number, mutations, module, mutant):
        for mutation in mutations:
            self.level_print(
                "[#{:>4}] {:<3} {}: ".format(
                    number, mutation.operator.name(), module.__name__
                ),
                ended=False,
                level=2,
            )
    ...
    def cant_load(self, name, exception):
        self.level_print(
            self.decorate("Can't load module: ", "red", attrs=["bold"])
            + "{} ({}: {})".format(
                name, exception.__class__.__name__, exception
            )
        )
    ...
    def _print_diff(self, mutant_src, original_src):
        ...
        diff = [
            self.decorate(line, "blue") if line.startswith("- ") else line
            for line in diff
        ]
        diff = [
            self.decorate(line, "green") if line.startswith("+ ") else line
            for line in diff
        ]
        print(
            "\n{}\n".format("-" * 80)
    ...
    def survived(self, time, *args, **kwargs):
        self.level_print(
            self.time_format(time) + " " + self.decorate("survived", "red"),
            continuation=True,
        )

    def timeout(self, time, *args, **kwargs):
        self.level_print(
            self.time_format(time) + " " + self.decorate("timeout", "yellow"),
            continuation=True,
        )
    ...
    def incompetent(self, time, *args, **kwargs):
        self.level_print(
            self.time_format(time)
            + " "
            + self.decorate("incompetent", "cyan"),
            continuation=True,
        )
...
class AccReportView:
    ...
    def killed(
        self, time, killer, exception_traceback, tests_run, *args, **kwargs
    ):
        self.end_mutation(
            "killed",
            time=time,
            ...
            yaml.dump(
                {
                    "targets": self.target,
                    "tests": [
                        {"name": test.__name__, "target": target, "time": time}
                        for test, target, time in self.tests
                    ],
