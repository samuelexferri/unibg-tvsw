    ...
    def has_failed_before(self, nodeid):
        return (
            next(
                (
                    test
                    for test in self.mutation_test_result.failed
                    if test.name == nodeid
                ),
                None,
            )
    ...
    def has_been_skipped_before(self, nodeid):
        return (
            next(
                (
                    test
                    for test in self.mutation_test_result.skipped
                    if test.name == nodeid
                ),
                None,
            )
    ...
    def pytest_runtest_logreport(self, report):
        if report.skipped:
            ...
            if "TypeError" in report.longrepr.reprcrash.message:
                self.mutation_test_result.set_type_error(
                    TypeError(str(report.longrepr.reprcrash))
                )
            else:
                ...
                self.mutation_test_result.add_failed(
                    report.nodeid,
                    report.longrepr.reprcrash.message.splitlines()[0],
                    report.longreprtext,
                )
        elif (
            report.passed
            and report.when == "teardown"
            and not self.has_failed_before(report.nodeid)
...
class PytestMutpyCoveragePlugin:
    def __init__(self, coverage_injector):
        ...
        self.coverage_result = CoverageTestResult(
    ...
    def pytest_runtest_teardown(self, nextitem):
        self.coverage_result.stop_measure_coverage(
            PytestTest(self.current_test)
        )
        self.current_test = None
...
class PytestTestSuite(BaseTestSuite):
    ...
    def add_tests(self, test_module, target_test):
        if target_test:
            self.tests.add(
                "{0}::{1}".format(test_module.__file__, target_test)
            )
    ...
    def run(self):
        ...
        pytest.main(
            args=list(self.tests) + ["-x", "-p", "no:terminal"],
    ...
    def run_with_coverage(self, coverage_injector=None):
        mutpy_plugin = PytestMutpyCoveragePlugin(
            coverage_injector=coverage_injector
        )
        pytest.main(
            list(self.tests) + ["-p", "no:terminal"],
            plugins=list(default_plugins) + [mutpy_plugin],
        )
    ...
    def __iter__(self):
        ...
        pytest.main(
            args=list(self.tests) + ["--collect-only", "-p", "no:terminal"],
            plugins=list(default_plugins) + [mutpy_plugin],
        )
