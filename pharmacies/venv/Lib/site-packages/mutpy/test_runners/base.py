...
class CoverageTestResult:
    ...
    def stop_measure_coverage(self, test):
        self.test_covered_nodes[repr(test)] = (
            self.coverage_injector.covered_nodes.copy()
            | self.always_covered_nodes
        )
...
class BaseTestRunner:
    ...
    def __init__(
        self, test_loader, timeout_factor, stdout_manager, mutate_covered
    ):
    ...
    def run_tests_with_mutant(
        ...
        if coverage_result:
            self.mark_not_covered_tests_as_skip(
                mutations, coverage_result, suite
            )
        timer = utils.Timer()
    ...
    def run_mutation_test_runner(self, suite, total_duration):
        live_time = self.timeout_factor * (
            total_duration if total_duration > 1 else 1
        )
    ...
    def inject_coverage(self, target_ast, target_module):
        ...
        coverage_module = coverage_injector.inject(
            target_ast, target_module.__name__
        )
    ...
    def mark_not_covered_tests_as_skip(
        self, mutations, coverage_result, suite
    ):
