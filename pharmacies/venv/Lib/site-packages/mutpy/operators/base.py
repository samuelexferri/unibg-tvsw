...
def copy_node(mutate):
    def f(self, node):
        copied_node = copy.deepcopy(node, memo={id(node.parent): node.parent,})
...
class MutationOperator:
    def mutate(
        ...
        for new_node in self.visit(node):
            yield Mutation(
                operator=self.__class__,
                node=self.current_node,
                visitor=self.visitor,
            ), new_node

    def visit(self, node):
        if self.has_notmutate(node) or (
            self.coverage_injector
            and not self.coverage_injector.is_covered(node)
        ):
            return
        if (
            self.only_mutation
            and self.only_mutation.node != node
            and self.only_mutation.node not in node.children
        ):
            return
    ...
    def generic_visit(self, node):
        for field, old_value in ast.iter_fields(node):
            if isinstance(old_value, list):
                ...
                generator = self.generic_visit_real_node(
                    node, field, old_value
                )
            else:
                generator = []
    ...
    def fix_lineno(self, node):
        if (
            ...
            and hasattr(node.parent, "lineno")
        ):
    ...
    def long_name(cls):
        return " ".join(
            map(str.lower, (re.split("([A-Z][a-z]*)", cls.__name__)[1::2]))
        )
