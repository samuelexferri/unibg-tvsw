...
def build_parser():
    ...
    parser.add_argument(
        ...
        "-e",
        action="store_true",
        help="use experimental operators",
    )
    parser.add_argument(
        "--operator",
        "-o",
        type=str,
        nargs="+",
        help="use only selected operators",
        metavar="OPERATOR",
    )
    parser.add_argument(
        "--disable-operator",
        ...
        nargs="+",
        default=[],
        help="disable selected operators",
        metavar="OPERATOR",
    )
    parser.add_argument(
        "--list-operators",
    ...
    parser.add_argument(
        "--path", "-p", type=str, metavar="DIR", help="extend Python path"
    )
    parser.add_argument(
        "--percentage",
        type=int,
    ...
    parser.add_argument(
        "--coverage", action="store_true", help="mutate only covered code"
    )
    parser.add_argument(
        "--order", type=int, metavar="ORDER", default=1, help="mutation order"
    )
    parser.add_argument(
        "--hom-strategy",
        type=str,
        metavar="HOM_STRATEGY",
        help="HOM strategy",
    ...
    parser.add_argument(
        "--list-hom-strategies",
        action="store_true",
        help="list available HOM strategies",
    )
    parser.add_argument(
        "--mutation-number",
        type=int,
        metavar="MUTATION_NUMBER",
...
def build_mutator(cfg):
    ...
    if cfg.operator:
        operators_set |= {
            get_operator(name, name_to_operator) for name in cfg.operator
        }
    else:
        operators_set |= operators.standard_operators

    operators_set -= {
        get_operator(name, name_to_operator) for name in cfg.disable_operator
    }
    ...
    if cfg.order == 1:
        ...
        return controller.HighOrderMutator(
            operators_set, cfg.percentage, hom_strategy=hom_strategy
        )
...
def build_hom_strategy(cfg):
    if cfg.order < 1:
        ...
        sys.exit(-1)
    try:
        name_to_hom_strategy = {
            hom_strategy.name: hom_strategy
            for hom_strategy in controller.hom_strategies
        }
        return name_to_hom_strategy[cfg.hom_strategy](order=cfg.order)
    except KeyError:
        print(
            "Unsupported HOM strategy {}! Use --list-hom-strategies to show strategies.".format(
                cfg.hom_strategy
            )
        )
        sys.exit(-1)
...
def get_operator(name, name_to_operator):
    try:
        ...
        print(
            "Unsupported operator {}! Use -l to show all operators.".format(
                name
            )
        )
        sys.exit(-1)
...
def build_name_to_operator_map():
    result = {}
    for operator in (
        operators.standard_operators | operators.experimental_operators
    ):
