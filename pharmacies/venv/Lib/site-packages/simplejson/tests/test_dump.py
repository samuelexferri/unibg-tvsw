...
class TestDump(TestCase):
    ...
    def test_stringify_key(self):
        ...
        for k, expect in items:
            self.assertEqual(
                json.loads(json.dumps({k: expect})), {expect: expect}
            )
            self.assertEqual(
        ...
        for v in [{}, {"other": 1}, {b("derp"): 1, "herp": 2}]:
            for sort_keys in [False, True]:
                ...
                self.assertEqual(
                    json.loads(
                        json.dumps(
                            {"": v0}, skipkeys=True, sort_keys=sort_keys
                        )
                    ),
                    {"": v1},
                )
    ...
    def test_encode_truefalse(self):
        self.assertEqual(
            json.dumps({True: False, False: True}, sort_keys=True),
    ...
    def test_bytes_toplevel(self):
        ...
        self.assertEqual(
            json.dumps(b("\xa4"), encoding="iso-8859-1"), r'"\u00a4"'
        )
        self.assertEqual(
            json.dumps(b("\xa4"), encoding="iso-8859-15"), r'"\u20ac"'
        )
        if PY3:
            self.assertRaises(
                TypeError, json.dumps, b("\xe2\x82\xac"), encoding=None
            )
            self.assertRaises(TypeError, json.dumps, b("\xa4"), encoding=None)
            self.assertEqual(
                json.dumps(
                    b("\xa4"), encoding=None, default=decode_iso_8859_15
                ),
                r'"\u20ac"',
            )
        else:
            self.assertEqual(
                json.dumps(b("\xe2\x82\xac"), encoding=None), r'"\u20ac"'
            )
            self.assertRaises(
                UnicodeDecodeError, json.dumps, b("\xa4"), encoding=None
            )
            self.assertRaises(
                UnicodeDecodeError,
                json.dumps,
    ...
    def test_bytes_nested(self):
        ...
        if PY3:
            ...
            self.assertRaises(
                TypeError, json.dumps, [b("\xa4")], encoding=None
            )
            self.assertEqual(
                json.dumps(
                    [b("\xa4")], encoding=None, default=decode_iso_8859_15
    ...
    def test_bytes_key(self):
        self.assertEqual(
            json.dumps({b("\xe2\x82\xac"): 42}), r'{"\u20ac": 42}'
        )
        ...
        if PY3:
            self.assertRaises(
                TypeError, json.dumps, {b("\xe2\x82\xac"): 42}, encoding=None
            )
            self.assertRaises(
                TypeError, json.dumps, {b("\xa4"): 42}, encoding=None
            )
            self.assertRaises(
                TypeError,
                json.dumps,
            ...
            self.assertEqual(
                json.dumps({b("\xe2\x82\xac"): 42}, encoding=None),
                r'{"\u20ac": 42}',
            )
            self.assertRaises(
                UnicodeDecodeError, json.dumps, {b("\xa4"): 42}, encoding=None
            )
            self.assertRaises(
                UnicodeDecodeError,
                json.dumps,
