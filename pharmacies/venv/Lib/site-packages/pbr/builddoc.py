    ...
    def _get_source_dir(self):
        ...
        if "source_dir" in option_dict:
            source_dir = os.path.join(
                option_dict["source_dir"][1], api_doc_dir
            )
        else:
...
"""
    ...
    def run(self):
        ...
        if warn_opts or warn_env:
            msg = (
                "The autodoc and autodoc_tree features are deprecated in "
        ...
        tree_index = options.get_boolean_option(
            option_dict,
            "autodoc_tree_index_modules",
            "AUTODOC_TREE_INDEX_MODULES",
        )
        ...
        if not os.getenv("SPHINX_DEBUG"):
            ...
            if auto_index:
                self.generate_autoindex(
                    set(
                        option_dict.get("autodoc_exclude_modules", [None, ""])[
                            1
                        ].split()
                    )
                )
        ...
        if self.builders != ["html"]:
            log.warn(
                "[pbr] Sphinx 1.6 added native support for "
