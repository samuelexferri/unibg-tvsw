...
class _AST2To3(ast27.NodeTransformer):
    ...
    def visit_list(self, l):
        return [
            self.visit(e) if isinstance(e, (ast27.AST, list)) else e for e in l
        ]
    ...
    def visit_TryFinally(self, n):
        if len(n.body) == 1 and isinstance(n.body[0], ast27.TryExcept):
            ...
            return ast3.Try(
                self.visit(n.body), [], [], self.visit(n.finalbody)
            )
    ...
    def visit_ExceptHandler(self, n):
        ...
        return ast3.ExceptHandler(
            self.maybe_visit(n.type), name, self.visit(n.body)
        )
    ...
    def visit_Print(self, n):
        ...
        return ast3.Expr(
            ast3.Call(
                ast3.Name(
                    "print", ast3.Load(), lineno=n.lineno, col_offset=-1
                ),
                self.visit(n.values),
                keywords,
                lineno=n.lineno,
    ...
    def visit_Exec(self, n):
        ...
        if new_globals is None:
            new_globals = ast3.Name(
                "None", ast3.Load(), lineno=-1, col_offset=-1
            )
        new_locals = self.maybe_visit(n.locals)
        if new_locals is None:
            new_locals = ast3.Name(
                "None", ast3.Load(), lineno=-1, col_offset=-1
            )
    ...
    def visit_Num(self, n):
        ...
        if new.n < 0:
            ...
            new.n = -new.n
            return ast3.UnaryOp(
                op=ast3.USub(), operand=_copy_attributes(new, n)
            )
        else:
            return new
