    ...
    for key, value in params.items():
        ...
        try:
            if location == "path":
                ...
                if not seen_body:
                    data[key] = utils.validate_form_param(
...
def _build_http_request(
    session, url, method, headers=None, encoding=None, params=empty_params
):
    """
...
class HTTPTransport(BaseTransport):
    ...
    def __init__(
        ...
        if request_callback is not None or response_callback is not None:
            ...
            session.mount(
                "http://", CallbackAdapter(request_callback, response_callback)
            )
