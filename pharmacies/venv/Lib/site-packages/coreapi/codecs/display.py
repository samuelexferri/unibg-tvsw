...
def _to_plaintext(
    node, indent=0, base_url=None, colorize=False, extra_offset=None
):
    ...
    if isinstance(node, Document):
        ...
        head = colorize_document(
            "<%s %s>"
            % (node.title.strip() or "Document", json.dumps(node.url))
        )
    ...
    if isinstance(node, Error):
        ...
        head = colorize_error(
            "<Error: %s>" % node.title.strip() if node.title else "<Error>"
        )
        ...
        body = ",\n".join(
            [
                body_indent
                + _to_plaintext(
                    value, indent + 1, base_url=base_url, colorize=colorize
                )
                for value in node
            ]
        )
