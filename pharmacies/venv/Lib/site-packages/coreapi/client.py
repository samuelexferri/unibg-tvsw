...
def _validate_parameters(link, parameters):
    ...
    required = set([field.name for field in link.fields if field.required])
    optional = set([field.name for field in link.fields if not field.required])
...
class Client(itypes.Object):
    def __init__(
        self, decoders=None, transports=None, auth=None, session=None
    ):
        ...
        if format:
            ...
            if not decoders:
                ...
                if format in installed_codecs:
                    ...
                    raise ValueError(
                        "No decoder available with format='%s'" % format
                    )
    ...
    def action(
        ...
        return transport.transition(
            link, self.decoders, params=params, link_ancestors=link_ancestors
        )
