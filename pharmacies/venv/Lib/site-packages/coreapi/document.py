...
class Document(itypes.Dict):
    ...
    def __init__(
        ...
        self._data = {
            key: _to_immutable(value) for key, value in content.items()
        }

    def clone(self, data):
        return self.__class__(
            self.url, self.title, self.description, self.media_type, data
        )
...
class Link(itypes.Object):
    ...
    def __init__(
        ...
        if (fields is not None) and any(
            ...
            raise TypeError(
                "Argument 'fields' must be a list of strings or fields."
            )
    ...
    def __eq__(self, other):
        return (
            ...
            and sorted(self.fields, key=lambda f: f.name)
            == sorted(other.fields, key=lambda f: f.name)
        )
