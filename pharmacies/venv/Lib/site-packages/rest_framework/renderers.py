...
class BrowsableAPIRenderer(BaseRenderer):
    ...
    def get_content(
        self, renderer, data, accepted_media_type, renderer_context
    ):
        """
    ...
    def get_context(self, data, accepted_media_type, renderer_context):
        ...
        if (
            getattr(view, "paginator", None)
            and view.paginator.display_page_controls
        ):
        ...
        return {
            ...
            "put_form": self.get_rendered_html_form(
                data, view, "PUT", request
            ),
            "post_form": self.get_rendered_html_form(
                data, view, "POST", request
            ),
...
class AdminRenderer(BrowsableAPIRenderer):
    ...
    def get_result_url(self, result, view):
        ...
        """
        if not hasattr(view, "reverse_action") or not hasattr(
            view, "lookup_field"
        ):
            return
        ...
        lookup_url_kwarg = (
            getattr(view, "lookup_url_kwarg", None) or lookup_field
        )
...
class DocumentationRenderer(BaseRenderer):
    ...
    def get_context(self, data, request):
        return {
            ...
            "lang_intro_htmls": [
                "rest_framework/docs/langs/%s-intro.html" % l
...
class CoreJSONRenderer(BaseRenderer):
    ...
    def __init__(self):
        assert (
            coreapi
        ), "Using CoreJSONRenderer, but `coreapi` is not installed."
...
class CoreAPIOpenAPIRenderer(_BaseOpenAPIRenderer):
    ...
    def __init__(self):
        assert (
            coreapi
        ), "Using CoreAPIOpenAPIRenderer, but `coreapi` is not installed."
        assert (
            yaml
        ), "Using CoreAPIOpenAPIRenderer, but `pyyaml` is not installed."
