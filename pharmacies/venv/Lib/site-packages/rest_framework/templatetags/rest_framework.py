...
def optional_logout(request, user):
    ...
    try:
        ...
        snippet = format_html(
            '<li class="navbar-text">{user}</li>', user=escape(user)
        )
        return mark_safe(snippet)
...
def format_value(value):
    ...
    if value is None or isinstance(value, bool):
        ...
        if (
            ...
            return mark_safe(
                '<a href="mailto:{value}">{value}</a>'.format(
                    value=escape(value)
                )
            )
        elif "\n" in value:
...
def schema_links(section, sec_key=None):
    ...
    """
    NESTED_FORMAT = (
        "%s > %s"  # this format is used in docs/js/api.js:normalizeKeys
    )
    links = section.links
    if section.data:
...
def urlize_quoted_links(
    text, trim_url_limit=None, nofollow=True, autoescape=True
):
    """
    ...
    for i, word in enumerate(words):
        if "." in word or "@" in word or ":" in word:
            ...
            if url:
                ...
                lead, trail = (
                    conditional_escape(lead),
                    conditional_escape(trail),
                )
                url, trimmed = (
                    conditional_escape(url),
                    conditional_escape(trimmed),
                )
