...
class GitWildMatchTest(unittest.TestCase):
    ...
    def test_01_absolute(self):
        ...
        self.assertEqual(
            results, {"an/absolute/file/path", "an/absolute/file/path/foo",}
        )
    ...
    def test_01_relative(self):
        ...
        results = set(
            pattern.match(
                ["spam", "spam/", "foo/spam", "spam/foo", "foo/spam/bar",]
            )
        )
        self.assertEqual(
            results, {"spam", "spam/", "foo/spam", "spam/foo", "foo/spam/bar",}
        )
    ...
    def test_01_relative_nested(self):
        ...
        pattern = GitWildMatchPattern(re.compile(regex), include)
        results = set(
            pattern.match(["foo/spam", "foo/spam/bar", "bar/foo/spam",])
        )
        self.assertEqual(results, {"foo/spam", "foo/spam/bar",})
    ...
    def test_03_parent_double_asterisk(self):
        ...
        pattern = GitWildMatchPattern(re.compile(regex), include)
        results = set(pattern.match(["foo/spam", "foo/spam/bar",]))
        self.assertEqual(results, {"foo/spam", "foo/spam/bar",})
    ...
    def test_04_prefix_wildcard(self):
        ...
        results = set(
            pattern.match(
                ["bar.py", "bar.py/", "foo/bar.py", "foo/bar.py/baz",]
            )
        )
        self.assertEqual(
            results, {"bar.py", "bar.py/", "foo/bar.py", "foo/bar.py/baz",}
        )
    ...
    def test_05_directory(self):
        ...
        pattern = GitWildMatchPattern(re.compile(regex), include)
        results = set(
            pattern.match(["dir/", "foo/dir/", "foo/dir/bar", "dir",])
        )
        self.assertEqual(results, {"dir/", "foo/dir/", "foo/dir/bar",})
    ...
    def test_06_registered(self):
		...
		"""
        self.assertIs(
            pathspec.util.lookup_pattern("gitwildmatch"), GitWildMatchPattern
        )
    ...
    def test_06_access_deprecated(self):
        ...
        self.assertTrue(
            issubclass(pathspec.GitIgnorePattern, GitWildMatchPattern)
        )
    ...
    def test_07_encode_bytes(self):
		...
		Test encoding bytes.
		"""
        encoded = "".join(map(unichr, range(0, 256))).encode(
    ...
    def test_07_decode_bytes(self):
		...
		Test decoding bytes.
		"""
        decoded = bytes(bytearray(range(0, 256))).decode(
            pathspec.patterns.gitwildmatch._BYTES_ENCODING
        )
    ...
    def test_07_match_bytes_and_bytes_complete(self):
        ...
        escaped = b"".join(
            b"\\" + encoded[i : i + 1] for i in range(len(encoded))
        )
