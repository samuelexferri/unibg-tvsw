...
class Parser(object):
    ...
    def __init__(self, loader):
        ...
        if (
            ...
            and getattr(self.loader, "_parser", None) is None
        ):
    ...
    def parse_document_content(self):
        ...
        if self.scanner.check_token(
            ...
            event = self.process_empty_scalar(
                self.scanner.peek_token().start_mark
            )
    ...
    def parse_block_mapping_key(self):
        ...
        if self.resolver.processing_version > (
            1,
            1,
        ) and self.scanner.check_token(ValueToken):
            self.state = self.parse_block_mapping_value
    ...
    def parse_block_mapping_value(self):
        ...
        if self.scanner.check_token(ValueToken):
            ...
            if not self.scanner.check_token(
                ...
                return self.process_empty_scalar(
                    token.end_mark, comment=comment
                )
        else:
            self.state = self.parse_block_mapping_key
            token = self.scanner.peek_token()
    ...
    def process_empty_scalar(self, mark, comment=None):
        # type: (Any, Any) -> Any
        return ScalarEvent(
            None, None, (True, False), "", mark, mark, comment=comment
        )
