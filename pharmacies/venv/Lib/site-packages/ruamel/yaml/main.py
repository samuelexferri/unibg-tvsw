...
class YAML(object):
    def __init__(
        ...
        self.typ = (
            ["rt"]
            if typ is None
            else (typ if isinstance(typ, list) else [typ])
        )
        self.pure = pure
        ...
        for pu in (
            [] if plug_ins is None else plug_ins
        ...
        if setup_rt:
            ...
            self.Constructor = (
                ruamel.yaml.constructor.RoundTripConstructor
            )  # type: Any
        del setup_rt
        self.stream = None
        self.canonical = None
        ...
        if typ_found == 0:
            raise NotImplementedError(
                'typ "{}"not recognised (need to install plug-in?)'.format(
                    self.typ
                )
            )
    ...
    def resolver(self):
        ...
        if not hasattr(self, attr):
            setattr(
                self, attr, self.Resolver(version=self.version, loader=self)
            )
        return getattr(self, attr)
        ...
        if self._context_manager:
            ...
            if _kw is not enforce:
                raise TypeError(
                    ...
                    "two were given ({!r})".format(
                        self.__class__.__name__, _kw
                    )
                )
    ...
    def official_plug_ins(self):
        ...
        res = [
            x.replace(gpbd, "")[1:-3]
            for x in glob.glob(bd + "/*/__plug_in__.py")
        ]
        return res
...
class YAMLContextManager(object):
    def __init__(self, yaml, transform=None):
        ...
        if not hasattr(self._output, "write") and hasattr(
            self._output, "open"
        ):
...
def load(stream, Loader=None, version=None, preserve_quotes=None):
    ...
    if Loader is None:
        warnings.warn(
            UnsafeLoaderWarning.text, UnsafeLoaderWarning, stacklevel=2
        )
        Loader = UnsafeLoader
...
def round_trip_load(stream, version=None, preserve_quotes=None):
    ...
    """
    return load(
        stream, RoundTripLoader, version, preserve_quotes=preserve_quotes
    )
...
def add_constructor(
    tag, object_constructor, Loader=None, constructor=Constructor
):
...
def add_multi_constructor(
    ...
    if Loader is None:
        ...
        if issubclass(Loader, BaseLoader):
            ...
            RoundTripConstructor.add_multi_constructor(
                tag_prefix, multi_constructor
            )
        else:
            raise NotImplementedError
