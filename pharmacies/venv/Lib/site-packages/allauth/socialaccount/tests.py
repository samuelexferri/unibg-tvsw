...
class OAuth2TestsMixin(object):
    ...
    def test_account_tokens(self, multiple_login=False):
        ...
        if self.app:
            ...
            self.assertEqual(
                t.token_secret,
                json.loads(
                    self.get_login_response_json(with_refresh_token=True)
                ).get("refresh_token", ""),
            )
    ...
    def login(self, resp_mock, process="login", with_refresh_token=True):
        ...
        with mocked_response(
            MockedResponse(
                200, response_json, {"content-type": "application/json"}
            ),
            resp_mock,
        ):
            resp = self.client.get(
...
class SocialAccountTests(TestCase):
    ...
    def test_populate_username_in_blacklist(self):
        ...
        self.assertNotIn(
            request.user.username, account_settings.USERNAME_BLACKLIST
        )
    ...
    def test_disconnect(self):
        ...
        account = SocialAccount.objects.create(
            uid="123", provider="twitter", user=user
        )
