...
def push_index(errors, key):
    return [Error(error.text, [key] + error.index) for error in errors]
    ...
    def validate(self, value, context=None):
        ...
        if self.additional_properties:
            for key in remaining_keys:
                error_items = self.additional_properties_schema.validate(
                    value[key], context
                )
                errors += push_index(error_items, key)
        else:
...
class Anything(Schema):
    errors = {"type": "Must be a valid primitive type."}
...
class Union(Schema):
    errors = {"match": "Must match one of the options."}
...
class Not(Schema):
    errors = {"must_not_match": "Must not match the option."}
