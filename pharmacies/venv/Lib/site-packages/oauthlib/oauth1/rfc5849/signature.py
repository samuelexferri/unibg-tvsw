...
def signature_base_string(
    http_method, base_str_uri, normalized_encoded_request_parameters
):
    ...
    if headers:
        ...
        if authorization_header is not None:
            params.extend(
                [
                    i
                    for i in utils.parse_authorization_header(
...
def verify_hmac_sha1(request, client_secret=None, resource_owner_secret=None):
    ...
    signature = sign_hmac_sha1(
        sig_base_str, client_secret, resource_owner_secret
    )
    match = safe_string_equals(signature, request.signature)
    if not match:
        log.debug(
            "Verify HMAC-SHA1 failed: signature base string: %s", sig_base_str
        )
    return match
...
def verify_hmac_sha256(
    ...
    if not match:
        log.debug(
            "Verify HMAC-SHA256 failed: signature base string: %s",
            sig_base_str,
        )
    return match
...
def verify_rsa_sha1(request, rsa_public_key):
    ...
    if not verify_ok:
        log.debug(
            "Verify RSA-SHA1 failed: signature base string: %s", sig_base_str
        )
    return verify_ok
