...
class MetadataEndpoint(BaseEndpoint):
   ...
   """
    ...
    def create_metadata_response(
        self, uri, http_method="GET", body=None, headers=None
    ):
    ...
    def validate_metadata(
        ...
        if key not in array:
            ...
            if not array[key].startswith("http"):
                raise ValueError(
                    "key {}: {} must be an URL".format(key, array[key])
                )

        elif is_list:
            if not isinstance(array[key], list):
                raise ValueError(
                    "key {}: {} must be an Array".format(key, array[key])
                )
    ...
    def validate_metadata_authorization(self, claims, endpoint):
        claims.setdefault(
            ...
            list(
                filter(lambda x: x != "none", endpoint._response_types.keys())
            ),
        )
        ...
        self.validate_metadata(
            claims, "response_modes_supported", is_list=True
        )
    ...
    def validate_metadata_server(self):
       ...
       * Other OPTIONAL fields:
       ...
       Discovery 1.0 [`OpenID.Discovery`_].
        ...
        self.validate_metadata(
            claims, "issuer", is_required=True, is_issuer=True
        )
