...
pchar = (
    r"(?: %(unreserved)s | %(pct_encoded)s | %(sub_delims)s | : | @ )"
    % locals()
)
...
IPv4address = (
    r"%(dec_octet)s \. %(dec_octet)s \. %(dec_octet)s \. %(dec_octet)s"
    % locals()
)
...
IPvFuture = (
    r"v %(HEXDIG)s+ \. (?: %(unreserved)s | %(sub_delims)s | : )+" % locals()
)
...
relative_ref = (
    r"%(relative_part)s (?: \? %(query)s)? (?: \# %(fragment)s)?" % locals()
)
...
absolute_URI = (
    r"^(?: %(scheme)s : %(hier_part)s (?: \? %(query)s )? )$" % locals()
)
