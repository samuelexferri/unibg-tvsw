...
class Retrying(object):
    def __init__(
        ...
        self._stop_max_delay = (
            100 if stop_max_delay is None else stop_max_delay
        )
        self._wait_fixed = 1000 if wait_fixed is None else wait_fixed
        self._wait_random_min = (
            0 if wait_random_min is None else wait_random_min
        )
        self._wait_random_max = (
            1000 if wait_random_max is None else wait_random_max
        )
        ...
        self._wait_incrementing_increment = (
            100
            if wait_incrementing_increment is None
            else wait_incrementing_increment
        )
        self._wait_exponential_multiplier = (
            1
            if wait_exponential_multiplier is None
            else wait_exponential_multiplier
        )
        self._wait_exponential_max = (
            MAX_WAIT if wait_exponential_max is None else wait_exponential_max
        )
        self._wait_jitter_max = (
            0 if wait_jitter_max is None else wait_jitter_max
        )
        ...
        if (
            wait_exponential_multiplier is not None
            or wait_exponential_max is not None
        ):
