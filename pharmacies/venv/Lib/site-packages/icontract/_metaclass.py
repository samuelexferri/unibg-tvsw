    ...
    if key not in ["__init__"]:
        ...
        for base in bases:
            if hasattr(base, key):
                ...
                base_contract_checker = icontract._checkers.find_checker(
                    func=base_func
                )
                ...
                if base_contract_checker is not None:
                    base_preconditions.extend(
                        base_contract_checker.__preconditions__
                    )
                    base_snapshots.extend(
                        base_contract_checker.__postcondition_snapshots__
                    )
                    base_postconditions.extend(
                        base_contract_checker.__postconditions__
                    )
        ...
        postconditions = _collapse_postconditions(
            base_postconditions=base_postconditions,
            postconditions=postconditions,
        )

    if preconditions or postconditions:
        if contract_checker is None:
            contract_checker = icontract._checkers.decorate_with_checker(
                func=func
            )
    ...
    for key, value in namespace.items():
        if inspect.isfunction(value) or isinstance(
            ...
            _decorate_namespace_function(
                bases=bases, namespace=namespace, key=key
            )
