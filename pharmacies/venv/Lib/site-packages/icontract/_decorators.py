    ...
    def __call__(self, func: CallableT) -> CallableT:
        ...
        for snap in snapshots:
            ...
            if snap.name == self._snapshot.name:
                raise ValueError(
                    "There are conflicting snapshots with the name: {!r}".format(
                        snap.name
                    )
                )
...
class ensure:  # pylint: disable=invalid-name
    ...
    def __call__(self, func: CallableT) -> CallableT:
        ...
        assert isinstance(
            getattr(contract_checker, "__postconditions__"), list
        )
...
class invariant:  # pylint: disable=invalid-name
    ...
    def __init__(
        ...
        if (
            self._contract.mandatory_args
            and self._contract.mandatory_args != ["self"]
        ):
            raise ValueError(
