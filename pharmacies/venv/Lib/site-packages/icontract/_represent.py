...
def inspect_decorator(
    ...
    assert (
        len(func_def_node.decorator_list) == 1
...
def find_lambda_condition(
    ...
    return ConditionLambdaInspection(
...
def repr_values(
    ...
    if _is_lambda(a_function=condition):
        ...
        ), "Expected a lambda inspection when given a condition as a lambda function"
    else:
        assert (
            lambda_inspection is None
        ), "Expected no lambda inspection in a condition given as a non-lambda function"
...
def generate_message(
    contract: Contract, condition_kwargs: Mapping[str, Any]
) -> str:
    ...
    if not _is_lambda(a_function=contract.condition):
        ...
        lambda_inspection = find_lambda_condition(
