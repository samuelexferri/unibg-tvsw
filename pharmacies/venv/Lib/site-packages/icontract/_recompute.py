...
class Visitor(ast.NodeVisitor):
    ...
    def visit_List(self, node: ast.List) -> List[Any]:
        ...
        if isinstance(node.ctx, ast.Store):
            raise NotImplementedError(
                "Can not compute the value of a Store on a list"
            )
    ...
    def visit_Tuple(self, node: ast.Tuple) -> Tuple[Any, ...]:
        ...
        if isinstance(node.ctx, ast.Store):
            raise NotImplementedError(
                "Can not compute the value of a Store on a tuple"
            )
    ...
    def visit_UnaryOp(self, node: ast.UnaryOp) -> Any:
        ...
        if isinstance(node.op, ast.UAdd):
            ...
            raise NotImplementedError(
                "Unhandled op of {}: {}".format(node, node.op)
            )
    ...
    def visit_BoolOp(self, node: ast.BoolOp) -> Any:
        ...
        if isinstance(node.op, ast.And):
            result = functools.reduce(
                lambda left, right: left and right, values, True
            )
        elif isinstance(node.op, ast.Or):
            result = functools.reduce(
                lambda left, right: left or right, values, True
            )
        else:
            raise NotImplementedError(
                "Unhandled op of {}: {}".format(node, node.op)
    ...
    def visit_Compare(self, node: ast.Compare) -> Any:
        ...
        comparators = [
            self.visit(node=comparator) for comparator in node.comparators
        ]
        ...
        for comparator, op in zip(comparators, node.ops):
            if isinstance(op, ast.Eq):
                ...
                raise NotImplementedError(
                    "Unhandled op of {}: {}".format(node, op)
                )
    ...
    def _execute_comprehension(
        ...
        args = [
            ast.arg(arg=name) for name in sorted(self._name_to_value.keys())
        ]

        if platform.python_version_tuple() < ("3",):
            raise NotImplementedError(
                "Python versions below not supported, got: {}".format(
    ...
    ) -> Any:  # pylint: disable=no-self-use
    ...
    def generic_visit(self, node: ast.AST) -> None:
        ...
        raise NotImplementedError(
            "Unhandled recomputation of the node: {} {}".format(
                type(node), node
            )
        )
