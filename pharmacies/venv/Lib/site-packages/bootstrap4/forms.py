...
def render_button(
    ...
    if size == "xs":
        ...
        raise BootstrapError(
            'Parameter "size" should be "xs", "sm", "lg" or empty ("{size}" given).'.format(
                size=size
            )
        )
...
def render_field_and_label(
    ...
    **kwargs
    ...
    if label:
        html = (
            render_label(label, label_for=label_for, label_class=label_class)
            + html
        )
    return html
...
def render_form_group(content, css_class=FORM_GROUP_CLASS):
    ...
    return '<div class="{css_class}">{content}</div>'.format(
