...
class FieldRenderer(BaseRenderer):
    ...
    def add_placeholder_attrs(self, widget=None):
        ...
        if (
            placeholder
            and self.set_placeholder
            and is_widget_with_placeholder(widget)
        ):
        ...
        for label in soup.find_all("label"):
            ...
            if input_id:
                ...
                try:
                    ...
                    pass
            if label_input:
                label_input.attrs["class"] = label_input.attrs.get(
                    "class", []
                ) + ["form-check-input"]
        return str(soup)
    ...
    def fix_date_select_input(self, html):
        ...
        return '<div class="row bootstrap4-multi-input">{html}</div>'.format(
            html=html
        )
    ...
    def make_input_group(self, html):
        if self.is_input_group:
            ...
            html = self.append_errors(
                "{before}{html}{after}".format(
                    before=before, html=html, after=after
                )
            )
