...
def _save(im, fp, filename, save_all=False):
    ...
    existing_pdf.write_comment(
        "created by Pillow {} PDF driver".format(__version__)
    )
    ...
    for imSequence in ims:
        im_pages = (
            ImageSequence.Iterator(imSequence) if save_all else [imSequence]
        )
        for im in im_pages:
            ...
            if filter == "ASCIIHexDecode":
                if bits == 1:
                    ...
                    im.putdata(data)
                ImageFile._save(
                    im, op, [("hex", (0, 0) + im.size, 0, im.mode)]
                )
            elif filter == "DCTDecode":
                Image.SAVE["JPEG"](im, op, filename)
            elif filter == "FlateDecode":
                ImageFile._save(
                    im, op, [("zip", (0, 0) + im.size, 0, im.mode)]
                )
            ...
            existing_pdf.write_obj(
                contents_refs[pageNumber], stream=page_contents
            )
