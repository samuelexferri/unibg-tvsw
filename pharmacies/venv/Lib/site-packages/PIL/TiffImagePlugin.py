...
class ImageFileDirectory_v2(MutableMapping):
    ...
    def named(self):
        ...
        """
        return {
            TiffTags.lookup(code).name: value for code, value in self.items()
        }
    ...
    def load(self, fp):
        ...
        try:
            for i in range(self._unpack("H", self._ensure_read(fp, 2))[0]):
                tag, typ, count, data = self._unpack(
                    "HHL4s", self._ensure_read(fp, 12)
                )
                if DEBUG:
                ...
                if size > 4:
                    ...
                    if DEBUG:
                        print(
                            "Tag Location: {} - Data Location: {}".format(
                                here, offset
                            ),
                            end=" ",
                        )
...
class AppendingTiffWriter:
    ...
    def rewriteLastShortToLong(self, value):
        ...
        if bytesWritten is not None and bytesWritten != 4:
            raise RuntimeError(
                "wrote only %u bytes but wanted 4" % bytesWritten
            )
    ...
    def rewriteLastShort(self, value):
        ...
        if bytesWritten is not None and bytesWritten != 2:
            raise RuntimeError(
                "wrote only %u bytes but wanted 2" % bytesWritten
            )
    ...
    def fixIFD(self):
        ...
        for i in range(numTags):
            tag, fieldType, count = struct.unpack(
