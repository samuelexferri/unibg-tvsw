...
try:
    ...
    if __version__ != getattr(core, "PILLOW_VERSION", None):
        raise ImportError(
            ...
            "Core version: %s\n"
            "Pillow version: %s"
            % (getattr(core, "PILLOW_VERSION", None), __version__)
        )
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def tobitmap(self, name="image"):
        ...
        return b"".join(
            [
                ("#define %s_width %d\n" % (name, self.size[0])).encode(
                    "ascii"
                ),
                ("#define %s_height %d\n" % (name, self.size[1])).encode(
                    "ascii"
                ),
                ("static char %s_bits[] = {\n" % name).encode("ascii"),
                data,
    ...
    def quantize(
        ...
        im.palette = ImagePalette.ImagePalette(
            mode, im.im.getpalette(mode, mode)
        )
    ...
    def remap_palette(self, dest_map, source_palette=None):
        ...
        for i, oldPosition in enumerate(dest_map):
            palette_bytes += real_source_palette[
                oldPosition * 3 : oldPosition * 3 + 3
            ]
        ...
        new_palette_bytes = (
            palette_bytes + (768 - len(palette_bytes)) * b"\x00"
        )
    ...
    def rotate(
        ...
        return self.transform(
            (w, h), AFFINE, matrix, resample, fillcolor=fillcolor
        )
    ...
    def thumbnail(self, size, resample=BICUBIC, reducing_gap=2.0):
        ...
        if self.size != size:
            im = self.resize(
                size, resample, box=box, reducing_gap=reducing_gap
            )
    ...
    def transform(
        ...
        if method == MESH:
            ...
            for box, quad in data:
                im.__transformer(
                    box, self, QUAD, quad, resample, fillcolor is None
                )
        else:
            im.__transformer(
    ...
    def __transformer(
        self, box, image, method, data, resample=NEAREST, fill=1
    ):
        w = box[2] - box[0]
        h = box[3] - box[1]
