...
def _parse_document(data, base_url=None):
    ...
    for path in paths.keys():
        ...
        for action in spec.keys():
            ...
            fields = []
            parameters = get_dicts(
                _get_list(operation, "parameters", default_parameters),
            ...
            for parameter in parameters:
                ...
                required = _get_bool(
                    parameter, "required", default=(location == "path")
                )
                if location == "body":
                    has_body = True
                    schema = _get_dict(
                        parameter, "schema", dereference_using=data
                    )
...
def _expand_schema(schema):
    ...
    ) and schema_properties:
        return [
            (
                key,
                key in schema_required,
                schema_properties[key].get("description"),
            )
...
def get_dicts(item, dereference_using=None):
    ...
    if dereference_using:
        return [
            dereference(value["$ref"], dereference_using)
